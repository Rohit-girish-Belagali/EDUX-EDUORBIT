...
import type { StreamEvent, LLMSelection } from "./ws";
...
export async function listSessions(
  ...
  return withClientCache<SessionSummary[]>(
    ...
    async () => {
      const response = await apiFetch(apiUrl(`/api/v1/sessions?${qs.toString()}`), { cache: "no-store" });
...
export async function getSession(sessionId: string, signal?: AbortSignal): Promise<SessionDetail> {
  const response = await apiFetch(apiUrl(`/api/v1/sessions/${sessionId}`), { cache: "no-store", signal });
...
export async function deleteSession(sessionId: string): Promise<void> {
  const response = await apiFetch(apiUrl(`/api/v1/sessions/${sessionId}`), { method: "DELETE" });
