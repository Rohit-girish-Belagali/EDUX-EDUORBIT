# EDUX Containerization

This document covers deploying EDUX from a container image: the
...
The published `ghcr.io/eduorbit/edux` image runs both the FastAPI
...
- `web/proxy.ts` catches `/api/*` and `/ws/*` and rewrites them to
  `EDUX_API_BASE_URL` at request time. That env var is set by the
...
- `supervisord` runs as root (PID 1) and drops each program (backend,
  frontend) to a non-root `edux` user (UID 1000) via its per-program
...
memory, and knowledge bases persist in the `edux-data` named volume.
...
- **Only `3782` needs to be published.** The browser talks exclusively to
  ...
  middleware (`web/proxy.ts`), which reads `EDUX_API_BASE_URL`
...
- **Detached:** add `-d`, then `docker logs -f edux` to follow,
  `docker stop edux` to stop, `docker rm edux` before reusing
  the name. The `edux-data` volume keeps your settings and workspace
  across restarts.
...
(`https://edux.example.com`), and the in-container Next.js middleware
...
on the host (inside the `edux-data` volume — `docker volume inspect
edux-data` to find its mountpoint) and set the in-network address the
...
`EDUX_API_BASE_URL` for `proxy.ts` (precedence: `next_public_api_base`,
then `next_public_api_base_external`, then `http://localhost:8001`). Note
that because the proxy is **server-side**, `EDUX_API_BASE_URL` is the
...
EDUX permits normal HTTP/HTTPS browser origins by default. With
...
point. It pulls the same `ghcr.io/eduorbit/edux:latest` image and
...
podman compose -f compose.yaml logs -f edux
```
...
- **`userns_mode: keep-id`.** The container's UID 0 maps to your host
  UID; the container's UID 1000 (the `edux` user inside the image)
...
non-root `edux` user (UID 1000) — `podman exec edux ps -o user,pid,comm`
shows the `uvicorn`/`node` children as `edux`. `supervisord` itself
...
programs are dropped to `edux` via the per-program `user=` directives.
...
- **`system.json` → `next_public_api_base`** (in-network) and
  **`next_public_api_base_external`** (cloud/external override). The
  entrypoint reads these and exports `EDUX_API_BASE_URL`, which
...
service up alongside the main `edux` service. With it running, the
...
always a backend that failed to start (check `docker logs edux` for the
`[program:backend]` lines) or a wrong `EDUX_API_BASE_URL` in a split
...
`ghcr.io/eduorbit/edux:latest` again.
...
- The image drops privileges to a non-root `edux` user (UID 1000)
...
- Auth (`data/user/settings/auth.json` → `auth_enabled = true`) gates
  ...
  `EDUX_AUTH_ENABLED` (exported by the entrypoint on every start)
