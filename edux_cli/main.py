"""CLI entry point for the standalone ``edux-cli`` package."""
...
from edux.runtime.mode import RunMode, set_mode
...
app = typer.Typer(
    name="edux",
    help="EDUX CLI – agent-first interface for capabilities, tools, and knowledge.",
...
app.add_typer(skill_app, name="skills")  # alias: `edux skills …`
...
def run_capability(
    ...
    maybe_run(run_turn_and_render(app=EDUXApp(), request=request, fmt=fmt))
...
def start(
    ...
    from edux.runtime.launcher import start as start_web
...
def serve(
    ...
    """Start the EDUX API server."""
    import asyncio
    import sys

    set_mode(RunMode.SERVER)
    if port is None:
        from edux.services.setup import get_backend_port
