...
# lives at module scope so a single ``edux chat`` session shares one
# ring across turns; ``edux run`` doesn't read it (single-shot mode),
...
async def stream_turn_as_json(*, app: EDUXApp, turn_id: str) -> None:
...
async def render_turn_stream(*, app: EDUXApp, turn_id: str) -> str:
...
class TurnStreamRenderer:
    ...
    async def _on_wait_for_input(self, item: dict[str, Any]) -> None:
        ...
        from edux.core.stream_bus import get_bus
