...
``eduhub`` (EDUX's native open skill registry). Installs run the full
...
def _resolve_token(explicit: str | None, hub: str) -> str | None:
    ...
    from edux.services.skill import credentials

    return (
        explicit
        or os.environ.get("EDUX_HUB_TOKEN")
...
def _summary_table(
    ...
    *,
    ...
    from edux.services.skill.taxonomy import domain_label, track_label
...
def register(app: typer.Typer) -> None:
    ...
    def skill_search(
        ...
        """Search a skill hub."""
        from edux.services.skill.hub import HubError, default_hub, get_hub_provider
        ...
        console.print(table)
        console.print("[dim]Install with: edux skill install <ref>[/]")
    ...
    def skill_install(
        ...
        from edux.services.skill.hub import HubError, install_from_hub
    ...
    def skill_login(
        ...
        from edux.services.skill import credentials
        from edux.services.skill.hub import HubError, default_hub, get_hub_provider
        from edux.services.skill.taxonomy import Option
    ...
    def skill_logout(
        ...
        """清除本地保存的某个 hub 登录令牌。"""
        from edux.services.skill import credentials
        from edux.services.skill.hub import default_hub
    ...
    def skill_publish(
        ...
        token: str | None = typer.Option(
            None, "--token", help="Publish token; else env / `edux skills login`."
        ),
        ...
        from edux.services.skill.hub import (
            HubError,
            default_hub,
            preflight_skill_dir,
            publish_to_hub,
        ...
        from edux.services.skill.service import SkillImportError
        ...
        if not pre.ok:
            ...
            console.print("  [dim]格式规范见 https://eduhub.edux.info/skill-format.md[/]")
            raise typer.Exit(code=1)
        console.print(
            f"[green]✓[/] 格式预检通过 "
        ...
        if not tok:
            console.print(
                "[bold red]未登录。[/] 运行 [bold]edux skills login[/] 完成浏览器授权，"
                "或用 --token / $EDUX_HUB_TOKEN 传入令牌。"
            )
            raise typer.Exit(code=1)
        ...
        console.print(
            f"  [dim]install with:[/] edux skill install {outcome.hub}:{outcome.slug}"
        )
    ...
    def skill_list() -> None:
        ...
        from edux.services.skill.service import get_skill_service
