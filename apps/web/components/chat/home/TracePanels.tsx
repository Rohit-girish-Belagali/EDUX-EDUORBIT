...
function getTraceHeader(
  ...
  if (
    ...
    // top-level status row carries the verbose "EDUX Reflecting…"
...
function detectStreamingMode(
  ...
  for (let idx = events.length - 1; idx >= 0; idx -= 1) {
    ...
    if (event.type === "content" && callKind === "llm_final_response") {
      ...
      // still open; keep the top-level title on "EDUX Exploring…" until
      // the bus moves on.
      if (event.stage === "exploring") return "exploring";
...
export function StreamingStatus({
  ...
  const name = agentName?.trim() || "EDUX";
  ...
  else if (mode === "responded") modeLabel = t("EDUX responded.");
 ...
 * reasoning trace once EDUX stops working and starts (or has finished)
 * its answer.
 *
 ...
 * ("EDUX Exploring… · 8s", settling to "EDUX responded. · 10s")
 * with the exploring trace nested directly beneath it.
 *
 * The trace is expanded by default while EDUX is still reasoning /
