...
/** "EDUX_Introduction.pdf" → "EDUX Introduction" — the card title
  ...
  return (
    <>
      {/* Activity block pinned to the TOP: the status header
          ("EDUX Exploring… · 8s" → "EDUX responded. · 10s") with
          the exploring trace nested beneath it — expanded while EDUX is
                ...
                <div className="mt-3 flex w-full max-w-[min(520px,90%)] items-center gap-2 rounded-xl border border-[var(--destructive)]/30 bg-[var(--destructive)]/5 px-3 py-2">
                  ...
                  <span className="min-w-0 flex-1 text-[12px] leading-[1.5] text-[var(--foreground)]">
                    {isKeyIssue ? (
                      <>
                        {t(
                          "The shared API key ran out or stopped working.",
                        )}{" "}
                        <Link
                          href="/settings/api-key"
                          className="font-medium underline underline-offset-2 hover:text-[var(--destructive)]"
                        >
                          {t("Add your own key in Settings")}
                        </Link>{" "}
                        {t("to keep chatting.")}
                      </>
                    ) : (
