...
export type UiSettings = {
  theme: "light" | "dark" | "glass" | "snow" | "neon";
  language: "en" | "zh";
};
...
const DIAGNOSTICS_RESULTS_KEY = "edux.settings.diagnosticsResults.v1";
...
export function SettingsProvider({ children }: { children: ReactNode }) {
  ...
  const [theme, setTheme] = useState<UiSettings["theme"]>("neon");
  ...
  const loadSettings = useCallback(async () => {
    ...
    try {
      ...
      // Apply the account-stored theme to the document (and localStorage) so
      // every browser converges on the account preference. Without this, a
      // stale localStorage value from a previous theme wins forever because
      // ThemeScript only reads localStorage.
