...
const PRIMARY_NAV: NavEntry[] = [
  ...
  {
    // Friends is ungated — connecting with people and chatting needs no
    // per-user model grant.
    href: "/friends",
    label: "Friends",
    icon: Users,
    tooltipKey: "Friends tooltip",
  },
  {
    href: "/co-writer",
    label: "Co-Writer",
  ...
  {
    // Timetable Planner is pulled out of the Learning Space the same way My
    // Agents / Memory / Knowledge Center were, for faster access. It stays
    // reachable from the Learning Space hub too — this is an added shortcut,
    // not a move. Ungated — planning a schedule needs no per-user model grant.
    href: "/space/timetable",
    label: "Timetable",
    icon: CalendarClock,
    tooltipKey: "Timetable tooltip",
  },
  {
    href: "/space",
...
const GITHUB_REPO_URL = "https://github.com/eduorbit/EDUX";
const DOCS_URL = "https://eduorbit.com/";
const RECENTS_COLLAPSED_KEY = "edux.sidebar.recentsCollapsed";
...
interface SidebarShellProps {
  ...
  /**
   * Content rendered as its own section between the primary nav and Chat
   * History (e.g. the Upcoming Timetable widget). Manages its own
   * fetch/collapse state internally — this shell just gives it a slot.
   * Only shown in the expanded rail; there's no room for it collapsed.
   */
...
export function SidebarShell({
  ...
  // Below the `lg` breakpoint the sidebar never occupies permanent layout
  // space — it's an off-canvas drawer toggled by a fixed hamburger button.
  // `collapsed` (the desktop rail preference) is ignored on mobile: there's
  // no room for an icon-only rail, so the drawer always shows full labels.
  ...
  // Body scroll lock + Escape-to-close while the drawer is open, matching
  // the Modal / PickerShell contract elsewhere in the app.
  useEffect(() => {
  ...
  // Primary/secondary nav links close the mobile drawer on click; this is a
  // no-op when rendered in the desktop static sidebar (setting false→false).
  const handleNavLinkClick =
    (item: NavEntry) => (event: React.MouseEvent) => {
      if (item.href === "/home") handleHomeClick(event);
      setMobileNavOpen(false);
    };

  // Mobile drawer body: always the full labeled nav (never the icon rail —
  // there's no room-saving reason to collapse it once it's an overlay), with
  // a close (X) button in place of the desktop collapse toggle.
    ...
    <>
      <div className="flex h-14 items-center justify-between px-4">
        ...
        <button
          ...
          aria-label={t("Close navigation") as string}
        >
          <X size={17} />
        </button>
      </div>
  ...
  // Mobile chrome shared by both desktop-collapsed and desktop-expanded
  // branches below: a slim persistent trigger rail (never overlaps page
  // header content, unlike a floating fixed button) plus the off-canvas
  // drawer + backdrop it opens.
  const mobileChrome = (
    <>
      <div className="flex h-screen w-12 shrink-0 flex-col items-center bg-[var(--secondary)] py-3 lg:hidden">
        <button
          onClick={() => setMobileNavOpen(true)}
          className="flex h-9 w-9 items-center justify-center rounded-lg text-[var(--foreground)]/85 transition-colors hover:bg-[var(--background)]/60 hover:text-[var(--foreground)]"
          aria-label={t("Open navigation") as string}
        >
      ...
      <aside
        role="dialog"
        aria-modal="true"
        aria-label={t("Navigation") as string}
        className={`fixed inset-y-0 left-0 z-50 flex w-[260px] flex-col overflow-y-auto bg-[var(--secondary)] shadow-xl transition-transform duration-200 lg:hidden ${
  ...
  /* ---- Collapsed state (desktop rail; lg and above) ---- */
  if (collapsed) {
    return (
      <>
        {mobileChrome}
        <aside className="group/sb relative hidden h-screen w-[60px] shrink-0 flex-col items-center bg-[var(--secondary)] py-3 transition-all duration-200 lg:flex">
  ...
  /* ---- Expanded state (desktop; lg and above) ---- */
  return (
    <>
      {mobileChrome}
      <aside className="hidden h-screen w-[220px] shrink-0 flex-col bg-[var(--secondary)] transition-all duration-200 lg:flex">
