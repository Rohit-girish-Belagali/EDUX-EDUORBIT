...
const MODEL_CHILDREN: SettingsLeaf[] = [
  ...
  },
  {
    ...
    label: { zh: "我的 API 密钥", en: "My API Key" },
    blurb: {
      zh: "共享密钥失效时使用的个人备用密钥。",
      en: "Your own fallback key for when the shared one runs out.",
    },
    icon: KeyRound,
...
const AGENT_CHILDREN: SettingsLeaf[] = [
  {
    ...
    blurb: {
      zh: "EDUX 调用本机 Claude Code 时的模型、推理强度与运行参数。",
  ...
  },
  {
    ...
    blurb: {
      zh: "EDUX 调用本机 Codex 时的模型、推理强度与运行参数。",
...
const STORAGE_PATHS: Record<string, string> = {
  ...
  "/settings/api-key": "data/user/settings/llm_key_override.json",
