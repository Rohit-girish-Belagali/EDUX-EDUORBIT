...
// exports `EDUX_API_BASE_URL` from `data/user/settings/system.json`
// (preferring `next_public_api_base`, then `next_public_api_base_external`,
// then `http://localhost:${BACKEND_PORT}`). In dev (`edux start`) it
// defaults to `http://localhost:8001`.
const API_BASE_URL =
  process.env.EDUX_API_BASE_URL ?? "http://localhost:8001";

const AUTH_ENABLED = parseAuthEnabled(process.env.EDUX_AUTH_ENABLED);
