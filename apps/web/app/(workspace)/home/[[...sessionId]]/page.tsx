...
export default function ChatPage() {
  ...
  const handleSend = useCallback(
    async (content: string) => {
      ...
      // (how many times EDUX may ask it) so the subagent capability uses it.
  ...
  // How many times EDUX may consult the selected agent this turn. Seeded
        ...
        >
          ...
          <div className="mx-auto flex w-full max-w-[960px] flex-1 min-h-0 flex-col overflow-hidden px-6">
            {sessionLoading ? (
              ...
              <div className="flex flex-1 min-h-0 flex-col items-center justify-end overflow-hidden pb-6 sm:pb-10 md:pb-14 animate-fade-in">
                <div className="flex items-center justify-center gap-2.5 sm:gap-3.5 md:gap-4">
                  <img
                    src="/logo_black.png"
                    alt="EDUX"
                    width={64}
                    height={64}
                    className="h-9 w-9 select-none sm:h-12 sm:w-12 md:h-16 md:w-16"
                    draggable={false}
                  />
                  <h1 className="dt-hero-title font-serif text-[22px] font-medium leading-[1.1] tracking-[-0.015em] text-[var(--foreground)] sm:text-[30px] md:text-[40px]">
                    {t(welcomeGreeting)}
                  </h1>
                </div>
              </div>
...
function SubagentTabWatcher({
  ...
  useEffect(() => {
    // Group by turn so all of one turn's consults (EDUX may ask the agent
