# EDUX — Agent-Native Architecture

## Overview

EDUX is an **agent-native** intelligent learning companion organized
...
`edux/capabilities/_shared.py` so every turn emits the same envelope
...
```bash
# Install
pip install edux      # Full app (CLI + Web/API + packaged Web assets)
pip install edux-cli  # CLI-only

# Run any capability
edux run chat "Explain Fourier transform"
edux run deep_solve "Solve x^2=4" -t rag --kb my-kb
edux run visualize "Animate sine wave" --config render_mode=manim_video
...
edux memory show
edux serve --port 8001       # API server only
edux start                   # backend + frontend together
```
...
| Path                                       | Purpose                              |
| ------------------------------------------ | ------------------------------------ |
| `edux/runtime/orchestrator.py`        | `ChatOrchestrator` — unified entry   |
| `edux/runtime/launcher.py`            | Backend + frontend lifecycle / port discovery |
| `edux/runtime/registry/`              | Tool + Capability registries         |
| `edux/runtime/bootstrap/builtin_capabilities.py` | Built-in capability class paths |
| `edux/services/config/runtime_settings.py` | JSON settings + process-env overrides |
| `edux/core/stream.py`, `stream_bus.py` | StreamEvent protocol + async fan-out |
| `edux/core/tool_protocol.py`          | `BaseTool` + `ToolDefinition`         |
| `edux/core/capability_protocol.py`    | `BaseCapability` + `CapabilityManifest` |
| `edux/core/context.py`                | `UnifiedContext` dataclass            |
| `edux/tools/builtin/__init__.py`      | All built-in tool wrappers           |
| `edux/capabilities/`                  | Built-in capability implementations  |
| `edux/app.py`                         | `EDUXApp` — Python SDK facade    |
| `edux_cli/main.py`                    | Typer CLI entry point                |
| `edux/api/routers/unified_ws.py`      | Unified WebSocket endpoint           |
...
```
pip install edux      — Full app (CLI + Web/API + packaged Web assets)
pip install edux-cli  — CLI-only (LLM + RAG + providers + document parsing)
...
.[math-animator]  — Manim addon (powers `visualize` Manim renders + `edux run math_animator`)
