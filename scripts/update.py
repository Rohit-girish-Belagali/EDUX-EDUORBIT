#!/usr/bin/env python
"""Safely update a local EDUX git checkout.
...
def run_update(repo_root: Path, *, assume_yes: bool) -> int:
    ...
    print("Restart EDUX if it is currently running.")
    return 0


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fetch, review, and fast-forward update a local EDUX checkout."
    )
    parser.add_argument(
        "--yes",
        "-y",
    ...
    parser.add_argument(
        ...
        help="Path to the git checkout to update. Defaults to this EDUX repository.",
    )
