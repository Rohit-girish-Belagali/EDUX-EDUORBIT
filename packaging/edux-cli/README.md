...
CLI-only EDUX distribution. It installs the `edux` command and the
...
FastAPI/Uvicorn server dependencies used by `edux start`.
...
the `edux` command at these source files.
