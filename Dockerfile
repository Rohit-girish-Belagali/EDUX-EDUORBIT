# ============================================
# EDUX Multi-Stage Dockerfile
# ============================================
# This Dockerfile builds a production-ready image for EDUX
# containing both the FastAPI backend and Next.js frontend
#
# Build/run:
#   docker build -t edux:local .
#   docker run -p 127.0.0.1:3782:3782 -p 127.0.0.1:8001:8001 \
#     -v edux-data:/app/data edux:local
#
# Prerequisites:
...
COPY edux/__version__.py /app/edux/__version__.py
...
# EDUX_API_BASE_URL (exported by the entrypoint on every start).
...
# Labels
LABEL maintainer="EDUX Team" \
      description="EDUX: AI-Powered Personalized Learning Assistant"
...
ENV PYTHONDONTWRITEBYTECODE=1 \
    ...
    PYTHONIOENCODING=utf-8 \
    NODE_ENV=production \
    EDUX_IGNORE_PROCESS_ENV_OVERRIDES=1
...
# (system.json, default on), exported to EDUX_SANDBOX_ALLOW_SUBPROCESS at
...
# (EDUX_SANDBOX_RUNNER_URL), which build_backend() prefers.
...
# user). Each child (backend/frontend) is dropped to this `edux` user via
# the per-program `user=edux` directive, so the app processes stay
# non-root in either runtime. UID 1000 also matches the host user under
# keep-id with a bind mount on ./data.
RUN groupadd --system --gid 1000 edux \
    && useradd --system --uid 1000 --gid 1000 --no-create-home --shell /usr/sbin/nologin edux \
    && chown -R edux:edux /app/data /app/web/.next
...
# edux user (UID 1000) via per-program `user=edux`; see the note on
...
exec python -m uvicorn edux.api.main:app --host ${BACKEND_HOST} --port ${BACKEND_PORT} --no-access-log
EOF
...
# the configured backend at request time, reading EDUX_API_BASE_URL
...
export EDUX_IGNORE_PROCESS_ENV_OVERRIDES=1
...
from edux.services.setup import init_user_directories
init_user_directories(Path('/app'))
" 2>/dev/null || echo "   ⚠️ Directory initialization skipped (will be created on first use)"

# Idempotent: re-chown /app/data so the unprivileged `edux` user (UID 1000)
# owns it. Cheap on no-op; the only first-start cost is one stat per file.
chown -R edux:edux /app/data 2>/dev/null || true

echo "⚙️  Loading runtime JSON settings..."
eval "$(python - <<'PY'
import shlex
from edux.services.config import export_runtime_settings_to_env
...
# EDUX_API_BASE_URL and EDUX_AUTH_ENABLED are exported by the
# export_runtime_settings_to_env eval above (see render_environment in
# edux/services/config/runtime_settings.py). web/proxy.ts reads them at
...
# `edux start` paths stay in sync.
echo "📌 API Base URL (proxy): ${EDUX_API_BASE_URL:-http://localhost:${BACKEND_PORT}}"
echo "📌 Auth enabled: ${EDUX_AUTH_ENABLED:-false}"
...
# and rootless-keep-id runtimes; children drop to the edux user via
...
# `next dev` runs as the unprivileged edux user (via `user=edux` in
...
RUN mkdir -p /app/web/.next \
    && chown edux:edux /app/web /app/web/.next
...
RUN cat > /etc/supervisor/conf.d/programs.conf <<'EOF'
[program:backend]
command=python -m uvicorn edux.api.main:app --host 0.0.0.0 --port %(ENV_BACKEND_PORT)s --reload --no-access-log
directory=/app
user=edux
autostart=true
autorestart=true
