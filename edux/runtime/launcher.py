"""Local Web launcher for the installed EDUX app."""
...
from edux.runtime.banner import labels_for, print_banner, resolve_language
from edux.runtime.home import EDUX_HOME_ENV, PACKAGE_ROOT, get_runtime_home
...
def _reset_runtime_singletons() -> None:
    """Make a just-selected EDUX_HOME visible to path/config singletons."""
    try:
        from edux.services.path_service import PathService
    ...
    try:
        ...
        pass
    try:
        from edux.services.config.model_catalog import ModelCatalogService
...
def _packaged_web_dir() -> Path | None:
    try:
        ...
        return None
    path = Path(edux_web.__file__).resolve().parent
...
def _copy_packaged_web_if_needed(
    ...
    *,
    ...
    marker = cache / ".edux-web-runtime.json"
...
def _resolve_frontend(
    ...
    *,
    ...
    if packaged is not None:
        if not node:
            raise SystemExit("Node.js 20+ is required to run the packaged EDUX Web app.")
    ...
    raise SystemExit(
        "EDUX Web assets are not installed. Install the full app with `pip install -U edux`, "
...
def start(home: str | Path | None = None) -> None:
    ...
    os.environ[EDUX_HOME_ENV] = str(runtime_home)
    ...
    common_env[EDUX_HOME_ENV] = str(runtime_home)
    ...
    common_env["EDUX_API_BASE_URL"] = backend_url
    common_env["EDUX_AUTH_ENABLED"] = "true" if auth_enabled else "false"
    ...
    backend_cmd = [
        ...
        # middleware (edux/api/main.py) surfaces only non-200s, so routine
