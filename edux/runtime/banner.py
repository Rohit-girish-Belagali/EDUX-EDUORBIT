"""Branded banner + localized labels for ``edux start`` / ``edux init``.
...
from edux.__version__ import __version__
...
LABELS: dict[str, dict[str, str]] = {
    "en": {
        ...
        "init.next_step": "Run `edux start` to launch EDUX.",
        "init.choice": "Choice",
        ...
        "start.frontend_restart_failed": (
            ...
            "Stop PID {pid} and run `edux start` again."
        ),
        ...
        "start.port_in_use": (
            "EDUX cannot start because port(s) already in use: {ports}. "
    ...
    "zh": {
        ...
        "init.next_step": "运行 `edux start` 启动 EDUX。",
        "init.choice": "请选择",
        ...
        "start.frontend_restart_failed": (
            ...
            "请先停止 PID {pid},然后重新运行 `edux start`。"
        ),
