...
from edux.logging import configure_logging
...
def validate_tool_consistency():
    ...
    """
    try:
        from edux.runtime.registry.capability_registry import get_capability_registry
...
async def lifespan(app: FastAPI):
    ...
    try:
        from edux.services.llm import get_llm_client
    ...
    try:
        from edux.events.event_bus import get_event_bus
    ...
    try:
        from edux.services.partners import get_partner_manager
    ...
    try:
        from edux.services.cron import get_cron_service
    ...
    try:
        from edux.services.pocketbase_client import ping_pocketbase
    ...
    try:
        from edux.services.memory import (
...
app = FastAPI(
    ...
    # See: https://github.com/eduorbit/EDUX/issues/112
...
# ones worth seeing.
#
# The `edux.access` logger gets its own INFO stdout handler rather than
...
_access_logger = logging.getLogger("edux.access")
if not any(getattr(h, "_edux_access_handler", False) for h in _access_logger.handlers):
    ...
    _access_handler._edux_access_handler = True  # type: ignore[attr-defined]
...
from edux.multi_user.router import router as multi_user_router  # noqa: E402
...
from edux.api.routers.auth import require_admin, require_auth  # noqa: E402
...
app.include_router(social.router, prefix="/api/v1/social", tags=["social"], dependencies=_auth)
app.include_router(
...
@app.get("/")
async def root():
    return {"message": "Welcome to EDUX API"}


if __name__ == "__main__":
    from edux.api.run_server import main as run_server_main
