...
from edux.i18n.metadata_i18n import tool_description_i18n
from edux.logging import (
    ProcessLogEvent,
    bind_log_context,
...
def _discover_plugins() -> list[Any]:
    try:
        from edux.plugins.loader import discover_plugins
    except Exception:
...
async def _execute_stream(tool_name: str, params: dict[str, Any]) -> AsyncGenerator[str, None]:
    ...
    stdout_stream = _QueueTextStream(
        event_queue, loop, stream=None, logger_name="edux.playground.stdout"
    )
    stderr_stream = _QueueTextStream(
        event_queue, loop, stream=None, logger_name="edux.playground.stderr"
    )
...
async def _execute_capability_stream(
    ...
    if capability_name == "chat" and partner_id:
        from edux.api.routers.partners import (
            ChatMessageRequest,
