...
from edux.api.routers.auth import require_admin
from edux.knowledge.kb_types import SUBAGENT_KB_TYPE
from edux.multi_user.knowledge_access import current_kb_manager
from edux.multi_user.partner_access import assert_partner_allowed, visible_partner_cards
from edux.services.rag.linked_kb import assert_path_allowed
from edux.services.subagent import (
...
async def backend_options():
    ...
    from edux.services.subagent.models import list_backend_options
...
async def sync_backend(kind: str):
    ...
    """
    from edux.services.subagent import get_backend
    from edux.services.subagent.models import sync_backend_options
...
async def create_connection(payload: ConnectSubagentRequest):
    ...
    consult within one EDUX chat lands in that one partner session.
    """
...
async def delete_connection(name: str):
    ...
    from edux.services.subagent.sessions import forget_connection
...
async def message_connection(name: str, payload: SubagentMessageRequest):
    ...
    live session EDUX consults (shared via the cross-turn registry, keyed by
    ...
    from edux.services.subagent import get_backend
    from edux.services.subagent.sessions import get_session, remember_session, session_key
