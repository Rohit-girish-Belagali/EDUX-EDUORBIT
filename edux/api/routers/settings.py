...
from edux.multi_user.model_access import allowed_llm_options
from edux.services.config import (
    get_config_test_runner,
    get_model_catalog_service,
    get_runtime_settings_service,
    resolve_llm_runtime_config,
)
from edux.services.config.origins import normalize_origins
from edux.services.config.user_llm_override import (
    clear_user_llm_override,
    load_user_llm_override,
    save_user_llm_override,
)
from edux.services.embedding.client import reset_embedding_client
from edux.services.llm.client import reset_llm_client
from edux.services.llm.config import clear_llm_config_cache
from edux.services.model_selection import list_llm_options
from edux.services.path_service import get_path_service
from edux.tools.builtin import USER_TOGGLEABLE_TOOL_NAMES
...
DEFAULT_UI_SETTINGS = {
    # "neon" is the futuristic dark cyan theme, the default in the UI.
    "theme": "neon",
    "language": "en",
...
class UISettings(BaseModel):
    theme: Literal["light", "dark", "glass", "snow", "neon"] = "neon"
...
class ThemeUpdate(BaseModel):
    theme: Literal["light", "dark", "glass", "snow", "neon"]
...
class LLMKeyOverrideUpdate(BaseModel):
...
def get_enabled_optional_tools() -> list[str]:
    ...
    """
    from edux.multi_user.tool_access import allowed_optional_tools
...
def _provider_choices() -> dict[str, list[dict[str, str]]]:
    ...
    from edux.services.config.provider_runtime import (
        EMBEDDING_PROVIDERS,
        IMAGEGEN_PROVIDERS,
        STT_PROVIDERS,
        TTS_PROVIDERS,
    ...
    from edux.services.provider_registry import PROVIDERS
...
def _mineru_settings_payload() -> dict[str, Any]:
    ...
    """
    from edux.services.parsing.engines.mineru.backend import local_cli_probe
...
def _document_parsing_payload() -> dict[str, Any]:
    ...
    from edux.services.parsing.engines._install import (
        installable_engines,
        model_downloadable_engines,
    )
    from edux.services.parsing.engines.factory import (
        get_parser,
        list_engines,
    )
    from edux.services.parsing.engines.mineru.backend import local_cli_probe
...
async def test_document_parsing(payload: DocumentParsingTest):
    ...
    _require_settings_admin()
    from edux.services.parsing.engines.factory import get_parser, is_engine_available
    ...
    if not cmd:
        return {
            ...
            "message": (
                ...
                f"(pip install edux[parse-{engine}]) so its CLI is on PATH."
            ),
        }
...
async def document_parsing_job_status(cursor: int = 0):
    _require_settings_admin()
    from edux.services.parsing.engines._install import get_background_job_manager
...
async def start_mineru_models_download(payload: MinerUModelDownloadPayload):
    ...
    """
    _require_settings_admin()
    from edux.services.parsing.engines.mineru.models import (
...
async def mineru_models_download_status(cursor: int = 0):
    _require_settings_admin()
    from edux.services.parsing.engines.mineru.models import get_model_download_manager
...
async def test_mineru_connection(payload: MinerUSettingsUpdate):
    ...
    _require_settings_admin()
    from edux.services.parsing.engines.mineru.cloud import verify_credentials
    from edux.services.parsing.engines.mineru.config import MinerUConfig, MinerUError

    if payload.mode == "local":
        from edux.services.parsing.engines.mineru.backend import (
            local_cli_probe,
            local_cli_version,
        )
...
async def fetch_models_from_provider(payload: FetchModelsPayload):
    ...
    """
    _require_settings_admin()
    from edux.services.llm.factory import fetch_models as fetch_llm_models
...
def _llm_key_override_view(override: dict[str, Any] | None) -> dict[str, Any]:
    if not override or not override.get("enabled"):
        return {"has_key": False, "provider_name": None, "masked_hint": None, "updated_at": None}
    return {
        "has_key": True,
        "provider_name": override.get("provider_name"),
        "masked_hint": override.get("masked_hint"),
        "updated_at": override.get("updated_at"),
    }


@router.get("/llm-key-override")
async def get_llm_key_override():
    """Whether the current user has their own API key saved.

    Any authenticated user — this is a personal fallback for when the shared
    admin-managed key runs out, not an admin-only catalog setting. The raw
    key is never returned, only a masked hint.
    """
    return _llm_key_override_view(load_user_llm_override())


@router.put("/llm-key-override")
async def update_llm_key_override(update: LLMKeyOverrideUpdate):
    provider_name = resolve_llm_runtime_config().provider_name
    override = save_user_llm_override(update.api_key, provider_name)
    return _llm_key_override_view(override)


@router.delete("/llm-key-override")
async def delete_llm_key_override():
    clear_user_llm_override()
    return _llm_key_override_view(None)
...
async def complete_tour(payload: TourCompletePayload | None = None):
    ...
    return {
        "status": "completed",
        "message": "Configuration saved. EDUX will restart shortly.",
        "launch_at": launch_at,
