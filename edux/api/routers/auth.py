...
from edux.services.config import load_auth_settings
...
from edux.multi_user.context import set_current_user, user_from_token_payload
from edux.multi_user.paths import local_admin_user
from edux.services.auth import (
    AUTH_ENABLED,
    POCKETBASE_ENABLED,
    TOKEN_EXPIRE_HOURS,
    TokenPayload,
...
class FirebaseLoginRequest(BaseModel):
    """Payload for the POST /firebase endpoint — an ID token from the client SDK."""
...
def _local_admin_token_payload() -> TokenPayload:
    ...
    ``edux/multi_user/paths.py``.
    """
    from edux.multi_user.models import LOCAL_ADMIN_ID, LOCAL_ADMIN_USERNAME
...
def _get_firebase_app():
    """Lazily initialize the Firebase Admin SDK from FIREBASE_SERVICE_ACCOUNT_PATH.

    Kept out of module-level import so a deployment that never enables
    Firebase login doesn't need the credential file to exist, and so tests
    that import this router don't need firebase-admin installed.
    """
    ...
    if not cred_path:
        raise HTTPException(
            ...
            detail="Firebase auth is not configured (set FIREBASE_SERVICE_ACCOUNT_PATH).",
        )
...
async def firebase_login(body: FirebaseLoginRequest, response: Response) -> dict:
    """Verify a Firebase ID token and issue our own session cookie.

    The frontend authenticates the user against Firebase directly (the
    client SDK), then hands us the resulting ID token. We verify it
    server-side and auto-provision a local shadow user record on first
    sign-in — the local record has no usable password, real auth already
    happened via Firebase — so the rest of the app's session/cookie/
    multi-user machinery (workspaces, admin grants) works unchanged. The
    first person to sign in this way becomes admin, same as the
    password-based flow.
    """
    if not AUTH_ENABLED:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Auth is disabled — enable it in data/user/settings/auth.json first.",
        )
    if POCKETBASE_ENABLED:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Firebase login is not supported alongside PocketBase mode.",
        )
    ...
    username = decoded.get("email") or decoded.get("phone_number") or decoded.get("uid")
    if not username:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Firebase token has no email, phone number, or uid",
        )

    was_empty = is_first_user()
    if get_user_info(username) is None:
        import secrets as _secrets

        add_user(username, _secrets.token_urlsafe(32))

    info = get_user_info(username)
    role = str(info.get("role", "user")) if info else "user"
    user_id = str(info.get("id", "")) if info else ""
    ...
    logger.info(f"User '{username}' logged in via Firebase (role={role!r})")
    return {
        "ok": True,
        "user_id": user_id,
        "username": username,
...
async def update_profile(
    ...
    from edux.multi_user.identity import delete_avatar_file
...
async def upload_avatar(
    ...
    from edux.multi_user.identity import save_avatar_file
...
async def get_avatar_image(
    ...
    from edux.multi_user.identity import get_avatar_file
