...
from edux.api.utils.progress_broadcaster import ProgressBroadcaster
from edux.api.utils.task_id_manager import TaskIDManager
from edux.api.utils.task_log_stream import capture_task_logs, get_task_stream_manager
from edux.knowledge.add_documents import DocumentAdder, remove_raw_document
from edux.knowledge.initializer import KnowledgeBaseInitializer
from edux.knowledge.kb_types import is_connected_kb
from edux.knowledge.manager import KnowledgeBaseManager
from edux.knowledge.naming import validate_knowledge_base_name
from edux.knowledge.progress_tracker import ProgressStage, ProgressTracker
from edux.multi_user.context import get_current_user
from edux.multi_user.knowledge_access import (
    assert_writable,
    current_kb_base_dir,
    current_kb_manager,
...
from edux.services.config import PROJECT_ROOT, load_config_with_main
from edux.services.rag.factory import (
    DEFAULT_PROVIDER,
    GRAPHRAG_PROVIDER,
    LIGHTRAG_PROVIDER,
    PAGEINDEX_PROVIDER,
...
from edux.services.rag.file_routing import FileTypeRouter
from edux.services.rag.linked_kb import (
    LINKABLE_PROVIDERS,
    assert_path_allowed,
    probe_linked_folder,
)
from edux.utils.document_extractor import (
    MAX_EXTRACTED_CHARS_PER_DOC,
    DocumentExtractionError,
    extract_text_from_path,
)
from edux.utils.document_validator import DocumentValidator
from edux.utils.error_utils import format_exception_message
...
def _save_zip_archive(
    ...
    from edux.utils.archive_extractor import ArchiveTooLargeError, safe_extract_zip
...
def _save_uploaded_files(
    ...
    from edux.services.pocketbase_client import is_pocketbase_enabled
...
def _upload_file_to_pb(kb_name: str, filename: str, file_path: Path) -> None:
    ...
    try:
        from edux.services.pocketbase_client import get_pb_client
...
def _assert_provider_ready(provider: str) -> None:
    ...
    if provider == PAGEINDEX_PROVIDER:
        from edux.services.rag.pipelines.pageindex.config import is_pageindex_configured
    ...
    if provider == GRAPHRAG_PROVIDER:
        from edux.services.rag.pipelines.graphrag.config import is_graphrag_available
        ...
        if not is_graphrag_available():
            raise HTTPException(
                ...
                detail=(
                    ...
                    "`pip install 'edux[graphrag]'` on the server before "
                    "creating a GraphRAG knowledge base."
                ),
            )

    if provider == LIGHTRAG_PROVIDER:
        from edux.services.rag.pipelines.lightrag.config import is_lightrag_available
        ...
        if not is_lightrag_available():
            raise HTTPException(
                ...
                detail=(
                    ...
                    "`pip install 'edux[rag-lightrag]'` on the server before "
...
def _enforce_provider_formats(provider: str, files: list[UploadFile]) -> None:
    ...
    if provider != PAGEINDEX_PROVIDER:
        return
    from edux.services.rag.pipelines.pageindex.pipeline import SUPPORTED_EXTENSIONS
...
def _matching_index_is_valid(kb_name: str, matching_version: dict | None) -> bool:
    ...
    if not matching_version:
        return False
    try:
        from edux.services.rag.index_probe import inspect_provider_version
        from edux.services.rag.pipelines.llamaindex.storage import (
...
async def get_rag_providers():
    ...
    try:
        from edux.services.config import get_kb_config_service
        from edux.services.rag.service import RAGService
...
def _pageindex_config_payload() -> dict:
    ...
    from edux.services.config import get_runtime_settings_service
...
async def update_pageindex_pipeline_config(payload: PageIndexConfigUpdate):
    ...
    try:
        from edux.services.config import get_runtime_settings_service
        from edux.services.rag.pipelines.pageindex.config import DEFAULT_API_BASE_URL
...
async def get_rag_pipeline_preflight(provider: str):
    ...
    """
    try:
        from edux.services.rag.preflight import engine_preflight
...
def _model_options_payload(kinds: list[str]) -> dict:
    ...
    """
    from edux.services.config import get_model_catalog_service
...
async def update_kb_config(kb_name: str, config: dict):
    ...
    try:
        from edux.services.config import get_kb_config_service
        from edux.services.rag.index_probe import has_ready_provider_index
...
async def probe_lightrag_server_route(payload: ProbeLightRagServerRequest):
    ...
    """
    from edux.services.rag.pipelines.lightrag_server.probe import probe_server
...
async def connect_lightrag_server_route(payload: ConnectLightRagServerRequest):
    ...
    """
    from edux.services.rag.pipelines.lightrag_server.config import SUPPORTED_MODES
    from edux.services.rag.pipelines.lightrag_server.probe import probe_server
...
async def run_reindex_task(kb_name: str, base_dir: str, task_id: str, signature_hash: str) -> None:
    ...
    with capture_task_logs(task_id):
        try:
            ...
            # provider=None → RAGService resolves the KB's EDUX-bound
...
async def reindex_knowledge_base(
    ...
    try:
        ...
        if provider_uses_embedding_versions(kb_provider):
            from edux.services.rag.embedding_signature import signature_from_embedding_config
            from edux.services.rag.index_versioning import (
...
async def websocket_progress(websocket: WebSocket, kb_name: str):
    ...
    from edux.api.routers.auth import ws_auth_failed, ws_require_auth
    from edux.multi_user.context import reset_current_user
