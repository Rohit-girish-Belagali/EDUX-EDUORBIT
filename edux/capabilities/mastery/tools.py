...
mastery engine (:mod:`edux.learning`).
...
from edux.capabilities.mastery.choices import (
    format_options,
    has_option_bodies,
    parse_options,
...
# builtin tools).
from edux.learning.models import (
    KnowledgePoint,
    KnowledgeType,
    LearningModule,
    PendingQuestion,
)
from edux.learning.policy import (
    QUALITATIVE_TYPES,
    display_mastery,
...
if TYPE_CHECKING:
    from edux.learning.service import LearningService
...
def _new_service() -> LearningService:
    from edux.learning.service import LearningService
    from edux.learning.storage import LearningStore
...
class MasteryGradeTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        if not path_id:
            return _no_path_result()
        from edux.learning.scheduler import SpacedRepetitionScheduler
