...
(resolved by :mod:`edux.capabilities.subagent.binding`). As a
...
from edux.capabilities.protocol import KnowledgeCapability, PromptBlock
from edux.capabilities.subagent.binding import connection_for_turn
from edux.capabilities.subagent.tools import SUBAGENT_TOOL_NAMES
...
class SubagentCapability(KnowledgeCapability):
    ...
    def augment_kwargs(
        ...
        if conn is None:
            return kwargs
        from edux.services.subagent import load_subagent_settings
        from edux.services.subagent.sessions import get_session, session_key
        ...
        # session the user/EDUX built up earlier (and the sidebar shares).
...
_DEFAULT_CONSULT_INSTRUCTION = (
    "You are being consulted programmatically by EDUX on the user's behalf, "
...
def _resolve_budget(context: UnifiedContext) -> int:
    ...
    """
    from edux.services.subagent import load_subagent_settings
    from edux.services.subagent.config import CONSULT_BUDGET_MAX, CONSULT_BUDGET_MIN
...
def _system_text(language: str, name: str, budget: int, kind: str = "") -> str:
    from edux.services.subagent import PARTNER_BACKEND_KIND
