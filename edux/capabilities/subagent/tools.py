...
:class:`~edux.capabilities.subagent.capability.SubagentCapability`, which
...
if TYPE_CHECKING:  # avoid importing the services package at module-load time (cycle)
    from edux.services.subagent import SubagentEvent
...
class ConsultSubagentTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        # Head this round with the question EDUX is putting to the agent, so
        ...
        if result.session_id:
            ...
            if session_key_value:
                from edux.services.subagent.sessions import remember_session
