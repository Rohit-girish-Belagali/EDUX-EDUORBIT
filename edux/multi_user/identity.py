...
# process FastAPI deployments (the ``edux start`` launcher) are fully covered;
...
def new_friend_code() -> str:
    """Generate a short, human-shareable code (e.g. ``EDU-7F3K2Q9H``)."""
    body = "".join(secrets.choice(_FRIEND_CODE_ALPHABET) for _ in range(8))
    return f"EDU-{body}"


def _unique_friend_code(existing_codes: set[str]) -> str:
    for _ in range(100):
        code = new_friend_code()
        if code not in existing_codes:
            return code
    # Astronomically unlikely with a 32^8 keyspace, but never loop forever.
...
def _canonical_record(
    ...
    *,
    ...
    return {
        ...
        "friend_code": str(value.get("friend_code") or ""),
    }
...
def load_users(  # nosec B107 - empty defaults mean "no env fallback supplied".
    ...
    # Backfill friend codes for records that predate this field (or were
    # migrated from a plain password-hash string).
    used_codes = {r["friend_code"] for r in canonical.values() if r.get("friend_code")}
    for record in canonical.values():
        if not record.get("friend_code"):
            record["friend_code"] = _unique_friend_code(used_codes)
            used_codes.add(record["friend_code"])
            changed = True
...
def save_user(username: str, hashed_password: str, role: Role = "user") -> dict[str, Any]:
    ...
    with _USERS_WRITE_LOCK:
        ...
        used_codes = {
            str(r.get("friend_code") or "") for u, r in users.items() if u != username
        }
        used_codes.discard("")
        record = {
            ...
            "friend_code": str(existing.get("friend_code") or "")
            or _unique_friend_code(used_codes),
        }
    ...
    return [
        {
            ...
            "friend_code": str(record.get("friend_code") or ""),
        }
...
def get_user_by_friend_code(code: str) -> tuple[str, dict[str, Any]] | None:
    normalized = code.strip().upper()
    for username, record in load_users().items():
        if str(record.get("friend_code") or "") == normalized:
            return username, record
    return None
