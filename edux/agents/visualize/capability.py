...
from edux.runtime.request_contracts import (
...
class VisualizeCapability(BaseCapability):
    ...
    async def run(self, context: UnifiedContext, stream: StreamBus) -> None:
        from edux.agents.visualize.models import ReviewResult
        from edux.agents.visualize.pipeline import VisualizePipeline
        from edux.agents.visualize.utils import (
            build_fallback_html,
        ...
        *,
        ...
        if importlib.util.find_spec("manim") is None:
            raise RuntimeError(
                ...
                "Install with `pip install 'edux[math-animator]'` "
                "or `pip install -r requirements/math-animator.txt`."
            )

        from edux.agents.math_animator.pipeline import MathAnimatorPipeline
        from edux.agents.math_animator.request_config import MathAnimatorRequestConfig
        from edux.core.trace import build_trace_metadata, new_call_id
        from edux.services.llm.config import get_llm_config
