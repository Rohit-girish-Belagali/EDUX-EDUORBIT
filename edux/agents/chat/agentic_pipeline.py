...
from edux.agents._shared.tool_composition import (
    ToolMountFlags,
...
from edux.agents.chat.agent_loop import AgentLoop
from edux.agents.chat.prompt_blocks import ChatPromptAssembler
from edux.capabilities import (
    LoopCapability,
    active_loop_capabilities,
    any_exclusive_capability_active,
)
from edux.core.agentic import (
    DispatchOutcome,
    LLMClientConfig,
    UsageTracker,
...
from edux.core.agentic.tool_dispatch import MAX_PARALLEL_TOOL_CALLS
from edux.core.context import UnifiedContext
from edux.core.stream_bus import StreamBus
from edux.core.trace import (
    build_trace_metadata,
    derive_trace_metadata,
    merge_trace_metadata,
    new_call_id,
)
from edux.runtime.registry.deferred_tools import (
    DeferredToolLoader,
    render_deferred_tools_manifest,
)
from edux.runtime.registry.tool_registry import get_tool_registry
from edux.services.config import get_chat_params
from edux.services.llm import (
    get_llm_config,
    get_token_limit_kwargs,  # noqa: F401  (re-exported for tests)
    prepare_multimodal_messages,
    supports_tools,  # noqa: F401  (re-exported for tests)
)
from edux.services.llm.context_window import resolve_effective_context_window
from edux.services.prompt import get_prompt_manager
from edux.tools.builtin import PARTNER_BUILTIN_TOOL_NAMES
...
def _drop_unconfigured_generation_tools(tools: list[str]) -> list[str]:
    ...
    if not present:
        return tools
    try:
        from edux.services.config.model_catalog import get_model_catalog_service
...
class AgenticChatPipeline:
    ...
    async def _prepare_deferred_tools(self, context: UnifiedContext) -> None:
        try:
            from edux.services.mcp import get_mcp_manager, load_loaded_tools
            ...
            from edux.multi_user.tool_access import allowed_mcp_tools, combine_whitelists
    ...
    async def _exec_allowed(self, context: UnifiedContext) -> bool:
        try:
            from edux.services.sandbox import IsolationLevel, get_sandbox_service
            ...
            if level is IsolationLevel.SYSTEM:
                ...
                from edux.multi_user.tool_access import exec_override
    ...
    async def _execute_tool_call(
        ...
        *,
        ...
        from edux.core.agentic import execute_tool_call
    ...
    def _augment_tool_kwargs(
        ...
        if tool_name == "rag":
            ...
            from edux.services.sandbox import Mount
    ...
    def _estimate_messages_tokens(messages: list[dict[str, Any]]) -> int:
        from edux.services.session.context_builder import count_tokens
