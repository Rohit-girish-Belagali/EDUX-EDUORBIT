...
def _concat_l3() -> str:
    ...
    """
    from edux.services.memory import get_memory_store, paths
...
def _resolve_partner_id() -> str | None:
    ...
    from edux.multi_user.context import get_current_user_or_none
    from edux.services.partners.scope import PARTNER_USER_PREFIX
...
class PartnerReadTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from edux.multi_user.paths import (
            get_admin_path_service,
            get_current_path_service,
        )
        from edux.services.memory import memory_path_service_override
...
class PartnerMemorizeTool(BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from edux.multi_user.paths import get_current_path_service
        from edux.services.memory import get_memory_store, memory_path_service_override
