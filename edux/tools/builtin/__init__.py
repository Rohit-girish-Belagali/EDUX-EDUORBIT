...
from edux.capabilities.mastery import MASTERY_TOOL_TYPES
from edux.capabilities.obsidian import OBSIDIAN_TOOL_TYPES
from edux.capabilities.solve import SOLVE_TOOL_TYPES
from edux.capabilities.subagent import SUBAGENT_TOOL_TYPES
from edux.core.tool_protocol import BaseTool, ToolDefinition, ToolParameter, ToolResult
from edux.tools.exec_tool import ExecTool
from edux.tools.media_gen_tool import ImagegenTool, VideogenTool
from edux.tools.partner_memory import (
    PARTNER_BUILTIN_TOOL_NAMES,
    PartnerMemorizeTool,
    PartnerReadTool,
    PartnerSearchTool,
)
from edux.tools.prompting import load_prompt_hints
...
class BrainstormTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from edux.tools.brainstorm import brainstorm
...
class RAGTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from edux.tools.rag_tool import rag_search
...
class WebSearchTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from edux.tools.web_search import web_search
...
class CodeExecutionTool(_PromptHintsMixin, BaseTool):
    ...
    it through :mod:`edux.services.sandbox`. No second LLM call, and the
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        from edux.services.sandbox import (
            ExecRequest,
            Mount,
            ResourceLimits,
            get_sandbox_service,
        )
        from edux.services.sandbox.artifacts import (
...
class ReasonTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from edux.tools.reason import reason
...
class PaperSearchToolWrapper(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from edux.tools.paper_search_tool import ArxivSearchTool
...
class GeoGebraAnalysisTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from edux.agents.vision_solver.vision_solver_agent import VisionSolverAgent
        from edux.services.llm.config import get_llm_config
...
class WriteMemoryTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from edux.services.memory import get_memory_store
        from edux.services.memory.trace import TraceEvent
...
class WebFetchTool(_PromptHintsMixin, BaseTool):
    ...
    ``edux.tools.web_fetch`` so this wrapper stays free of network
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from edux.tools.web_fetch import (
            DEFAULT_MAX_CHARS,
...
class ListNotebookTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from edux.tools.list_notebook import list_notebooks_or_records
...
class WriteNoteTool(_PromptHintsMixin, BaseTool):
    ...
    * ``edit`` — patch an existing record's title / body / summary.
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from edux.tools.write_note import write_note
...
class GithubTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from edux.tools.github_query import run_github_query
...
class AskUserTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from edux.tools.ask_user import build_ask_user_payload
...
class ReadSkillTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        ...
        from edux.services.skill.service import (
...
class CronTool(_PromptHintsMixin, BaseTool):
    ...
    async def execute(self, **kwargs: Any) -> ToolResult:
        from edux.tools.cron_tool import run_cron_action
