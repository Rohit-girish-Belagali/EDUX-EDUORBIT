"""Bridge EDUX's runtime config into a GraphRAG ``settings.yaml``.
...
EDUX's already-resolved LLM + embedding runtime config and let
...
* GraphRAG talks to its models via LiteLLM with ``model_provider: openai`` + a
  custom ``api_base`` — which is exactly how EDUX reaches any
...
class GraphRagNotConfiguredError(RuntimeError):
    """Raised when EDUX's LLM / embedding config can't back GraphRAG."""
...
def is_graphrag_available() -> bool:
    ...
    extra: ``pip install 'edux[graphrag]'``. Until it is installed the
...
def query_config_from_settings() -> GraphRagQueryConfig:
    ...
    try:
        from edux.services.config import load_graphrag_settings
...
def build_settings(*, llm_cfg: Any = None, embedding_cfg: Any = None) -> dict[str, Any]:
    """Assemble the GraphRAG ``settings.yaml`` payload from EDUX config.

    ``llm_cfg`` / ``embedding_cfg`` are injectable for tests; in production they
    are resolved from EDUX's catalog. Raises
    :class:`GraphRagNotConfiguredError` if either side has no usable model.
    """
    if llm_cfg is None:
        from edux.services.config import resolve_llm_runtime_config

        llm_cfg = resolve_llm_runtime_config()
    if embedding_cfg is None:
        from edux.services.embedding import get_embedding_config
    ...
    return {
        ...
        # Plain-text input: EDUX's ingestion writes parsed ``.txt`` files
        ...
        # GraphRAG/LanceDB defaults to 3072 dimensions; EDUX must stamp the
