...
(its optional ``markitdown`` reader is just a converter). So EDUX owns the
"document → text" step and hands GraphRAG ready ``.txt`` files. We deliberately
reuse EDUX's existing extraction primitives here so multimodal/parsing
stays a EDUX-side concern (matching how the LlamaIndex pipeline already
