...
EDUX first (see ``ingestion``) so GraphRAG only ever sees ``.txt`` input.
    ...
    def _ensure_available(self) -> None:
        if not gr_config.is_graphrag_available():
            raise gr_config.GraphRagNotAvailableError(
                ...
                "`pip install 'edux[graphrag]'` to use GraphRAG knowledge bases."
            )
...
def _context_to_sources(context_data: dict[str, Any]) -> list[dict[str, Any]]:
    """Map GraphRAG context records into EDUX's source-citation shape."""
