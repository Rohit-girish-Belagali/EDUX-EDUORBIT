...
Documents are turned into a MinerU-style ``content_list`` by EDUX's shared
parse layer (``edux/services/parsing``) — the same bridge the question
...
from edux.services.rag.kb_paths import resolve_kb_dir
...
class LightRagPipeline:
    ...
    def _ensure_available(self) -> None:
        if not lr_config.is_lightrag_available():
            raise lr_config.LightRagNotAvailableError(
                ...
                "`pip install 'edux[rag-lightrag]'` to use LightRAG knowledge bases."
            )
    ...
    async def _ingest(self, rag: Any, file_paths: List[str]) -> int:
        ...
        """
        from edux.services.parsing import ParserError, get_parse_service
