...
fix. All imports are lazy so EDUX runs fine without the optional dependency
installed.

A RAG-Anything instance is built from EDUX's LLM/vision/embedding adapters
...
def build_rag(working_dir: Path) -> Any:
    ...
    # EDUX always feeds RAG-Anything a pre-parsed ``content_list`` (the
    # parse layer runs upstream via EDUX's own ParseService), so
