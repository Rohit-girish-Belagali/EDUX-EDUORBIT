"""Bridge EDUX's runtime config into LightRAG / RAG-Anything.
...
builds the three adapters LightRAG needs from EDUX's already-resolved LLM /
...
Decoupling notes:
* ``llm_model_func`` / ``vision_model_func`` wrap EDUX's unified model
  callables and DROP LightRAG's internal kwargs (``hashing_kv``,
  ``keyword_extraction``, …) so they never leak into ``factory.complete``.
* ``embedding_func`` reuses EDUX's embedding client, wrapped in LightRAG's
...
class LightRagNotConfiguredError(RuntimeError):
    """Raised when EDUX's LLM / embedding config can't back LightRAG."""


def is_lightrag_available() -> bool:
    """True when RAG-Anything (which bundles LightRAG) can be imported.

    Opt-in extra: ``pip install 'edux[rag-lightrag]'``. Until installed the
...
def query_kwargs_from_settings() -> dict:
    ...
    """
    try:
        from edux.services.config import load_lightrag_settings
...
def build_llm_model_func():
    """Wrap EDUX's unified LLM callable for LightRAG.
...
def build_vision_model_func():
    """Wrap EDUX's vision-capable callable for RAG-Anything's image step."""
    from edux.services.llm import get_llm_client
...
def build_embedding_func():
    """Wrap EDUX's embedding client in LightRAG's ``EmbeddingFunc``."""
    from lightrag.utils import EmbeddingFunc

    from edux.services.embedding import get_embedding_client, get_embedding_config
