...
from edux.services.cron.service import CronJob
...
async def _maybe_send_desktop_notification(job: CronJob, text: str) -> None:
    ...
    EDUX still persists/delivers through its normal chat or partner
    ...
    title = f"EDUX: {job.name or 'Reminder'}"
...
async def _execute_chat_job(job: CronJob) -> tuple[str, str | None]:
    ...
    from edux.multi_user.models import CurrentUser
    from edux.multi_user.paths import local_admin_user, scope_for_user, user_context
    from edux.runtime.orchestrator import ChatOrchestrator
    from edux.services.session import get_sqlite_session_store
