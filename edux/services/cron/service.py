...
EDUX runs one server process, so a single in-process scheduler owns
the store.
...
executor lives in :mod:`edux.services.cron.executor`.
"""
...
def get_cron_service() -> CronService:
    ...
    if _service is None:
        ...
        from edux.services.cron.executor import execute_job
