"""
Authentication service for EDUX.
...
Quick setup (single user via data/user/settings/auth.json):
    ...
    3. Generate a password hash:
           python -c "from edux.services.auth import hash_password; print(hash_password('yourpassword'))"
...
from edux.services.config import load_auth_settings, load_integrations_settings
...
if AUTH_ENABLED and not POCKETBASE_ENABLED and not AUTH_SECRET:
    from edux.multi_user.identity import load_or_create_auth_secret
...
def _make_user_record(hashed: str, role: str = "user", created_at: str = "") -> dict[str, Any]:
    ...
    from edux.multi_user.identity import new_user_id
...
def _load_users() -> dict[str, dict]:
    ...
    """
    from edux.multi_user.identity import load_users
...
def add_user(username: str, plain_password: str, role: str = "user") -> None:
    ...
    """
    from edux.multi_user.identity import save_user
...
def list_users() -> list[dict]:
    ...
    from edux.multi_user.identity import list_user_info
...
def delete_user(username: str) -> bool:
    ...
    """
    from edux.multi_user.identity import delete_user as _delete_user
...
def set_role(username: str, role: str) -> bool:
    ...
    from edux.multi_user.identity import set_role as _set_role
...
def set_avatar(username: str, avatar: str) -> bool:
    ...
    """
    from edux.multi_user.identity import set_avatar as _set_avatar
...
def decode_token(token: str) -> TokenPayload | None:
    ...
    if POCKETBASE_ENABLED:
        from edux.services.pocketbase_client import validate_pb_token
...
def authenticate_pb(username: str, password: str) -> tuple[TokenPayload, str] | None:
    ...
    <username>@edux.local to match the email used at registration.
    """
    try:
