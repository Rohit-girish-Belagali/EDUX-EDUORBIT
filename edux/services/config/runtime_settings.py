...
# Document parsing settings. The parse layer (edux/services/parsing)
...
_DEFAULT_MINERU_ENGINE: dict[str, Any] = {
    ...
    # separate conda) so its heavy deps never conflict with EDUX's.
    "local_cli_path": "",
...
# EDUX's legacy text extractors for PDF / Office / text-like files.
...
IGNORE_PROCESS_OVERRIDES_ENV = "EDUX_IGNORE_PROCESS_ENV_OVERRIDES"
...
class RuntimeSettingsService:
    ...
    def render_environment(self) -> dict[str, str]:
        ...
        return {
            ...
            "EDUX_SANDBOX_ALLOW_SUBPROCESS": _bool_env(system["sandbox_allow_subprocess"]),
            ...
            # rewrites /api/* and /ws/* to EDUX_API_BASE_URL and uses
            # EDUX_AUTH_ENABLED to gate the login redirect. The launcher and
            # the Docker entrypoint both export these through render_environment,
            # so the two deployment paths stay in sync. EDUX_API_BASE_URL is
            ...
            "EDUX_AUTH_ENABLED": _bool_env(auth["enabled"]),
    ...
    def _apply_system_process_overrides(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        if value := self._process_env_value("EDUX_SANDBOX_ALLOW_SUBPROCESS"):
    ...
    def _apply_llamaindex_process_overrides(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        # (EDUX_RAG_RETRIEVAL_PROFILE / RAG_RETRIEVAL_PROFILE); preserve it.
        payload = dict(settings)
        if value := (
            self._process_env_value("EDUX_RAG_RETRIEVAL_PROFILE")
