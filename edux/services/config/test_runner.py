...
class ConfigTestRunner:
    ...
    async def _test_llm(self, run: TestRun, catalog: dict[str, Any]) -> None:
        from edux.services.llm import clear_llm_config_cache, get_token_limit_kwargs
        from edux.services.llm import complete as llm_complete
    ...
    async def _test_embedding(
        ...
        from edux.services.embedding.client import EmbeddingClient
        from edux.services.embedding.config import EmbeddingConfig
        ...
        probe_texts = [
            "EDUX embedding smoke test",
            "EDUX retrieval batch probe",
        ]
    ...
    def _test_search(self, run: TestRun, catalog: dict[str, Any]) -> None:
        from edux.services.search import web_search
        ...
        run.emit("info", "Running search query: EDUX configuration health check")
        result = web_search("EDUX configuration health check", provider=provider)
        run.emit(
            "response",
    ...
    async def _test_tts(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from edux.services.config.provider_runtime import resolve_tts_runtime_config
        from edux.services.voice import synthesize_speech
        ...
        sample = "EDUX voice check. 这是一段语音合成测试。"
    ...
    async def _test_stt(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from edux.services.config.provider_runtime import resolve_stt_runtime_config
        from edux.services.voice import transcribe_audio
    ...
    async def _test_imagegen(self, run: TestRun, catalog: dict[str, Any]) -> None:
        ...
        from edux.services.config.provider_runtime import resolve_imagegen_runtime_config
        from edux.services.imagegen import generate_image
    ...
    async def _test_videogen(self, run: TestRun, catalog: dict[str, Any]) -> None:
        from edux.services.config.provider_runtime import resolve_videogen_runtime_config
        from edux.services.videogen import probe_video
