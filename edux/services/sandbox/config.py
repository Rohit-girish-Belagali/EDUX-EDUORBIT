...
* ``EDUX_SANDBOX_RUNNER_URL`` set ⇒ runner sidecar (Docker deployment);
* else on Linux with a functional ``bwrap`` ⇒ bwrap (bare-metal);
* else, only when ``EDUX_SANDBOX_ALLOW_SUBPROCESS=1`` ⇒ restricted
...
:mod:`edux.tools.exec_tool`). Per-user quotas live in
:mod:`edux.services.sandbox.quota`.
"""
...
from edux.services.sandbox.backends import (
    BwrapBackend,
    RestrictedSubprocessBackend,
    RunnerSidecarBackend,
    SandboxBackend,
)
from edux.services.sandbox.spec import ResourceLimits

RUNNER_URL_ENV = "EDUX_SANDBOX_RUNNER_URL"
ALLOW_SUBPROCESS_ENV = "EDUX_SANDBOX_ALLOW_SUBPROCESS"

# Per-user execution quotas (see quota.py). Conservative defaults; override
# via the matching env vars.
MAX_CONCURRENT_ENV = "EDUX_SANDBOX_MAX_CONCURRENT"
MAX_PER_MINUTE_ENV = "EDUX_SANDBOX_MAX_PER_MINUTE"
...
def build_backend(settings: SandboxSettings) -> SandboxBackend | None:
    ...
    backend's ``health()`` — see :mod:`edux.services.sandbox.service`.
    """
    import sys
