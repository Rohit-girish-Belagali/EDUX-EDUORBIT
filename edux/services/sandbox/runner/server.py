...
(:class:`edux.services.sandbox.backends.RunnerSidecarBackend`).
...
Wire contract (must match ``RunnerSidecarBackend``):
  ...
  ``POST /exec``   -> request/response JSON described by the dataclasses in
                      :mod:`edux.services.sandbox.spec`. Request::
...
Mounts note:
  ...
  (``EDUX_RUNNER_ALLOWED_WORKDIRS``) as defence in depth.
"""
