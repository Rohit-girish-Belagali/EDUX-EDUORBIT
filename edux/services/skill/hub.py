...
A hub skill is the same artefact EDUX already speaks natively — a
...
# EDUX ships pointing at EduHub, its native open skill registry: a bare
# ``install <slug>`` with no ``<hub>:`` prefix resolves here.
DEFAULT_HUB = "eduhub"
_CLAWHUB_BASE_URL = "https://clawhub.ai/api/v1"
_EDUHUB_BASE_URL = "https://eduhub.edux.info/api/v1"
...
class ClawHubProvider:
    ...
    def fetch(self, slug: str, *, version: str | None = None) -> FetchedSkill:
        ...
        tmp = Path(tempfile.mkdtemp(prefix="edux-skill-"))
        try:
    ...
    def catalog(
        ...
        EDUX-native browser renders without a second round-trip.
        """
        if query.strip():
