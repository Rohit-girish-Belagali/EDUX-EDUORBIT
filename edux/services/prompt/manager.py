...
from edux.runtime.home import PACKAGE_ROOT
...
class PromptManager:
    ...
    # Modules that are not under edux/agents/ directory
    # Map module_name → on-disk path component under edux/
    ...
    def _candidate_prompt_dirs(self, module_name: str) -> list[Path]:
        ...
        if module_name in self.NON_AGENT_MODULES:
            ...
            current_dir = PACKAGE_ROOT / "edux" / module_name / "prompts"
            return [legacy_dir, current_dir]

        legacy_dir = PACKAGE_ROOT / "src" / "agents" / module_name / "prompts"
        current_dir = PACKAGE_ROOT / "edux" / "agents" / module_name / "prompts"
        return [legacy_dir, current_dir]
