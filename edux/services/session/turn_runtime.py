...
from edux.core.stream import StreamEvent, StreamEventType
from edux.services.llm.exceptions import (
    LLMAuthenticationError,
    LLMRateLimitError,
    ProviderQuotaExceededError,
)
from edux.services.llm.utils import clean_thinking_tags
from edux.services.path_service import get_path_service
from edux.services.session.protocol import SessionStoreProtocol
...
def _error_code_for_exception(exc: Exception) -> str | None:
    """Classify a turn-terminal failure so the frontend can distinguish a
    shared-API-key problem (auth/quota/rate-limit) from any other failure."""
    if isinstance(exc, LLMAuthenticationError):
        return "auth_error"
    if isinstance(exc, ProviderQuotaExceededError):
        # Checked before LLMRateLimitError — it's a subclass of it.
...
def _llm_selection_dict(value: Any) -> dict[str, str] | None:
    from edux.services.model_selection import LLMSelection
...
class TurnRuntimeManager:
    ...
    def __init__(self, store: SessionStoreProtocol | None = None) -> None:
        from edux.services.session import get_session_store
    ...
    async def start_turn(self, payload: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        try:
            from edux.runtime.request_contracts import validate_capability_config
        ...
        if llm_selection:
            try:
                from edux.multi_user.model_access import apply_allowed_llm_selection
            ...
            from edux.multi_user.model_access import (
        ...
        if llm_selection:
            ...
            from edux.services.model_selection import (
                LLMSelection,
        ...
        if payload.get("tools") is None:
            try:
                from edux.api.routers.settings import get_enabled_optional_tools
    ...
    async def _run_turn(self, execution: _TurnExecution) -> None:
        ...
        try:
            from edux.agents.notebook import NotebookAnalysisAgent
            from edux.book.context import build_book_context
            from edux.core.context import Attachment, UnifiedContext
            from edux.runtime.orchestrator import ChatOrchestrator
            from edux.services.memory import get_memory_store
            from edux.services.model_selection.runtime import (
                activate_llm_selection,
            )
            from edux.services.model_selection.runtime import (
                reset_llm_selection as reset_active_llm_selection,
            )
            from edux.services.notebook import get_notebook_manager
            from edux.services.session.context_builder import ContextBuilder
            from edux.services.skill import get_skill_service
            ...
            from edux.services.storage import get_attachment_store
            ...
            from edux.utils.document_extractor import extract_documents_from_records
            ...
            from edux.multi_user.context import get_current_user
            from edux.multi_user.paths import get_admin_path_service
            from edux.multi_user.skill_access import assigned_skill_ids
            from edux.services.persona import PersonaService, get_persona_service
            from edux.services.skill.service import SkillService, render_skills_manifest
            ...
            if is_chat_capability:
                from edux.services.session.source_inventory import (
                    build_inventory,
                    render_manifest,
                )
    ...
    async def _maybe_generate_session_title(
        ...
        *,
        ...
        title = ""
        try:
            from edux.services.llm import stream as llm_stream
