...
*partner session key*. The first consult of a EDUX chat session has none,
so we mint a fresh ``dt-…`` key and return it; the cross-turn registry
(:mod:`edux.services.subagent.sessions`) remembers it against
(chat session, connection), so every later consult in the same EDUX chat —
within one turn or across turns — resumes the SAME partner session. The partner
page then sees one complete history session per EDUX chat, titled from the
...
from edux.services.subagent.base import OnEvent, SubagentBackend
from edux.services.subagent.config import BackendConfig
from edux.services.subagent.types import (
    EVENT_ERROR,
    EVENT_REASONING,
    EVENT_TEXT,
    EVENT_TOOL,
...
if TYPE_CHECKING:  # avoid importing the core/partner packages at module load
    from edux.core.stream import StreamEvent
...
class PartnerBackend(SubagentBackend):
    ...
    async def consult(
        ...
        *,
        ...
        # a EDUX chat → mint a stable, colon-free key; the registry threads
