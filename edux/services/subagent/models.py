...
reasoning effort EDUX drives each backend with. Those lists change over
...
from edux.services.subagent.process import probe_version
from edux.services.subagent.registry import get_backend
...
async def _claude_options() -> BackendOptions:
    from edux.services.subagent.claude_models import load_cached_claude_models
...
async def sync_backend_options(kind: str) -> BackendOptions:
    ...
    """
    if kind == "claude_code":
        from edux.services.subagent.claude_models import sync_claude_models
