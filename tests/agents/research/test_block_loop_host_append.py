...
from edux.agents.research.data_structures import DynamicTopicQueue, ToolTrace
from edux.agents.research.pipeline import (
    LABEL_APPEND,
    LABEL_FINISH,
    LABEL_THINK,
    ResearchedBlock,
...
from edux.core.agentic.tool_dispatch import DispatchOutcome
...
def _make_pipeline(monkeypatch: pytest.MonkeyPatch) -> ResearchPipeline:
    ...
    monkeypatch.setattr("edux.agents.research.pipeline.get_llm_config", lambda: _FakeLLM())
    monkeypatch.setattr(
        "edux.agents.research.pipeline.get_tool_registry", lambda: _FakeRegistry()
    )
...
def _make_pipeline_with_registry(
    ...
    *,
    ...
    monkeypatch.setattr("edux.agents.research.pipeline.get_llm_config", lambda: _FakeLLM())
    monkeypatch.setattr("edux.agents.research.pipeline.get_tool_registry", lambda: registry)
    monkeypatch.setattr("edux.agents.research.pipeline.user_has_memory", lambda: False)
    monkeypatch.setattr("edux.agents.research.pipeline.user_has_notebooks", lambda: False)
    ...
    monkeypatch.setattr(
        "edux.agents.research.pipeline.exec_capability_available", lambda: True
    )
