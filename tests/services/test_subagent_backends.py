...
from edux.services.subagent.claude_code import ClaudeCodeBackend
from edux.services.subagent.codex import CodexBackend
from edux.services.subagent.config import (
    CONSULT_BUDGET_MAX,
...
from edux.services.subagent.process import stream_process_lines
from edux.services.subagent.types import ConsultResult
...
def test_claude_command_applies_model_effort_system_prompt() -> None:
    backend = ClaudeCodeBackend()
    cfg = BackendConfig(model="opus", effort="high", system_prompt="consulted by EDUX")
    ...
    assert "--append-system-prompt" in cmd and "consulted by EDUX" in cmd
...
def test_materialize_images_writes_only_resolvable_images(tmp_path) -> None:
    ...
    from edux.core.context import Attachment
    from edux.services.subagent.images import materialize_images
...
def test_parse_claude_model_screen() -> None:
    from edux.services.subagent.claude_models import _parse_model_screen
...
def test_claude_models_cache_roundtrip(monkeypatch, tmp_path) -> None:
    from edux.services.subagent import claude_models as cm
...
async def test_claude_options_prefers_synced_cache(monkeypatch) -> None:
    ...
    curated fallback."""
    from edux.services.subagent import claude_models as cm
    from edux.services.subagent import models as models_mod
...
def test_session_registry_roundtrip(monkeypatch, tmp_path) -> None:
    from edux.services.subagent import sessions as sess
...
def test_registry_partner_is_non_cli_backend() -> None:
    from edux.services.subagent import PARTNER_BACKEND_KIND, get_backend, list_backend_kinds
...
async def test_detect_all_excludes_partner_backend() -> None:
    from edux.services.subagent import detect_all
...
def _patch_manager(monkeypatch, manager) -> None:
    import edux.services.partners as partners_pkg
...
async def test_partner_consult_mints_session_key_and_returns_reply(monkeypatch) -> None:
    ...
    from edux.services.subagent.partner import PartnerBackend
...
def test_partner_event_mapping_covers_channels() -> None:
    from edux.core.stream import StreamEvent, StreamEventType
    from edux.services.subagent.partner import _to_subagent_events
