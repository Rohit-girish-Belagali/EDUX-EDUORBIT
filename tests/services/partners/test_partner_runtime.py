...
from edux.partners.bus.events import InboundMessage
from edux.partners.bus.queue import MessageBus
from edux.services.partners.manager import PartnerConfig
from edux.services.partners.runtime import PartnerRunner
from edux.services.partners.sessions import PartnerSessionStore
...
class _FakeOrchestrator:
    ...
    async def handle(self, context):
        from edux.services.memory.paths import memory_root
...
def fake_orchestrator(monkeypatch):
    import edux.runtime.orchestrator as orch_mod
    from edux.services.model_selection import runtime as selection_runtime
...
def _runner(partners_root, config: PartnerConfig | None = None) -> PartnerRunner:
    from edux.partners.config.paths import get_partner_sessions_dir
...
class TestTurnExecution:
    ...
    async def test_llm_config_error_folds_into_graceful_reply(
        ...
        from edux.services.llm.exceptions import LLMConfigError
        from edux.services.model_selection import runtime as selection_runtime
...
class TestContextAssembly:
    ...
    async def test_context_carries_soul_tools_and_metadata(self, partners_root, fake_orchestrator):
        from edux.services.partners.workspace import write_soul
    ...
    async def test_default_tools_resolve_to_full_toggleable_set(
        ...
        from edux.agents._shared.tool_composition import default_optional_tools
...
class TestBuiltinToolsAndMemory:
    ...
    async def test_turn_runs_against_partner_memory(self, partners_root, fake_orchestrator):
        ...
        from edux.partners.config.paths import get_partner_workspace
...
class TestLiveTurn:
    def test_buffer_replays_for_late_subscriber(self):
        from edux.services.partners.manager import LiveTurn
    ...
    async def test_web_turn_runs_on_instance_and_survives_resubscribe(
        ...
        from edux.services.partners.manager import PartnerManager
...
class TestPartnerCommands:
    ...
    async def test_sessions_resume_delete_commands(self, partners_root, fake_orchestrator):
        from edux.services.partners.commands import PartnerCommandHandler
    ...
    async def test_telegram_bot_command_suffix_is_supported(self, partners_root, fake_orchestrator):
        ...
        reply = await runner.process_message(_msg("/new@EDUXBot"))
