...
from edux.partners.channels.zulip import ZulipChannel, ZulipConfig
...
class TestIsMentioned:
    ...
    def test_content_fallback_when_flags_empty(self):
        ...
        ch = _make_channel()
        ch._bot_user_id = 100
        ch._bot_full_name = "EDUX Bot"
        assert ch._is_mentioned({"flags": [], "content": "hi @**EDUX Bot** help"}) is True

    def test_content_fallback_requires_full_name(self):
        ch = _make_channel()
        ch._bot_user_id = 100
        ch._bot_full_name = ""
        assert ch._is_mentioned({"flags": [], "content": "hi @**EDUX Bot**"}) is False
    ...
    def test_content_fallback_disambiguated_mention(self):
        ...
        ch._bot_user_id = 100
        ch._bot_full_name = "EDUX Bot"
        assert ch._is_mentioned({"flags": [], "content": "hi @**EDUX Bot|100** help"}) is True
...
class TestDownloadAttachments:
    def test_extracts_from_markdown_content(self, tmp_path: Path):
        ...
        with (
            ...
            patch(
                "edux.partners.channels.zulip.get_media_dir",
                return_value=tmp_path,
            ),
            patch("edux.partners.channels.zulip.requests.get") as mock_get,
        ):
            mock_resp = MagicMock()
...
class TestSend:
    ...
    async def test_send_text_uses_call_endpoint(self):
        ...
        from edux.partners.bus.events import OutboundMessage
...
class TestSubscribeStreams:
    ...
    def test_subscribe_partial_failure_logs_warning(self):
        ...
        with patch("edux.partners.channels.zulip.logger.warning") as mock_warning:
            ch._subscribe_to_streams()
    ...
    def test_subscribe_non_success_all_already_subscribed_is_debug_only(self):
        ...
        with (
            patch("edux.partners.channels.zulip.logger.warning") as mock_warning,
            patch("edux.partners.channels.zulip.logger.debug") as mock_debug,
        ):
            ch._subscribe_to_streams()
...
class TestStart:
    ...
    async def test_start_profile_failure(self, monkeypatch):
        ...
        with patch("edux.partners.channels.zulip.ZulipChannel._call_with_retry") as mock_retry:
