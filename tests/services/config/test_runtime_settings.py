...
from edux.services.config.runtime_settings import (
...
def test_render_environment_uses_json_backed_runtime_names(monkeypatch, tmp_path: Path) -> None:
    ...
    # middleware). EDUX_AUTH_ENABLED gates the login redirect;
    # EDUX_API_BASE_URL is where the frontend server reaches the backend
    # (falls back to localhost:<backend_port> when no in-network / external base
    # is configured).
    assert env["EDUX_AUTH_ENABLED"] == "true"
    assert env["EDUX_API_BASE_URL"] == "http://localhost:8010"
...
def test_startup_ensure_creates_missing_runtime_jsons_with_defaults(
    ...
    from edux.services.config import model_catalog as model_catalog_module
    from edux.services.config import runtime_settings as runtime_settings_module
...
def test_document_parsing_v1_to_v2_migration(tmp_path: Path) -> None:
    ...
    from edux.services.config.runtime_settings import _atomic_write_json
