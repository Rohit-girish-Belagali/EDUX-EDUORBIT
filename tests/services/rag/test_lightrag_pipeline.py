...
from edux.services.rag.index_versioning import resolve_storage_dir_for_read
from edux.services.rag.pipelines.lightrag import config as lr_config
from edux.services.rag.pipelines.lightrag import engine, storage
from edux.services.rag.pipelines.lightrag.pipeline import LightRagPipeline
...
def test_embedding_func_returns_numpy_array(monkeypatch) -> None:
    ...
    monkeypatch.setattr("edux.services.embedding.get_embedding_config", lambda: _Config())
    monkeypatch.setattr("edux.services.embedding.get_embedding_client", lambda: _Client())
...
def test_lightrag_llm_adapter_preserves_messages_and_drops_extra_kwargs(
    ...
    monkeypatch.setattr("edux.services.llm.get_llm_client", lambda: _Client())
...
def test_build_rag_skips_raganything_parser_install_check(monkeypatch) -> None:
    ...
    time, even though EDUX only ever inserts a pre-parsed ``content_list``
...
def _stub_parse(monkeypatch, *, blocks=None, markdown: str = "# md") -> None:
    from edux.services.parsing.types import ParsedDocument
    ...
    monkeypatch.setattr("edux.services.parsing.get_parse_service", lambda: _Service())
