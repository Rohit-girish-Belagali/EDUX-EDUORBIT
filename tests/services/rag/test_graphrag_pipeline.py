...
from edux.services.rag.pipelines.graphrag import config as gr_config
from edux.services.rag.pipelines.graphrag import engine, ingestion, storage
from edux.services.rag.pipelines.graphrag.pipeline import GraphRagPipeline, _context_to_sources
...
def test_ingestion_uses_active_parse_service_for_parser_files(tmp_path, monkeypatch) -> None:
    from edux.services import parsing
...
def _stub_build(monkeypatch) -> list[dict]:
    ...
    monkeypatch.setattr(
        "edux.services.config.resolve_llm_runtime_config",
        lambda: _Cfg("gpt-4o-mini", "https://llm.test/v1", "sk-llm"),
    )
    monkeypatch.setattr(
        "edux.services.embedding.get_embedding_config",
...
def test_router_blocks_graphrag_when_unavailable(monkeypatch) -> None:
    ...
    from edux.api.routers import knowledge
