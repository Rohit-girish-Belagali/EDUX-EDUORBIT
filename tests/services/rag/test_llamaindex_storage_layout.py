...
from edux.services.rag.index_versioning import EmbeddingSignature
...
async def test_incremental_add_migrates_matching_legacy_index_to_flat_version(
    ...
    from edux.services.rag.pipelines.llamaindex import storage as storage_module
    from edux.services.rag.pipelines.llamaindex.pipeline import LlamaIndexPipeline
...
def test_hybrid_retriever_uses_official_query_fusion_when_bm25_available(
    ...
    from edux.services.rag.pipelines.llamaindex import retrievers as retriever_module
    from edux.services.rag.pipelines.llamaindex.config import RetrievalConfig
...
def test_retrieval_config_reads_profile_from_env(monkeypatch: pytest.MonkeyPatch) -> None:
    from edux.services.rag.pipelines.llamaindex import config as config_module

    monkeypatch.setenv("EDUX_RAG_RETRIEVAL_PROFILE", " vector ")
