...
from edux.services.session.turn_runtime import (
    TurnRuntimeManager,
...
class TestRegenerateLastTurn:
    ...
    async def test_end_to_end_skips_memory_refresh_and_no_duplicate_user(
        ...
        monkeypatch.setattr(
            "edux.services.llm.config.get_llm_config", lambda: SimpleNamespace()
        )
        monkeypatch.setattr(
            "edux.services.session.context_builder.ContextBuilder",
            FakeContextBuilder,
        )
