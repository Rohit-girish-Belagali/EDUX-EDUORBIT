...
from edux.services.memory import paths as paths_mod
from edux.services.memory.consolidator.modes import audit as audit_mod
from edux.services.memory.consolidator.modes import dedup as dedup_mod
from edux.services.memory.consolidator.modes import update as update_mod
from edux.services.memory.document import Document, Entry, parse, serialize
from edux.services.memory.ids import new_entry_id
from edux.services.memory.snapshot.entity import Entity
...
async def test_update_l2_appends_facts_from_chunk(memory_dir, monkeypatch):
    ...
    monkeypatch.setattr(
        "edux.services.memory.consolidator.modes.update.snap.read_snapshot",
    ...
    with (
        patch("edux.services.memory.consolidator.modes.update.call_llm", side_effect=fake_llm),
        patch.object(update_mod, "load_memory_settings") as mock_settings,
    ):
        from edux.services.memory.settings import (
            ChunkingSettings,
            DedupSettings,
            MemorySettings,
        )
...
async def test_update_l2_idempotent_when_no_new_entities(memory_dir, monkeypatch):
    ...
    with (
        patch(
            "edux.services.memory.consolidator.modes.update.call_llm",
            side_effect=llm_returns_one,
        ),
        patch.object(update_mod, "load_memory_settings") as mock_settings,
    ):
        from edux.services.memory.settings import DedupSettings, MemorySettings
...
async def test_audit_l2_applies_replace_edit(memory_dir, monkeypatch):
    ...
    monkeypatch.setattr(
        "edux.services.memory.consolidator.modes.audit.snap.read_snapshot",
    ...
    with patch("edux.services.memory.consolidator.modes.audit.call_llm", side_effect=fake_llm):
...
async def test_dedup_early_stop_when_no_edits(memory_dir, monkeypatch):
    ...
    with (
        patch("edux.services.memory.consolidator.modes.dedup.call_llm", side_effect=fake_llm),
