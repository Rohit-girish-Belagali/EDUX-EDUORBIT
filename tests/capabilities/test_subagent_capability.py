...
from edux.agents._shared.tool_composition import ToolMountFlags, compose_enabled_tools
from edux.capabilities import any_exclusive_capability_active
from edux.capabilities.subagent import (
    SUBAGENT_TOOL_NAMES,
    ConsultSubagentTool,
    SubagentCapability,
    connection_for_turn,
)
from edux.capabilities.subagent import binding as subagent_binding
from edux.core.context import UnifiedContext
from edux.runtime.registry.tool_registry import get_tool_registry
from edux.services.subagent.config import BackendConfig
from edux.services.subagent.types import ConsultResult, SubagentEvent
...
def _bind(monkeypatch, *, kind: str = "claude_code", cwd: str = "", name: str = "myagent") -> None:
    ...
    monkeypatch.setattr(
        "edux.multi_user.knowledge_access.resolve_kb_metadata",
        lambda ref: (
...
def test_binding_cached(monkeypatch) -> None:
    ...
    monkeypatch.setattr("edux.multi_user.knowledge_access.resolve_kb_metadata", fake)
...
async def test_consult_streams_events_and_threads_session(monkeypatch) -> None:
    ...
    monkeypatch.setattr("edux.services.subagent.get_backend", lambda kind: backend)
...
async def test_session_id_persists_across_turns(monkeypatch, tmp_path) -> None:
    ...
    # the local agent keeps context across EDUX's separate messages.
