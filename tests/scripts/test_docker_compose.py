...
def test_render_docker_env_reads_json_only(tmp_path: Path) -> None:
    ...
    assert values == {
        "EDUX_DOCKER_BACKEND_PORT": "9001",
        "EDUX_DOCKER_FRONTEND_PORT": "4000",
        "EDUX_DOCKER_POCKETBASE_PORT": "19090",
    }
    saved = output_path.read_text(encoding="utf-8")
    assert "\nBACKEND_PORT=" not in saved
    assert "EDUX_DOCKER_BACKEND_PORT=9001" in saved
...
def test_render_docker_env_uses_defaults_for_missing_or_invalid_json(tmp_path: Path) -> None:
    ...
    assert values["EDUX_DOCKER_BACKEND_PORT"] == "8001"
    assert values["EDUX_DOCKER_FRONTEND_PORT"] == "3782"
    assert values["EDUX_DOCKER_POCKETBASE_PORT"] == "8090"
...
def test_compose_files_do_not_consume_legacy_env_names() -> None:
    ...
    for name in ("docker-compose.yml", "docker-compose.ghcr.yml"):
        ...
        assert "EDUX_DOCKER_BACKEND_PORT" in content
...
def test_dockerfile_is_json_driven_without_bundle_sed() -> None:
    ...
    (including EDUX_API_BASE_URL / EDUX_AUTH_ENABLED) and web/proxy.ts
    ...
    assert "EDUX_IGNORE_PROCESS_ENV_OVERRIDES=1" in content
...
def test_supervisord_runs_as_root_with_unprivileged_children() -> None:
    ...
    supervisord to the unprivileged ``edux`` user via ``gosu`` made child
    ...
    ``user=edux`` directive instead, which keeps them unprivileged in both
    ...
    assert "gosu edux /usr/bin/supervisord" not in content
    # Every supervisord program drops to the unprivileged edux user, so the
    ...
    for block in program_blocks:
        ...
        assert "user=edux" in section, (
            f"supervisord program '{name}' must run as edux (user=edux)"
        )
...
def test_frontend_api_is_url_agnostic_passthrough() -> None:
    ...
    assert "EDUX_API_BASE_URL" in proxy_ts
