...
def _load_question_extractor_module():
    ...
    stubbed_modules = {
        "edux.services.config": {"get_agent_params": lambda *_args, **_kwargs: {}},
        "edux.services.llm": {"complete": lambda *_args, **_kwargs: None},
        "edux.services.llm.capabilities": {
            "supports_response_format": lambda *_args, **_kwargs: False
        },
        "edux.services.llm.config": {"get_llm_config": lambda: None},
        "edux.utils.json_parser": {"parse_json_response": lambda *_args, **_kwargs: {}},
    }
