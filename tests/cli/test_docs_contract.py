...
def _edux_commands() -> list[str]:
    commands: list[str] = []
    pending = ""
    for path in _command_doc_paths():
        for line in path.read_text(encoding="utf-8").splitlines():
            stripped = line.strip()
            if not pending and not stripped.startswith("edux "):
                continue
...
def test_documented_edux_subcommands_exist() -> None:
    top_level = {
        "book",
        "chat",
        "config",
...
def test_deep_research_examples_include_required_config() -> None:
    examples = [
        command for command in _edux_commands() if "edux run deep_research" in command
    ]
...
def test_docs_do_not_advertise_removed_cli_forms() -> None:
    ...
    assert "edux provider logout" not in text
    assert "edux memory show summary" not in text
