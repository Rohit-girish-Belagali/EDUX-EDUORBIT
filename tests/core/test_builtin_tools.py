...
from edux.runtime.registry.tool_registry import ToolRegistry
from edux.services.path_service import PathService
from edux.services.sandbox.spec import ExecResult
from edux.tools.builtin import (
    BrainstormTool,
    CodeExecutionTool,
    ExecTool,
...
async def test_exec_tool_reports_generated_public_artifacts(
    ...
    import edux.services.sandbox as sandbox_pkg
    import edux.services.sandbox.artifacts as sandbox_artifacts
...
async def test_brainstorm_tool_passes_llm_arguments(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_module(monkeypatch, "edux.tools.brainstorm", brainstorm=fake_brainstorm)
...
async def test_rag_tool_forwards_query_and_extra_kwargs(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_module(monkeypatch, "edux.tools.rag_tool", rag_search=fake_rag_search)
...
async def test_web_search_tool_wraps_sync_function(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_module(monkeypatch, "edux.tools.web_search", web_search=fake_web_search)
...
async def test_reason_tool_passes_llm_arguments(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_module(monkeypatch, "edux.tools.reason", reason=fake_reason)
...
async def test_geogebra_analysis_tool_handles_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_module(
        monkeypatch,
        "edux.agents.vision_solver.vision_solver_agent",
