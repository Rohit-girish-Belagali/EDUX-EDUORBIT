...
from edux.agents.chat.capability import ChatCapability
from edux.agents.question.capability import DeepQuestionCapability
from edux.agents.research.capability import DeepResearchCapability
from edux.agents.visualize.capability import VisualizeCapability
import edux.agents.visualize.pipeline as visualize_pipeline
from edux.capabilities.solve.capability import DeepSolveCapability
...
from edux.runtime.bootstrap.builtin_capabilities import BUILTIN_CAPABILITY_CLASSES
...
async def test_chat_capability_streams_content_and_geogebra_context(
    ...
    monkeypatch.setattr("edux.agents.chat.capability.AgenticChatPipeline", FakePipeline)
...
async def test_deep_solve_capability_runs_chat_loop_in_solve_mode(
    ...
    monkeypatch.setattr("edux.capabilities.solve.capability.AgenticChatPipeline", FakePipeline)

    context = UnifiedContext(
        user_message="solve x^2=4",
        language="en",
        metadata={"turn_id": "turn-edux"},
    ...
    assert captured["solve_session_id"] == "turn-edux"
...
async def test_deep_question_capability_uses_single_call_followup_agent(
    ...
    _install_module(
        monkeypatch,
        "edux.agents.question.coordinator",
        AgentCoordinator=FakeCoordinator,
    )
    _install_module(
        monkeypatch,
        "edux.agents.question.agents.followup_agent",
        FollowupAgent=FakeFollowupAgent,
    )
    _install_module(
        monkeypatch,
        "edux.services.llm.config",
...
async def test_deep_research_capability_delegates_to_pipeline(
    ...
    """
    import edux.agents.research.capability as deep_research_mod
    import edux.agents.research.request_config  # noqa: F401
