...
class TestErrors:
    ...
    def test_base_error_str_with_details(self) -> None:
        err = EDUXError("boom", details={"key": "val"})
        assert "key" in str(err)

    def test_hierarchy(self) -> None:
        assert issubclass(ConfigurationError, EDUXError)
        assert issubclass(ValidationError, EDUXError)
        assert issubclass(ServiceError, EDUXError)
