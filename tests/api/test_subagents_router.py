...
if FastAPI is not None and TestClient is not None:
    subagents_module = importlib.import_module("edux.api.routers.subagents")
...
def client(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "edux.services.subagent.config._settings_path",
        lambda: tmp_path / "subagent.json",
    )

    async def fake_detect():
        from edux.services.subagent.types import DetectResult
...
def test_backend_options_endpoint_shape(client, monkeypatch):
    ...
    from edux.services.subagent.models import BackendOptions, ModelOption
...
def test_message_connection_streams_and_persists(client, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("edux.services.subagent.get_backend", lambda kind: _FakeBackend())
