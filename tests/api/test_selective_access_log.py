...
The real middleware logs through the ``edux.access`` logger, which carries
...
class TestSelectiveAccessLog:
    ...
    def test_200_not_logged(self):
        """200 responses should not produce edux.access log records."""
        app = _build_app_with_middleware()
        with _CaptureAccess() as cap:
            with TestClient(app) as client:
                client.get("/ok")
