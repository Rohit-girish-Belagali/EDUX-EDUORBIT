...
catalog so the web panel can render it in EDUX's own UI (no iframe, no
...
from edux.services.skill import hub as hub_module
from edux.services.skill.hub import ClawHubProvider
...
def _mock_provider() -> ClawHubProvider:
        ...
        if path == "/api/v1/skills/socratic-tutor":
            return httpx.Response(
                ...
                json={
                    ...
                    "owner": {"displayName": "EDUX", "htmlUrl": "https://edux.info"},
    ...
    return ClawHubProvider(
        "eduhub",
        base_url="https://eduhub.edux.info/api/v1",
        client=httpx.Client(transport=httpx.MockTransport(handler)),
    )


def _build_app() -> FastAPI:
    router = importlib.import_module("edux.api.routers.skills").router
    app = FastAPI()
...
def test_hub_detail_endpoint(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert data["web_url"] == "https://eduhub.edux.info/skills/socratic-tutor"
