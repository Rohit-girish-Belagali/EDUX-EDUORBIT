...
from edux.services.session.sqlite_store import SQLiteSessionStore
from edux.services.session.turn_runtime import TurnRuntimeManager
...
async def test_turn_runtime_replays_events_and_materializes_messages(
    ...
    monkeypatch.setattr("edux.services.llm.config.get_llm_config", lambda: SimpleNamespace())
    monkeypatch.setattr(
        "edux.services.session.context_builder.ContextBuilder", FakeContextBuilder
    )
    monkeypatch.setattr("edux.runtime.orchestrator.ChatOrchestrator", FakeOrchestrator)
    monkeypatch.setattr(
        "edux.book.context.build_book_context",
    ...
    monkeypatch.setattr(
        "edux.services.memory.get_memory_store",
    ...
    monkeypatch.setattr(
        "edux.services.skill.get_skill_service",
        _fake_skill_service,
    )
    monkeypatch.setattr(
        "edux.services.persona.get_persona_service",
...
async def test_turn_runtime_persists_llm_selection_in_turn_snapshot(
    ...
    monkeypatch.setattr(
        "edux.services.config.get_model_catalog_service",
        lambda: SimpleNamespace(load=_model_catalog),
    )
    monkeypatch.setattr(
        "edux.services.model_selection.runtime.activate_llm_selection",
        fake_activate,
    )
    monkeypatch.setattr(
        "edux.services.model_selection.runtime.reset_llm_selection",
    ...
    monkeypatch.setattr("edux.services.skill.get_skill_service", _fake_skill_service)
    monkeypatch.setattr("edux.services.persona.get_persona_service", _fake_persona_service)
