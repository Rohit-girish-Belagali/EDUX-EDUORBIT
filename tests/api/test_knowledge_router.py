...
if FastAPI is not None and TestClient is not None:
    knowledge_router_module = importlib.import_module("edux.api.routers.knowledge")
...
def test_create_preserves_known_nondefault_provider(monkeypatch, tmp_path: Path) -> None:
    ...
    pageindex_config = importlib.import_module("edux.services.rag.pipelines.pageindex.config")
...
def test_upload_task_marks_provider_failures_as_error(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "edux.knowledge.add_documents.RAGService",
        _FailingRagService,
    )
...
def test_reindex_accepts_default_alias(monkeypatch, tmp_path: Path) -> None:
    ...
    embedding_signature = importlib.import_module("edux.services.rag.embedding_signature")
    index_versioning = importlib.import_module("edux.services.rag.index_versioning")
    monkeypatch.setattr(
...
def test_update_config_coerces_legacy_provider_to_llamaindex() -> None:
    ...
    config_module = importlib.import_module("edux.services.config")
    app = _build_app()
...
def _patch_server_probe(monkeypatch, *, ok: bool, error: str | None = None) -> None:
    ...
    from edux.services.rag.pipelines.lightrag_server import probe as probe_module
