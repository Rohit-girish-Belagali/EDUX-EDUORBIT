...
from edux.api.routers import settings as settings_router
...
from edux.services.config.runtime_settings import RuntimeSettingsService
from edux.services.embedding import client as embedding_client_module
from edux.services.embedding import config as embedding_config_module
from edux.services.llm import client as llm_client_module
from edux.services.llm import config as llm_config_module
...
async def test_network_settings_roundtrip_normalizes_cors_origins(
    ...
    payload = settings_router.NetworkSettingsUpdate(
        backend_port=8101,
        frontend_port=3882,
        public_api_base="https://api.example.com/edux",
    ...
    assert response["settings"]["public_api_base"] == "https://api.example.com/edux"
...
async def test_mineru_payload_includes_local_cli_probe(
    ...
    from edux.services.parsing.engines.mineru import backend as mineru_backend
...
async def test_mineru_models_download_start_requires_downloader(
    ...
    from edux.services.parsing.engines.mineru import models as mineru_models
...
@pytest.mark.asyncio
async def test_llm_key_override_roundtrip_never_leaks_raw_key(
    monkeypatch: pytest.MonkeyPatch, tmp_path
) -> None:
    ...
    monkeypatch.setattr(
        settings_router,
        "resolve_llm_runtime_config",
        lambda: type("_Resolved", (), {"provider_name": "openai"})(),
    )

    # Nothing saved yet.
    initial = await settings_router.get_llm_key_override()
    assert initial == {
        "has_key": False,
        "provider_name": None,
        "masked_hint": None,
    ...
    saved = await settings_router.update_llm_key_override(
        settings_router.LLMKeyOverrideUpdate(api_key="sk-testsecret1234")
    )
    assert saved["has_key"] is True
    assert saved["provider_name"] == "openai"
    assert saved["masked_hint"] == "sk-...1234"
    assert "sk-testsecret1234" not in json.dumps(saved)

    fetched = await settings_router.get_llm_key_override()
    assert fetched == saved

    # The raw key is persisted server-side (never re-served in plaintext).
    on_disk = json.loads(override_file.read_text())
    assert on_disk["api_key"] == "sk-testsecret1234"

    cleared = await settings_router.delete_llm_key_override()
    assert cleared == {
        "has_key": False,
        "provider_name": None,
        "masked_hint": None,
...
async def test_fetch_models_returns_picker_options(monkeypatch: pytest.MonkeyPatch) -> None:
    import edux.services.llm.factory as factory_module
