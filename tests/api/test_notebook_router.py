...
notebook_router = importlib.import_module("edux.api.routers.question_notebook").router
sessions_router = importlib.import_module("edux.api.routers.sessions").router
...
def store(tmp_path: Path, monkeypatch) -> SQLiteSessionStore:
    ...
    monkeypatch.setattr(
        "edux.api.routers.question_notebook.get_sqlite_session_store",
        lambda: instance,
    )
    monkeypatch.setattr(
        "edux.api.routers.sessions.get_sqlite_session_store",
        lambda: instance,
    )
    return instance
