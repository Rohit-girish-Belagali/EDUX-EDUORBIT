...
<p align="center"><img src="../../assets/figs/logo/logo.png" alt="EDUX logo" height="56" style="vertical-align: middle;">&nbsp;<img src="../../assets/figs/logo/banner.png" alt="EDUX" height="48" style="vertical-align: middle;"></p>

# EDUX: Dożywotnie Spersonalizowane Korepetycje

<p align="center">
  <a href="https://eduorbit.com" target="_blank"><img alt="Docs — eduorbit.com" src="https://img.shields.io/badge/Docs-eduorbit.com%20%E2%86%97-0A0A0A?style=for-the-badge&labelColor=F5F5F4" height="36"></a>
</p>

<p align="center">
  <a href="https://trendshift.io/repositories/17099?utm_source=repository-badge&amp;utm_medium=badge&amp;utm_campaign=badge-repository-17099" target="_blank" rel="noopener noreferrer"><img src="https://trendshift.io/api/badge/repositories/17099" alt="EduOrbit%2FEDUX | Trendshift" width="250" height="55"/></a>&nbsp;
  <a href="https://trendshift.io/repositories/17099?utm_source=trendshift-badge&amp;utm_medium=badge&amp;utm_campaign=badge-trendshift-17099" target="_blank" rel="noopener noreferrer"><img src="https://trendshift.io/api/badge/trendshift/repositories/17099/daily" alt="EduOrbit%2FEDUX | Trendshift" width="250" height="55"/></a>&nbsp;
  <a href="https://trendshift.io/repositories/17099?utm_source=trendshift-badge&amp;utm_medium=badge&amp;utm_campaign=badge-trendshift-17099" target="_blank" rel="noopener noreferrer"><img src="https://trendshift.io/api/badge/trendshift/repositories/17099/weekly?language=Python" alt="EduOrbit%2FEDUX | Trendshift" width="250" height="55"/></a>
</p>
...
[Funkcje](#-główne-funkcje) · [Zacznij](#-pierwsze-kroki) · [Eksploruj](#-eksploracja-edux) · [CLI](#️-edux-cli--interfejs-natywny-dla-agentów) · [Ekosystem](#-ekosystem--eduhub-i-społeczność-umiejętności) · [Społeczność](#-społeczność)

</div>

---

> 🤝 **Zapraszamy do wszelkich form współpracy!** Głosuj na elementy planu działania lub proponuj nowe w [`Roadmap`](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/issues/498), a szczegóły dotyczące strategii gałęzi, standardów kodowania i sposobu rozpoczęcia pracy znajdziesz w naszym [Przewodniku dla współtwórców](../../CONTRIBUTING.md).

### 📰 Aktualności

- **2026-05-22** 🌐 Oficjalna strona dokumentacji dostępna na [**eduorbit.com**](https://eduorbit.com/) — przewodniki, odniesienia i wycieczki po możliwościach w jednym miejscu.
...
- **2026-04-10** 📄 Nasz artykuł jest już dostępny na arXiv — przeczytaj [preprint](https://arxiv.org/abs/2604.26962), aby dowiedzieć się więcej o projekcie i pomysłach stojących za EDUX.
- **2026-02-06** 🚀 10 tys. gwiazdek w zaledwie 39 dni! Ogromne podziękowania dla naszej niesamowitej społeczności.
- **2026-01-01** 🎊 Szczęśliwego Nowego Roku! Dołącz do naszego [Discorda](https://discord.gg/eRsjPgMU4t), [WeChat](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/issues/78) lub [Dyskusji](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/discussions) — razem kształtujmy przyszłość EDUX.
- **2025-12-29** 🎓 EDUX jest oficjalnie wydany!

## ✨ Główne funkcje

EDUX to natywne dla agentów środowisko nauki, które łączy korepetycje, rozwiązywanie problemów, generowanie quizów, badania, wizualizacje i ćwiczenia opanowania wiedzy w jednym rozszerzalnym systemie.
...
EDUX oferuje cztery ścieżki instalacji. Wszystkie współdzielą jeden układ obszaru roboczego: ustawienia żyją w `data/user/settings/` pod katalogiem, z którego uruchamiasz (lub pod `EDUX_HOME` / `edux start --home` jeśli ustawisz je jawnie). Dla pełnej aplikacji zalecany przepływ to **wybierz katalog obszaru roboczego → zainstaluj → `edux init` → `edux start`**.

<details>
<summary><b>Opcja 1 — Instalacja z PyPI</b> · pełna lokalna aplikacja Web + CLI, bez potrzeby klonowania</summary>

Pełna lokalna aplikacja Web + CLI, bez potrzeby klonowania. Wymaga **Python 3.11+** i środowiska uruchomieniowego **Node.js 20+** na PATH (spakowany serwer standalone Next.js jest uruchamiany przez `edux start`).

```bash
mkdir -p my-edux && cd my-edux
pip install -U edux
edux init     # pyta o porty + dostawcę LLM + opcjonalne osadzanie
edux start    # uruchamia backend + frontend; utrzymuj terminal otwarty
```

`edux init` pyta o port backendu (domyślnie `8001`), port frontendu (domyślnie `3782`), dostawcę LLM / bazowy URL / klucz API / model i opcjonalnego dostawcę osadzania dla Bazy wiedzy / RAG.

Po `edux start` otwórz adres URL frontendu wydrukowany w terminalu — domyślnie [http://127.0.0.1:3782](http://127.0.0.1:3782). Naciśnij `Ctrl+C` w tym terminalu, aby zatrzymać backend i frontend. Pominięcie `edux init` jest dobre dla szybkiego testu; aplikacja uruchamia się z domyślnymi portami i pustymi ustawieniami modelu, skonfiguruj je później w **Settings → Models**.
...
```bash
conda create -n edux python=3.11
conda activate edux
python -m pip install --upgrade pip
```
...
**Zablokowany serwer deweloperski:** jeśli `edux start` zgłasza istniejący frontend, który nie odpowiada, zatrzymaj PID, który wydrukuje. Jeśli żaden proces Next.js nie jest uruchomiony, pliki blokady są przestarzałe — usuń je i spróbuj ponownie:
...
- `ghcr.io/eduorbit/edux:latest` — stabilne wydanie
- `ghcr.io/eduorbit/edux:pre` — wersja wstępna, gdy dostępna
...
```bash
docker run --rm --name edux \
  -p 127.0.0.1:3782:3782 \
  -v edux-data:/app/data \
  ghcr.io/eduorbit/edux:latest
```
...
Otwórz [http://127.0.0.1:3782](http://127.0.0.1:3782). Kontener tworzy `/app/data/user/settings/*.json` przy pierwszym uruchomieniu; skonfiguruj dostawców modeli ze strony Web Settings. Konfiguracja, klucze API, dzienniki, pliki obszaru roboczego, pamięć i bazy wiedzy są przechowywane w woluminie `edux-data`.
...
- **Odłączony:** dodaj `-d`, następnie `docker logs -f edux`, aby śledzić, `docker stop edux`, aby zatrzymać, `docker rm edux` przed ponownym użyciem nazwy. Wolumin `edux-data` przechowuje ustawienia i obszar roboczy między restartami.
...
`edux init --cli` współdzieli ten sam układ `data/user/settings/` co pełna aplikacja, ale pomija monity o porty backendu/frontendu i domyślnie wyłącza osadzanie (wybierz `Yes` jeśli planujesz używać `edux kb …` lub narzędzi RAG). Nadal zapisuje kompletny układ środowiska uruchomieniowego (`system.json`, `auth.json`, `integrations.json`, `model_catalog.json`, `main.yaml`, `agents.yaml`) i nadal pyta o aktywnego dostawcę LLM i model.

<details>
<summary><b>Typowe polecenia</b></summary>

```bash
edux chat                                          # interaktywny REPL
edux chat --capability deep_solve --tool rag --kb my-kb
edux run chat "Explain Fourier transform"
edux run deep_solve "Solve x^2 = 4" --tool rag --kb my-kb
edux kb create my-kb --doc textbook.pdf
edux memory show
edux config show
```
...
Lokalna instalacja `edux-cli` nie zawiera zasobów webowych ani zależności serwera. Zachowaj kod źródłowy — instalacja edytowalna wskazuje na niego. Aby dodać aplikację Web później, zainstaluj pakiet PyPI (Opcja 1) i uruchom `edux init` + `edux start` z tego samego obszaru roboczego.
...
- **docker-compose:** zamiast tego kierowany do utwardzonego, o najmniejszych uprawnieniach **sidecar runner** (`Dockerfile.runner`) przez `EDUX_SANDBOX_RUNNER_URL` — najsilniejsza postawa, automatycznie preferowana gdy jest obecna.

Piaskownica podprocesu jest kontrolowana przez ustawienie `sandbox_allow_subprocess` w `data/user/settings/system.json` (domyślnie `true`). Uruchamianie kodu generowanego przez model na twoim hoście to prawdziwa decyzja dotycząca zaufania — ustaw na `false` (lub eksportuj `EDUX_SANDBOX_ALLOW_SUBPROCESS=0`), aby wyłączyć wykonanie po stronie hosta kosztem tego, że umiejętności biurowe nie będą mogły produkować plików.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/OVERVIEW.png" alt="Strona główna EDUX — obszar roboczy Chat z każdą powierzchnią w pasku bocznym" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/system%20architecture.png" alt="Architektura systemu EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/home/00-overview.png" alt="Obszar roboczy czatu EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/chat-agent-loop.png" alt="Pętla agenta czatu EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/partners/00-partners%20overview.png" alt="Obszar roboczy Partners EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/partners-architecture.png" alt="Architektura Partners EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/myagents/00-overview.png" alt="Obszar roboczy My Agents EDUX" width="900">
</div>

My Agents zamienia innych agentów w kontekst dla EDUX i wykonuje dwie odrębne rzeczy. **Połącz żywego agenta** — Claude Code lub Codex CLI na twoim komputerze, lub jednego z twoich Partners — i konsultuj go z wnętrza tury czatu: EDUX faktycznie *uruchamia* drugiego agenta i strumieniuje jego pracę do panelu Activity przez narzędzie `consult_subagent`. Wybierz go chipem Agent (lub wpisz `@`) i ustaw ile rund może trwać konsultacja.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/co-writer/00-overview.png" alt="Obszar roboczy Co-Writer EDUX" width="900">
</div>
...
Jego wyróżniającym pomysłem jest **precyzyjna edycja**: zaznacz fragment i poproś EDUX o przepisanie, rozszerzenie lub skrócenie. Agent edycji może ugruntować zmianę w bazie wiedzy lub dowodach webowych, zachowuje ślad swoich wywołań narzędzi i pokazuje każdą zmianę jako diff akceptuj/odrzuć — więc nic nie ląduje dopóki tego nie zatwierdzisz.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/book/00-book_overview.png" alt="Biblioteka książek EDUX" width="900">
</div>
...
Każdy rozdział kompiluje się do typowanych bloków — tekst, callouts, quizy, fiszki, osie czasu, kod, figury, interaktywny HTML, animacje, grafy konceptów, dogłębne analizy i notatki użytkownika — a każda strona ma swój własny Page Chat. Bloki są edytowalne: wstawiaj, przenoś, regeneruj lub zmieniaj typ bloku bez przepisywania rozdziału. Polecenia konserwacyjne takie jak `edux book health` i `edux book refresh-fingerprints` pomagają wykryć kiedy wiedza źródłowa odbiega od skompilowanych stron.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/knowledge/00-overview.png" alt="Knowledge Center EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/learning-space/00-overview.png" alt="Centrum Learning Space EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/memory/00-overview.png" alt="Przegląd Memory EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/memory/01-3%20layer%20memory%20graph.png" alt="Graf pamięci EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/settings/00-setting%20overview.png" alt="Centrum ustawień EDUX" width="900">
</div>
...
Uwierzytelnianie jest **domyślnie wyłączone** — EDUX działa jednoosobowo. Włącz je a jedno drzewo `data/` obsługuje obszar roboczy administratora, izolowane obszary robocze per-użytkownik i obszary robocze partnerów obok siebie:
...
**Włącz:** włącz uwierzytelnianie w `data/user/settings/auth.json`, uruchom ponownie `edux start`, zarejestruj pierwszego administratora pod `/register`, następnie dodaj użytkowników z `/admin/users` i przypisz modele, KB, umiejętności, Partners, politykę narzędzi/MCP i dostęp do wykonania kodu przez uprawnienia.
...
## ⌨️ EDUX CLI — Interfejs natywny dla agentów

Jeden plik binarny `edux`, dwa sposoby wejścia: interaktywny **REPL** dla osób żyjących w terminalu i strukturalny **JSON** dla innych agentów które prowadzą EDUX jako narzędzie. Te same możliwości, narzędzia i bazy wiedzy w obu przypadkach.

<details>
<summary><b>Prowadź sam</b></summary>

`edux chat` otwiera interaktywny REPL; `edux run <capability> "<message>"` uruchamia jedną turę i wychodzi. Oba używają tych samych flag `--capability`, `--tool`, `--kb` i `--config`.

```bash
edux chat                                              # interaktywny REPL
edux chat --capability deep_solve --kb my-kb --tool rag
edux run chat "Explain the Fourier transform" --tool rag --kb textbook
edux run deep_research "Survey 2026 papers on RAG" \
...
EDUX jest zbudowany aby być *obsługiwany przez innego agenta*. Dodaj `--format json` do dowolnego `run` a każda tura strumieniuje **NDJSON — jedno zdarzenie na linię** (`content`, `tool_call`, `tool_result`, `done`, …), każda linia oznaczona swoim `session_id`. Uruchomienia są bezpieczne bez TTY: pauza `ask_user` bez TTY automatycznie rozwiązuje się pustą odpowiedzią zamiast zawieszać.

```bash
# Jednorazowo, czytelne maszynowo
edux run deep_solve "Find d/dx[sin(x^2)]" --tool reason --format json

# Łącz tury w jednej sesji stanowej — przechwyć id, ponownie użyj
SID=$(edux run deep_research "Survey 2026 papers on RAG" \
  --config mode=report --config depth=standard --format json \
  | jq -r 'select(.type=="done").session_id')
edux run deep_question "Quiz me on that survey" --session "$SID" --format json
```

Repozytorium zawiera główny [`SKILL.md`](../../SKILL.md) — około 150-liniowy dokument przekazania który uczy każdy LLM używający narzędzi całej powierzchni w jednym czytaniu. Przekaż go Claude Code, Codex lub OpenCode (automatycznie pobierają `SKILL.md`) lub opakuj `edux run` jako narzędzie w pętli LangChain / AutoGen. Pełne przepisy: [Agent Handoff](https://eduorbit.com/docs/cli/agent-handoff/).
...
| Polecenie | Opis |
|:---|:---|
| `edux init` | Utwórz lub zaktualizuj `data/user/settings` dla bieżącego obszaru roboczego |
| `edux start [--home PATH]` | Uruchom backend + frontend razem |
| `edux serve [--port PORT]` | Uruchom tylko backend FastAPI |
| `edux run <capability> <message>` | Uruchom jedną turę możliwości (`chat`, `deep_solve`, `deep_question`, `deep_research`, `visualize`, `math_animator`, `mastery_path`); dodaj `--format json` dla wyjścia NDJSON |
| `edux chat` | Interaktywny REPL z kontrolkami możliwości, narzędzia, KB, notatnika i historii |
| `edux partner list/create/start/stop` | Zarządzaj partnerami połączonymi przez IM |
| `edux kb list/info/create/add/search/set-default/delete` | Zarządzaj bazami wiedzy LlamaIndex |
| `edux skill search/install/list/remove/login/logout/publish/update` | Zarządzaj umiejętnościami, instaluj z hubów i publikuj własne (`eduhub:<slug>` domyślnie, patrz Ekosystem) |
| `edux memory show/clear` | Inspekcjonuj dokumenty pamięci L2/L3 lub wyczyść pamięć L1/wszystko |
| `edux session list/show/open/rename/delete` | Zarządzaj współdzielonymi sesjami |
| `edux notebook list/create/show/add-md/replace-md/remove-record` | Zarządzaj notatnikami z plików Markdown |
| `edux book list/health/refresh-fingerprints` | Inspekcjonuj książki i odświeżaj odciski źródeł |
| `edux plugin list/info` | Inspekcjonuj zarejestrowane narzędzia i możliwości |
| `edux config show` | Wydrukuj podsumowanie konfiguracji |
| `edux provider login <provider>` | Uwierzytelnianie dostawcy (`openai-codex` logowanie OAuth; `github-copilot` weryfikuje istniejącą sesję auth Copilot) |
...
Pakiet tylko CLI żyje w `packaging/edux-cli`. W tym kodzie źródłowym zainstaluj go ze źródła:
...
Umiejętności EDUX używają otwartego formatu **Agent-Skills** — folder z podręcznikiem `SKILL.md` (frontmatter YAML + Markdown) i opcjonalnymi plikami referencyjnymi. Nic w tym nie jest specyficzne dla EDUX, więc każdy rejestr który mówi w tym formacie staje się źródłem dla twojej biblioteki. EDUX jest dostarczany z **[EduHub](https://eduhub.eduorbit.com/)** — naszym własnym rejestrem umiejętności skupionym na edukacji — podłączonym jako domyślny hub.

<details>
<summary><b>EduHub — Ekosystem umiejętności EDUX</b></summary>

[**EduHub**](https://eduhub.eduorbit.com/) to hub społecznościowy który EDUX uruchomił do dzielenia się umiejętnościami agentów zorientowanymi na nauczanie — tutorzy sokratejscy, kreatory fiszek, informacje zwrotne na eseje, plany egzaminów, objaśniacze konceptów i więcej. Jest wbudowany w EDUX, więc nie ma nic do konfigurowania: gołe slug lub prefiks `eduhub:` rozwiązuje do niego.
...
```bash
edux skill search "socratic tutor"               # wyszukaj EduHub (domyślny hub)
edux skill install socratic-tutor                # pobierz → weryfikuj → zarejestruj
edux skill install eduhub:socratic-tutor@1.2.0   # przypnij hub i wersję
edux skill list                                  # lokalne umiejętności z proweniencją hubu
```

**Opublikuj własną** — spakuj `SKILL.md` i podziel się ze społecznością:

```bash
edux skill login                                 # logowanie przeglądarki do EduHub
edux skill publish ./my-skill                    # interaktywne: wybierz ścieżkę + tagi, następnie prześlij
edux skill update                                # wycofaj lub wydaj nową wersję
```

EduHub jest również samodzielnym, kompatybilnym z ClawHub rejestrem, więc agenty które nie są EDUXem (Claude Code, Codex, …) mogą używać go bezpośrednio przez CLI `eduhub` — `npx eduhub install socratic-tutor`.
...
- frontmatter jest normalizowany do schematu EDUX a `always:` jest **usuwane**, więc pobrana umiejętność nigdy nie może wymusić się do każdego systemowego promptu;
...
Ponieważ EDUX mówi otwartym formatem Agent-Skills, **[ClawHub](https://clawhub.ai/)** działa również jako pierwszorzędne źródło — jest wbudowany obok EduHub. Wybierz go z prefiksem hubu:

```bash
edux skill search "git release notes" --hub clawhub
edux skill install clawhub:git-release-notes@1.0.1
```
...
EDUX to projekt open-source prowadzony przez [Bingxi Zhao](https://github.com/pancacake) w ramach Grupy [EduOrbit](https://github.com/EduOrbit), i iteruje w **w pełni open-source formie**, budowany razem ze społecznością. Do tej pory **NIE** mamy żadnych płatnych produktów online jakiegokolwiek rodzaju. Zapraszamy do kontaktu pod adresem **bingxizhao39@gmail.com** w sprawie dyskusji, pomysłów lub współpracy.

### 🙏 Podziękowania

Serdeczne podziękowania dla [**Chao Huang**](https://sites.google.com/view/chaoh), dyrektora Data Intelligence Lab @ HKU, i naszych współpracowników z EduOrbit za ciepłe wsparcie — szczególnie [**Jiahao Zhang**](https://github.com/zzhtx258), [**Zirui Guo**](https://github.com/LarFii) i [**Xubin Ren**](https://github.com/Re-bin). Jesteśmy również głęboko wdzięczni **społeczności open-source**: wasze gwiazdki, zgłoszenia, pull requesty i dyskusje kształtują EDUX każdego dnia.

EDUX stoi również na ramionach wybitnych projektów open-source które dostarczyły nam zarówno narzędzia jak i inspirację:

| Projekt | Rola / Inspiracja |
|:---|:---|
| [**LlamaIndex**](https://github.com/run-llama/llama_index) | Kręgosłup potoku RAG i indeksowania dokumentów |
| [**nanobot**](https://github.com/EduOrbit/nanobot) | Ultralekki silnik agenta który zasilał oryginalny TutorBot *(EduOrbit)* |
| [**LightRAG**](https://github.com/EduOrbit/LightRAG) | Prosty i szybki RAG *(EduOrbit)* |
| [**AutoAgent**](https://github.com/EduOrbit/AutoAgent) | Framework agentów bez kodu *(EduOrbit)* |
| [**AI-Researcher**](https://github.com/EduOrbit/AI-Researcher) | Zautomatyzowany potok badań naukowych *(EduOrbit)* |
...
| [**Claude Code**](https://github.com/anthropics/claude-code) | Agentowy CLI kodowania który zainspirował pętlę agenta EDUX |
...
Chcemy aby EDUX stale iterował i się rozwijał — a ostatecznie stał się prezentem który oddajemy społeczności open-source. Nasz [**plan działania**](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/issues/498) jest aktualizowany na bieżąco; głosuj na elementy tam lub proponuj nowe. Jeśli chcesz współtworzyć, zapoznaj się z [**Przewodnikiem dla współtwórców**](../../CONTRIBUTING.md) po strategię gałęzi, standardy kodowania i sposób rozpoczęcia.

<div align="center">

Mamy nadzieję, że EDUX stanie się prezentem dla społeczności. 🎁

<a href="https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=Rohit-girish-Belagali/EDUX-EDUORBIT&max=999" alt="Współtwórcy" />
</a>
...
<a href="https://www.star-history.com/#Rohit-girish-Belagali/EDUX-EDUORBIT&type=timeline&legend=top-left">
  <picture>
    ...
    <img alt="Wykres historii gwiazdek" src="https://api.star-history.com/svg?repos=Rohit-girish-Belagali/EDUX-EDUORBIT&type=timeline&legend=top-left" />
  </picture>
</a>
...
<p align="center">
 <a href="https://www.star-history.com/eduorbit/edux">
  <picture>
   ...
   <img alt="Ranking historii gwiazdek" src="https://api.star-history.com/badge?repo=Rohit-girish-Belagali/EDUX-EDUORBIT" />
  </picture>
 </a>
</p>
...
<p>
  <img src="https://visitor-badge.laobi.icu/badge?page_id=EduOrbit.EDUX&style=for-the-badge&color=00d4ff" alt="Odwiedziny">
</p>
