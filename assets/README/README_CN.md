...
[![WeChat](https://img.shields.io/badge/微信-交流群-07C160?style=flat-square&logo=wechat&logoColor=white)](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/issues/78)

[核心功能](#-核心功能) · [快速开始](#-快速开始) · [功能探索](#-探索-edux) · [CLI 命令行](#️-edux-cli--智能体原生界面) · [生态系统](#-生态系统--eduhub-与技能社区) · [社区](#-社区)

</div>

---

> 🤝 **欢迎各种形式的贡献！** 在 [`路线图`](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/issues/498) 为议题投票或提出新建议，详见 [贡献指南](CONTRIBUTING.md)，了解分支策略、编码规范及参与方式。

### 📰 新闻动态

- **2026-05-22** 🌐 官方文档站点上线 [**eduorbit.com**](https://eduorbit.com/) — 指南、参考文档与能力演示一站汇聚。
- **2026-04-19** 🎉 111 天突破 2 万 Star！感谢大家对真正个性化智能辅导的支持。
- **2026-04-10** 📄 论文已发布于 arXiv — 阅读 [预印本](https://arxiv.org/abs/2604.26962)，了解 EDUX 的设计理念与背后的思考。
- **2026-02-06** 🚀 仅 39 天突破 1 万 Star！衷心感谢我们出色的社区。
- **2026-01-01** 🎊 新年快乐！加入我们的 [Discord](https://discord.gg/eRsjPgMU4t)、[微信群](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/issues/78) 或 [Discussions](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/discussions) — 一起塑造 EDUX 的未来。
- **2025-12-29** 🎓 EDUX 正式发布！

## ✨ 核心功能

EDUX 是一个智能体原生的学习工作区，将辅导、解题、测验生成、研究、可视化和掌握度练习整合在一个可扩展的系统中。
...
EDUX 提供四种安装方式，共享同一个工作区布局：设置存储在启动目录下的 `data/user/settings/`（或通过 `EDUX_HOME` / `edux start --home` 指定的位置）。完整应用的推荐流程为：**选择工作目录 → 安装 → `edux init` → `edux start`**。

<details>
<summary><b>方式一 — 从 PyPI 安装</b> · 完整本地 Web 应用 + CLI，无需克隆仓库</summary>

完整本地 Web 应用 + CLI，无需克隆仓库。需要 **Python 3.11+** 以及 PATH 中的 **Node.js 20+** 运行时（打包的 Next.js 独立服务器由 `edux start` 启动）。

```bash
mkdir -p my-edux && cd my-edux
pip install -U edux
edux init     # 提示配置端口 + LLM 提供商 + 可选嵌入
edux start    # 启动后端 + 前端；保持终端窗口打开
```

`edux init` 会提示配置后端端口（默认 `8001`）、前端端口（默认 `3782`）、LLM 提供商 / 基础 URL / API Key / 模型，以及可选的知识库 / RAG 嵌入提供商。

`edux start` 完成后，打开终端打印的前端 URL — 默认为 [http://127.0.0.1:3782](http://127.0.0.1:3782)。在该终端按 `Ctrl+C` 可同时停止后端和前端。跳过 `edux init` 也可用于快速体验；应用会以默认端口和空模型设置启动，稍后在 **Settings → Models** 中配置即可。
...
**开发服务器卡住：** 如果 `edux start` 报告有已存在但无响应的前端进程，停止它打印的 PID。如果实际上没有 Next.js 进程在运行，则锁文件已过时 — 删除后重试：
...
- `ghcr.io/eduorbit/edux:latest` — 稳定版本
- `ghcr.io/eduorbit/edux:pre` — 预发布版本（如有）
...
打开 [http://127.0.0.1:3782](http://127.0.0.1:3782)。容器首次启动时会创建 `/app/data/user/settings/*.json`；通过 Web Settings 页面配置模型提供商。配置、API Key、日志、工作区文件、记忆和知识库均持久化在 `edux-data` 卷中。
...
- **后台运行：** 添加 `-d`，然后用 `docker logs -f edux` 查看日志，`docker stop edux` 停止，重用名称前执行 `docker rm edux`。`edux-data` 卷在重启之间保留设置和工作区。
...
`next_public_api_base_external`（及其别名 `public_api_base`）作为低优先级的备用配置被接受。CORS 使用前端**来源**，而非 API URL。禁用认证时，EDUX 默认允许普通 HTTP/HTTPS 浏览器来源。启用认证时，需添加精确的前端来源：
...
`edux init --cli` 与完整应用共享同一 `data/user/settings/` 布局，但跳过后端/前端端口提示，并默认将嵌入设为**关闭**（如计划使用 `edux kb …` 或 RAG 工具，选择 `Yes`）。它仍会写入完整的运行时布局（`system.json`、`auth.json`、`integrations.json`、`model_catalog.json`、`main.yaml`、`agents.yaml`），并提示选择活跃的 LLM 提供商和模型。
...
本地 `edux-cli` 安装不包含 Web 资产或服务器依赖。请保留源码仓库 — 可编辑安装指向它。若之后需要添加 Web 应用，从同一工作区安装 PyPI 包（方式一），并运行 `edux init` + `edux start`。
...
- **docker-compose：** 通过 `EDUX_SANDBOX_RUNNER_URL` 路由至加固的最小权限**运行器 sidecar**（`Dockerfile.runner`） — 安全性最强，有 sidecar 时自动优先使用。

子进程沙箱由 `data/user/settings/system.json` 中的 `sandbox_allow_subprocess` 设置控制（默认 `true`）。在宿主机上运行模型生成的代码是一个真实的信任决策 — 将其设为 `false`（或导出 `EDUX_SANDBOX_ALLOW_SUBPROCESS=0`）可禁用宿主机侧执行，代价是 Office 技能将无法生成文件。
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/OVERVIEW.png" alt="EDUX 主页 — 带有侧边栏所有入口的 Chat 工作区" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/system%20architecture.png" alt="EDUX 系统架构" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/home/00-overview.png" alt="EDUX 聊天工作区" width="900">
</div>

循环设计刻意保持简单：模型按轮次思考，在有用时调用工具，观察结果，最终以不调用工具的消息结束。`ask_user` 是特殊工具 — 智能体不是凭空猜测，而是可以暂停当前轮次，提出结构化的澄清问题，在你回答后恢复。

<div align="center">
<img src="../../assets/figs/system/chat-agent-loop.png" alt="EDUX 聊天智能体循环" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/partners/00-partners%20overview.png" alt="EDUX Partners 工作区" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/partners-architecture.png" alt="EDUX Partners 架构" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/myagents/00-overview.png" alt="EDUX 我的智能体工作区" width="900">
</div>

"我的智能体"将其他智能体转化为 EDUX 的上下文，具备两种不同的功能。**连接实时智能体** — 连接你机器上的 Claude Code 或 Codex CLI，或你的某个 Partner，在聊天轮次中调用它：EDUX 实际上会*运行*另一个智能体，并通过 `consult_subagent` 工具将其工作流式传输到 Activity 面板。通过智能体选项（或输入 `@`）选择它，并设置调用可进行的最大轮数。

<div align="center">
<img src="../../assets/figs/web-1.4.6+/home/08-subagent%20demo%20with%20claude%20code.png" alt="实时调用 Claude Code 子智能体" width="900">
</div>

**导入历史对话** — 将已有的 Claude Code 和 Codex 历史记录作为命名的、可搜索的、可续聊的智能体导入。选择要导入的日期范围；刷新时自动重新同步。通过 `+` → 我的智能体在任意聊天轮次中引用已导入的对话，EDUX 会将其作为第三方对话记录读取 — 它始终是*对方的*对话，不会被 EDUX 以自己的口吻解读。
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/co-writer/00-overview.png" alt="EDUX Co-Writer 工作区" width="900">
</div>
...
其核心理念是**精准编辑**：选中一段文字，让 EDUX 对其进行改写、扩展或缩短。编辑智能体可以基于知识库或网络证据进行修改，保留工具调用追踪，并以接受/拒绝差异对比的形式展示每处变更 — 直到你批准后才会生效。
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/book/00-book_overview.png" alt="EDUX 书籍库" width="900">
</div>
...
每章编译为类型化块 — 文本、标注、测验、闪卡、时间轴、代码、图形、交互式 HTML、动画、概念图、深度解析和用户笔记 — 每页都有独立的 Page Chat。块可编辑：插入、移动、重新生成或切换块类型，无需重写整章。维护命令如 `edux book health` 和 `edux book refresh-fingerprints` 有助于检测来源知识库与已编译页面的漂移情况。
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/knowledge/00-overview.png" alt="EDUX 知识中心" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/learning-space/00-overview.png" alt="EDUX 学习空间中心" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/memory/00-overview.png" alt="EDUX 记忆概览" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/memory/01-3%20layer%20memory%20graph.png" alt="EDUX 记忆图谱" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/settings/00-setting%20overview.png" alt="EDUX 设置中心" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/settings/01-appearance%20settings.png" alt="EDUX 外观设置与主题" width="900">
</div>

大多数部分采用草稿-应用流程，因此你可以在提交前测试提供商配置。开箱即提供四种主题 — Default、Cream、Dark 和 Glass。项目根目录的 `.env` 文件被刻意忽略；运行时配置存储在 `data/user/settings/*.json` 下，除非 `EDUX_HOME` 或 `edux start --home` 将应用指向其他位置。
...
认证默认**关闭** — EDUX 以单用户模式运行。开启后，单个 `data/` 目录树可同时托管管理员工作区、隔离的用户工作区和 Partner 工作区：
...
**启用方式：** 在 `data/user/settings/auth.json` 中开启认证，重启 `edux start`，在 `/register` 注册第一个管理员，然后从 `/admin/users` 添加用户，并通过授权分配模型、知识库、技能、Partner、工具/MCP 策略和代码执行权限。
...
一个 `edux` 可执行文件，两种使用方式：供习惯在终端中工作的人使用的交互式 **REPL**，以及供将 EDUX 作为工具来驱动的其他智能体使用的结构化 **JSON** 输出。两种方式共享相同的能力、工具和知识库。

<details>
<summary><b>自己驱动</b></summary>

`edux chat` 打开交互式 REPL；`edux run <capability> "<message>"` 执行单次对话并退出。两者共享相同的 `--capability`、`--tool`、`--kb` 和 `--config` 标志。

```bash
edux chat                                              # 交互式 REPL
edux chat --capability deep_solve --kb my-kb --tool rag
edux run chat "解释傅里叶变换" --tool rag --kb textbook
edux run deep_research "调研 2026 年 RAG 论文" \
...
EDUX 专为*被其他智能体操作*而设计。在任何 `run` 命令中添加 `--format json`，每个轮次将流式输出 **NDJSON — 每行一个事件**（`content`、`tool_call`、`tool_result`、`done` 等），每行带有其 `session_id` 标记。运行是无头安全的：无 TTY 时，`ask_user` 暂停会以空回复自动解决，而不是挂起。
...
SID=$(edux run deep_research "调研 2026 年 RAG 论文" \
  --config mode=report --config depth=standard --format json \
  | jq -r 'select(.type=="done").session_id')
edux run deep_question "就那篇调研测验我" --session "$SID" --format json
```

仓库根目录附带 [`SKILL.md`](SKILL.md) — 约 150 行的交接文档，让任何支持工具调用的 LLM 一次性掌握所有接口。将其传递给 Claude Code、Codex 或 OpenCode（它们会自动读取 `SKILL.md`），或将 `edux run` 包装为 LangChain / AutoGen 循环中的工具。完整示例：[Agent Handoff](https://eduorbit.com/docs/cli/agent-handoff/)。
...
| 命令 | 说明 |
|:---|:---|
| `edux init` | 为当前工作区创建或更新 `data/user/settings` |
| `edux start [--home PATH]` | 同时启动后端 + 前端 |
| `edux serve [--port PORT]` | 仅启动 FastAPI 后端 |
| `edux run <capability> <message>` | 运行单次能力对话（`chat`、`deep_solve`、`deep_question`、`deep_research`、`visualize`、`math_animator`、`mastery_path`）；添加 `--format json` 可获得 NDJSON 输出 |
| `edux chat` | 交互式 REPL，支持能力、工具、知识库、笔记本和历史控制 |
| `edux partner list/create/start/stop` | 管理 IM 连接的 Partners |
| `edux kb list/info/create/add/search/set-default/delete` | 管理 LlamaIndex 知识库 |
| `edux skill search/install/list/remove/login/logout/publish/update` | 管理技能、从 Hub 安装并发布自己的技能（默认 `eduhub:<slug>`，详见生态系统） |
| `edux memory show/clear` | 查看 L2/L3 记忆文档或清除 L1/全部记忆 |
| `edux session list/show/open/rename/delete` | 管理共享会话 |
| `edux notebook list/create/show/add-md/replace-md/remove-record` | 从 Markdown 文件管理笔记本 |
| `edux book list/health/refresh-fingerprints` | 查看书籍并刷新来源指纹 |
| `edux plugin list/info` | 查看已注册的工具和能力 |
| `edux config show` | 打印配置摘要 |
| `edux provider login <provider>` | 提供商认证（`openai-codex` OAuth 登录；`github-copilot` 验证现有 Copilot 认证会话） |
...
仅 CLI 包位于 `packaging/edux-cli`。在此代码仓库中，从源码安装：
...
EDUX 技能使用开放的 **Agent-Skills** 格式 — 一个包含 `SKILL.md` 剧本（YAML frontmatter + Markdown）和可选参考文件的文件夹。该格式与 EDUX 无关，因此任何支持该格式的注册表都可以成为你的技能库来源。EDUX 内置了 **[EduHub](https://eduhub.eduorbit.com/)** — 我们自己的教育技能注册表 — 作为默认 Hub。

<details>
<summary><b>EduHub — EDUX 的技能生态</b></summary>

[**EduHub**](https://eduhub.eduorbit.com/) 是 EDUX 为分享教学导向的智能体技能而创建的社区 Hub — 苏格拉底式导师、闪卡生成器、作文反馈、考试蓝图、概念讲解器等。它内置于 EDUX，无需任何配置：裸 slug 或 `eduhub:` 前缀均可解析到它。

**查找与安装** — 在浏览器中，打开**学习空间 → 技能 → 从 EduHub 导入**，浏览目录并将技能直接下载到你的库中。从终端：

```bash
edux skill search "socratic tutor"               # 在 EduHub（默认 Hub）中搜索
edux skill install socratic-tutor                # 获取 → 验证 → 注册
edux skill install eduhub:socratic-tutor@1.2.0   # 指定 Hub 和版本
edux skill list                                  # 本地技能及其 Hub 来源
```

**发布自己的技能** — 打包一个 `SKILL.md` 并分享给社区：

```bash
edux skill login                                 # 浏览器登录 EduHub
edux skill publish ./my-skill                    # 交互式：选择分类 + 标签，然后上传
edux skill update                                # 回滚或发布新版本
```

EduHub 也是一个独立的、ClawHub 兼容的注册表，因此非 EDUX 的智能体（Claude Code、Codex 等）可以通过 `eduhub` CLI 直接使用它 — `npx eduhub install socratic-tutor`。
...
因为 EDUX 支持开放的 Agent-Skills 格式，**[ClawHub](https://clawhub.ai/)** 也是一等来源 — 它与 EduHub 并列内置。使用 Hub 前缀选择：
...
EDUX 是一个由 [EduOrbit](https://github.com/EduOrbit) 团队中的 [Bingxi Zhao](https://github.com/pancacake) 主导的开源项目，以**完全开源的形式**持续迭代，与社区共同构建。迄今为止，我们**没有**任何形式的付费在线产品。欢迎通过 **bingxizhao39@gmail.com** 联系我们，探讨想法或合作。

### 🙏 致谢

衷心感谢香港大学数据智能实验室主任 [**Chao Huang**](https://sites.google.com/view/chaoh) 的大力支持，以及 EduOrbit 实验室同学们的热心相助 — 特别是 [**Jiahao Zhang**](https://github.com/zzhtx258)、[**Zirui Guo**](https://github.com/LarFii) 和 [**Xubin Ren**](https://github.com/Re-bin)。我们也对**开源社区**深表感激：你们的 Star、Issue、Pull Request 和讨论，每天都在塑造 EDUX。

EDUX 也站在众多优秀开源项目的肩膀上，它们给予了我们工具和灵感：

| 项目 | 角色 / 启发 |
|:---|:---|
| [**LlamaIndex**](https://github.com/run-llama/llama_index) | RAG 流水线和文档索引基础 |
| [**nanobot**](https://github.com/EduOrbit/nanobot) | 驱动原版 TutorBot 的超轻量智能体引擎 *(EduOrbit)* |
| [**LightRAG**](https://github.com/EduOrbit/LightRAG) | 简单快速的 RAG *(EduOrbit)* |
| [**AutoAgent**](https://github.com/EduOrbit/AutoAgent) | 零代码智能体框架 *(EduOrbit)* |
| [**AI-Researcher**](https://github.com/EduOrbit/AI-Researcher) | 自动化研究流水线 *(EduOrbit)* |
...
| [**Claude Code**](https://github.com/anthropics/claude-code) | 启发 EDUX 智能体循环的智能体编程 CLI |
| [**ManimCat**](https://github.com/Wing900/ManimCat) | Math Animator 的 AI 驱动数学动画生成 |

### 🗺️ 路线图与贡献

我们希望 EDUX 持续迭代与进步 — 并最终成为我们回馈开源社区的礼物。我们的[**路线图**](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/issues/498)持续更新；欢迎在那里为议题投票或提出新想法。如果你想贡献，请查看[**贡献指南**](CONTRIBUTING.md)，了解分支策略、编码规范及参与方式。

<div align="center">

我们希望 EDUX 成为送给社区的一份礼物。🎁

<a href="https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=Rohit-girish-Belagali/EDUX-EDUORBIT&max=999" alt="贡献者" />
</a>
...
<p>
  <img src="https://visitor-badge.laobi.icu/badge?page_id=EduOrbit.EDUX&style=for-the-badge&color=00d4ff" alt="访问量">
</p>
