...
# EDUX: लाइफलॉन्ग व्यक्तिगत ट्यूटरिंग
...
[विशेषताएं](#-मुख्य-विशेषताएं) · [शुरू करें](#-शुरू-करें) · [एक्सप्लोर करें](#-edux-को-एक्सप्लोर-करें) · [CLI](#️-edux-cli--एजेंट-नेटिव-इंटरफेस) · [इकोसिस्टम](#-इकोसिस्टम--eduhub-और-skills-community) · [समुदाय](#-समुदाय)

</div>

---

> 🤝 **हम किसी भी प्रकार के योगदान का स्वागत करते हैं!** [`Roadmap`](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/issues/498) पर roadmap items के लिए वोट करें या नए प्रस्तावित करें, और branching strategy, coding standards और शुरू करने के तरीके के लिए हमारी [Contributing Guide](../../CONTRIBUTING.md) देखें।

### 📰 समाचार

- **2026-05-22** 🌐 आधिकारिक डॉक्स साइट [**eduorbit.com**](https://eduorbit.com/) पर live — guides, references, और capability tours एक ही जगह।
...
- **2026-04-10** 📄 हमारा paper अब arXiv पर live है — EDUX के design और विचारों के लिए [preprint](https://arxiv.org/abs/2604.26962) पढ़ें।
- **2026-02-06** 🚀 39 दिनों में 10k स्टार्स! हमारे अविश्वसनीय community के समर्थन के लिए बहुत धन्यवाद।
- **2026-01-01** 🎊 नया साल मुबारक! हमारे [Discord](https://discord.gg/eRsjPgMU4t), [WeChat](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/issues/78), या [Discussions](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/discussions) से जुड़ें — आइए मिलकर EDUX को आकार दें।
- **2025-12-29** 🎓 EDUX आधिकारिक रूप से जारी हुआ!

## ✨ मुख्य विशेषताएं

EDUX एक agent-native learning workspace है जो tutoring, problem solving, quiz generation, research, visualization, और mastery practice को एक extensible system में जोड़ता है।
...
EDUX चार installation paths के साथ आता है। वे सभी एक workspace layout साझा करते हैं: settings उस directory के नीचे `data/user/settings/` में रहती हैं जहां से आप launch करते हैं (या `EDUX_HOME` / `edux start --home` के नीचे अगर आप explicitly set करते हैं)। पूरे app के लिए, recommended flow है **workspace directory चुनें → install करें → `edux init` → `edux start`**।

<details>
<summary><b>Option 1 — PyPI से Install करें</b> · पूरा local Web app + CLI, clone की जरूरत नहीं</summary>

पूरा local Web app + CLI, clone की जरूरत नहीं। **Python 3.11+** और PATH पर **Node.js 20+** runtime चाहिए (`edux start` packaged Next.js standalone server को spawn करता है)।

```bash
mkdir -p my-edux && cd my-edux
pip install -U edux
edux init     # ports + LLM provider + optional embedding के लिए prompt करता है
edux start    # backend + frontend शुरू करता है; terminal खुला रखें
```

`edux init` backend port (default `8001`), frontend port (default `3782`), LLM provider / base URL / API key / model, और Knowledge Base / RAG के लिए optional embedding provider के लिए prompt करता है।

`edux start` के बाद, terminal में print किया गया frontend URL खोलें — default रूप से [http://127.0.0.1:3782](http://127.0.0.1:3782)। backend और frontend दोनों को रोकने के लिए उस terminal में `Ctrl+C` दबाएं। Quick trial के लिए `edux init` छोड़ना ठीक है; app default ports और empty model settings के साथ boot होगा, उन्हें बाद में **Settings → Models** में configure करें।
...
**Stuck dev server:** अगर `edux start` एक existing frontend report करता है जो respond नहीं कर रहा, तो उस PID को stop करें जो वह print करता है। अगर कोई Next.js process actually नहीं चल रही, तो lock files stale हैं — उन्हें remove करें और retry करें:
...
- `ghcr.io/eduorbit/edux:latest` — stable release
- `ghcr.io/eduorbit/edux:pre` — pre-release, जब उपलब्ध हो
...
[http://127.0.0.1:3782](http://127.0.0.1:3782) खोलें। Container पहले boot पर `/app/data/user/settings/*.json` बनाता है; Web Settings page से model providers configure करें। Config, API keys, logs, workspace files, memory, और knowledge bases `edux-data` volume में persist करते हैं।
...
- **Detached:** `-d` add करें, फिर follow करने के लिए `docker logs -f edux`, stop करने के लिए `docker stop edux`, नाम reuse करने से पहले `docker rm edux`। `edux-data` volume आपकी settings और workspace को restarts के पार रखता है।
...
`next_public_api_base_external` (और इसका alias `public_api_base`) lower-precedence fallbacks के रूप में accept किए जाते हैं। CORS frontend **origins** उपयोग करता है, API URLs नहीं। Auth disabled होने पर, EDUX default रूप से normal HTTP/HTTPS browser origins permit करता है। Auth enabled होने पर, exact frontend origins add करें:
...
`edux init --cli` पूरे app के समान `data/user/settings/` layout share करता है लेकिन backend/frontend port prompts skip करता है और embeddings को **off** default करता है (अगर आप `edux kb …` या RAG tools उपयोग करने की योजना रखते हैं तो `Yes` चुनें)। यह फिर भी एक complete runtime layout (`system.json`, `auth.json`, `integrations.json`, `model_catalog.json`, `main.yaml`, `agents.yaml`) लिखता है और active LLM provider और model के लिए prompt करता है।
...
Local `edux-cli` install में कोई Web assets या server dependencies नहीं हैं। Source checkout को आसपास रखें — editable install उस पर point करता है। बाद में Web app add करने के लिए, PyPI package (Option 1) install करें और same workspace से `edux init` + `edux start` run करें।
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/OVERVIEW.png" alt="EDUX home — sidebar में हर surface के साथ Chat workspace" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/system%20architecture.png" alt="EDUX system architecture" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/home/00-overview.png" alt="EDUX chat workspace" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/chat-agent-loop.png" alt="EDUX chat agent loop" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/partners/00-partners%20overview.png" alt="EDUX partners workspace" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/partners-architecture.png" alt="EDUX partners architecture" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/myagents/00-overview.png" alt="EDUX My Agents workspace" width="900">
</div>

My Agents दूसरे agents को EDUX के लिए context बनाता है, और दो अलग काम करता है। **लाइव एजेंट connect करें** — आपकी machine पर Claude Code या Codex CLI, या आपके Partners में से एक — और इसे chat turn के अंदर से consult करें: EDUX actually दूसरे agent को *run* करता है और इसके काम को `consult_subagent` tool के जरिए Activity panel में stream करता है। इसे Agent chip से select करें (या `@` type करें), और set करें कि consult कितने rounds ले सकता है।
...
**पिछली conversations import करें** — अपनी existing Claude Code और Codex history को named, searchable, resumable agents के रूप में bring in करें। Import करने के लिए कौन से days लेने हैं चुनें; refreshing उन्हें re-sync करता है। किसी भी chat turn से imported conversation को `+` → My Agents के जरिए reference करें, और EDUX इसे एक third-party transcript के रूप में पढ़ता है — यह उनकी conversation रहती है, EDUX की अपनी आवाज नहीं।
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/co-writer/00-overview.png" alt="EDUX Co-Writer workspace" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/book/00-book_overview.png" alt="EDUX book library" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/knowledge/00-overview.png" alt="EDUX Knowledge Center" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/learning-space/00-overview.png" alt="EDUX Learning Space hub" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/memory/00-overview.png" alt="EDUX memory overview" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/memory/01-3%20layer%20memory%20graph.png" alt="EDUX memory graph" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/settings/00-setting%20overview.png" alt="EDUX settings hub" width="900">
</div>
...
Authentication **default रूप से बंद** है — EDUX single-user चलता है। इसे on करें और एक `data/` tree एक admin workspace, isolated per-user workspaces, और partner workspaces को side by side host करती है:
...
**Enable करें:** `data/user/settings/auth.json` में auth on करें, `edux start` restart करें, `/register` पर पहला admin register करें, फिर `/admin/users` से users add करें और grants के जरिए models, KBs, skills, Partners, tool/MCP policy, और code-execution access assign करें।
...
## ⌨️ EDUX CLI — एजेंट-नेटिव इंटरफेस

एक `edux` binary, दो तरीके से: terminal में रहने वालों के लिए interactive **REPL**, और EDUX को tool के रूप में drive करने वाले दूसरे agents के लिए structured **JSON**। दोनों तरफ same capabilities, tools, और knowledge bases।

<details>
<summary><b>खुद drive करें</b></summary>

`edux chat` एक interactive REPL खोलता है; `edux run <capability> "<message>"` एक single turn fire करके exit करता है। दोनों same `--capability`, `--tool`, `--kb`, और `--config` flags बोलते हैं।
...
| Command | विवरण |
|:---|:---|
| `edux init` | Current workspace के लिए `data/user/settings` create या update करें |
| `edux start [--home PATH]` | Backend + frontend को एक साथ launch करें |
| `edux serve [--port PORT]` | केवल FastAPI backend start करें |
| `edux run <capability> <message>` | एक single capability turn run करें (`chat`, `deep_solve`, `deep_question`, `deep_research`, `visualize`, `math_animator`, `mastery_path`); NDJSON output के लिए `--format json` add करें |
| `edux chat` | capability, tool, KB, notebook, और history controls के साथ interactive REPL |
| `edux partner list/create/start/stop` | IM-connected partners manage करें |
| `edux kb list/info/create/add/search/set-default/delete` | LlamaIndex knowledge bases manage करें |
| `edux skill search/install/list/remove/login/logout/publish/update` | Skills manage करें, hubs से install करें, और अपनी खुद publish करें (default `eduhub:<slug>`, Ecosystem देखें) |
| `edux memory show/clear` | L2/L3 memory docs inspect करें या L1/all memory clear करें |
| `edux session list/show/open/rename/delete` | Shared sessions manage करें |
| `edux notebook list/create/show/add-md/replace-md/remove-record` | Markdown files से notebooks manage करें |
| `edux book list/health/refresh-fingerprints` | Books inspect करें और source fingerprints refresh करें |
| `edux plugin list/info` | Registered tools और capabilities inspect करें |
| `edux config show` | Configuration summary print करें |
| `edux provider login <provider>` | Provider auth (`openai-codex` OAuth login; `github-copilot` existing Copilot auth session validate करता है) |
...
CLI-only package `packaging/edux-cli` में रहता है। इस checkout में, इसे source से install करें:
...
EDUX skills open **Agent-Skills** format उपयोग करती हैं — एक `SKILL.md` playbook (YAML frontmatter + Markdown) और optional reference files के साथ एक folder। इसमें EDUX-specific कुछ नहीं है, इसलिए format बोलने वाली कोई भी registry आपकी library के लिए एक source बन जाती है। EDUX **[EduHub](https://eduhub.eduorbit.com/)** के साथ ship होता है — हमारी अपनी education-focused skill registry — default hub के रूप में built in।

<details>
<summary><b>EduHub — EDUX का skill ecosystem</b></summary>

[**EduHub**](https://eduhub.eduorbit.com/) वह community hub है जिसे EDUX ने teaching-oriented agent skills share करने के लिए launch किया — Socratic tutors, flashcard builders, essay feedback, exam blueprints, concept explainers, और बहुत कुछ। यह EDUX में built in है, इसलिए configure करने की कोई जरूरत नहीं: एक bare slug या `eduhub:` prefix इसे resolve करता है।
...
```bash
edux skill search "socratic tutor"               # EduHub search करें (default hub)
edux skill install socratic-tutor                # fetch → verify → register
edux skill install eduhub:socratic-tutor@1.2.0   # hub और version pin करें
edux skill list                                  # उनके hub provenance के साथ local skills
```

**अपनी खुद publish करें** — एक `SKILL.md` package करें और community के साथ share करें:

```bash
edux skill login                                 # EduHub पर browser sign-in
edux skill publish ./my-skill                    # interactive: track + tags चुनें, फिर upload
edux skill update                                # roll back या नया version release करें
```

EduHub एक standalone, ClawHub-compatible registry भी है, इसलिए EDUX नहीं होने वाले agents (Claude Code, Codex, …) इसे `eduhub` CLI के जरिए directly use कर सकते हैं — `npx eduhub install socratic-tutor`।
...
- frontmatter EDUX के schema में normalize होता है और `always:` **stripped** होता है, इसलिए एक downloaded skill खुद को हर system prompt में force नहीं कर सकती;
...
क्योंकि EDUX open Agent-Skills format बोलता है, **[ClawHub](https://clawhub.ai/)** भी एक first-class source है — यह EduHub के साथ built in है। इसे hub prefix से चुनें:
...
EDUX [EduOrbit](https://github.com/EduOrbit) Group के अंदर [Bingxi Zhao](https://github.com/pancacake) द्वारा lead किया जाने वाला एक open-source project है, और यह **पूरी तरह open-source रूप में**, community के साथ मिलकर बनाया जाता है। अब तक, हमारे पास किसी भी प्रकार के **paid online products नहीं** हैं। discussions, ideas, या collaboration के लिए **bingxizhao39@gmail.com** पर contact करें।

### 🙏 आभार

[**Chao Huang**](https://sites.google.com/view/chaoh), Data Intelligence Lab @ HKU के director, और उनके warm support के लिए हमारे EduOrbit labmates — विशेष रूप से [**Jiahao Zhang**](https://github.com/zzhtx258), [**Zirui Guo**](https://github.com/LarFii), और [**Xubin Ren**](https://github.com/Re-bin) — के प्रति हार्दिक आभार। हम **open-source community** के प्रति भी गहराई से आभारी हैं: आपके stars, issues, pull requests, और discussions हर एक दिन EDUX को आकार देते हैं।

EDUX outstanding open-source projects के कंधों पर खड़ा है जिन्होंने हमें tools और inspiration दोनों दिए:

| Project | भूमिका / Inspiration |
|:---|:---|
| [**LlamaIndex**](https://github.com/run-llama/llama_index) | RAG pipeline और document-indexing backbone |
| [**nanobot**](https://github.com/EduOrbit/nanobot) | Ultra-lightweight agent engine जिसने original TutorBot को powered किया *(EduOrbit)* |
| [**LightRAG**](https://github.com/EduOrbit/LightRAG) | Simple & fast RAG *(EduOrbit)* |
| [**AutoAgent**](https://github.com/EduOrbit/AutoAgent) | Zero-code agent framework *(EduOrbit)* |
| [**AI-Researcher**](https://github.com/EduOrbit/AI-Researcher) | Automated research pipeline *(EduOrbit)* |
...
| [**Claude Code**](https://github.com/anthropics/claude-code) | Agentic coding CLI जिसने EDUX agent loop को inspire किया |
| [**ManimCat**](https://github.com/Wing900/ManimCat) | Math Animator के लिए AI-driven math animation generation |

### 🗺️ Roadmap और योगदान

हम चाहते हैं कि EDUX iterate और improve करता रहे — और अंततः open-source community को एक gift बने। हमारा [**roadmap**](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/issues/498) continuously update होता है; वहां items पर vote करें या नए propose करें। अगर आप contribute करना चाहते हैं, तो branching strategy, coding standards, और शुरू करने के तरीके के लिए [**Contributing Guide**](../../CONTRIBUTING.md) देखें।

<div align="center">

हम आशा करते हैं कि EDUX community के लिए एक उपहार बने। 🎁
