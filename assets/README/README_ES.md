...
# EDUX: Tutoría Personalizada de Por Vida
...
[Características](#-características-principales) · [Comenzar](#-comenzar) · [Explorar](#-explorar-edux) · [CLI](#️-edux-cli--interfaz-nativa-de-agentes) · [Ecosistema](#-ecosistema--eduhub-y-la-comunidad-de-skills) · [Comunidad](#-comunidad)

</div>

---

> 🤝 **¡Damos la bienvenida a cualquier tipo de contribución!** Vota en los elementos del roadmap o propone nuevos en [`Roadmap`](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/issues/498), y consulta nuestra [Guía de Contribución](../../CONTRIBUTING.md) para la estrategia de ramas, estándares de código y cómo comenzar.

### 📰 Noticias

- **2026-05-22** 🌐 Sitio de documentación oficial disponible en [**eduorbit.com**](https://eduorbit.com/) — guías, referencias y tours de capacidades, todo en un solo lugar.
- **2026-04-19** 🎉 ¡20k estrellas en 111 días! Gracias por el apoyo hacia una tutoría verdaderamente personalizada e inteligente.
- **2026-04-10** 📄 Nuestro artículo ya está en arXiv — lee el [preprint](https://arxiv.org/abs/2604.26962) para conocer el diseño y las ideas detrás de EDUX.
- **2026-02-06** 🚀 ¡10k estrellas en solo 39 días! Un enorme agradecimiento a nuestra increíble comunidad.
- **2026-01-01** 🎊 ¡Feliz Año Nuevo! Únete a nuestro [Discord](https://discord.gg/eRsjPgMU4t), [WeChat](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/issues/78) o [Discussions](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/discussions) — juntos demos forma al futuro de EDUX.
- **2025-12-29** 🎓 ¡EDUX está oficialmente lanzado!

## ✨ Características Principales

EDUX es un espacio de trabajo de aprendizaje nativo de agentes que conecta tutoría, resolución de problemas, generación de cuestionarios, investigación, visualización y práctica de dominio en un sistema extensible.
...
EDUX incluye cuatro rutas de instalación. Todas comparten un diseño de espacio de trabajo: la configuración vive en `data/user/settings/` bajo el directorio desde el que se inicia (o bajo `EDUX_HOME` / `edux start --home` si se establece explícitamente). Para la aplicación completa, el flujo recomendado es **elegir un directorio de espacio de trabajo → instalar → `edux init` → `edux start`**.

<details>
<summary><b>Opción 1 — Instalar desde PyPI</b> · aplicación web local completa + CLI, sin necesidad de clonar</summary>

Aplicación web local completa + CLI, sin necesidad de clonar. Requiere **Python 3.11+** y un runtime **Node.js 20+** en PATH (el servidor standalone Next.js empaquetado es iniciado por `edux start`).

```bash
mkdir -p my-edux && cd my-edux
pip install -U edux
edux init     # solicita puertos + proveedor LLM + embedding opcional
edux start    # inicia backend + frontend; mantener la terminal abierta
```

`edux init` solicita el puerto de backend (predeterminado `8001`), el puerto de frontend (predeterminado `3782`), proveedor LLM / URL base / clave API / modelo y un proveedor de embeddings opcional para Base de Conocimiento / RAG.

Después de `edux start`, abre la URL del frontend impresa en la terminal — por defecto [http://127.0.0.1:3782](http://127.0.0.1:3782). Presiona `Ctrl+C` en esa terminal para detener tanto el backend como el frontend. Omitir `edux init` está bien para una prueba rápida; la aplicación arranca con puertos predeterminados y configuración de modelo vacía, configúralos luego en **Settings → Models**.
...
**Servidor de desarrollo atascado:** si `edux start` informa de un frontend existente que no responde, detén el PID que imprime. Si no hay ningún proceso Next.js en ejecución, los archivos de bloqueo están obsoletos — elimínalos y vuelve a intentarlo:
...
- `ghcr.io/eduorbit/edux:latest` — versión estable
- `ghcr.io/eduorbit/edux:pre` — versión preliminar, cuando esté disponible
...
- **Desconectado:** agrega `-d`, luego `docker logs -f edux` para seguir, `docker stop edux` para detener, `docker rm edux` antes de reutilizar el nombre. El volumen `edux-data` mantiene tu configuración y espacio de trabajo entre reinicios.
...
`next_public_api_base_external` (y su alias `public_api_base`) se aceptan como alternativas de menor precedencia. CORS usa **orígenes** de frontend, no URLs de API. Con la autenticación deshabilitada, EDUX permite los orígenes normales de navegador HTTP/HTTPS por defecto. Con la autenticación habilitada, agrega los orígenes exactos del frontend:
...
`edux init --cli` comparte el mismo diseño `data/user/settings/` que la aplicación completa pero omite las solicitudes de puertos de backend/frontend y establece embeddings en **desactivado** (elige `Yes` si planeas usar `edux kb …` o herramientas RAG). Aún escribe un diseño de runtime completo (`system.json`, `auth.json`, `integrations.json`, `model_catalog.json`, `main.yaml`, `agents.yaml`) y aún solicita el proveedor LLM y modelo activos.

<details>
<summary><b>Comandos comunes</b></summary>

```bash
edux chat                                          # REPL interactivo
...
La instalación local de `edux-cli` no incluye activos web ni dependencias de servidor. Mantén el checkout de fuente cerca — la instalación editable apunta a él. Para agregar la aplicación web más tarde, instala el paquete PyPI (Opción 1) y ejecuta `edux init` + `edux start` desde el mismo espacio de trabajo.
...
- **docker-compose:** enrutado en su lugar a un **sidecar runner** endurecido y con privilegios mínimos
  (`Dockerfile.runner`) a través de `EDUX_SANDBOX_RUNNER_URL` — la postura más sólida, y preferida
...
`EDUX_SANDBOX_ALLOW_SUBPROCESS=0`) para deshabilitar la ejecución del lado del host, a costa de
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/OVERVIEW.png" alt="Inicio de EDUX — el espacio de trabajo Chat con todas las superficies en la barra lateral" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/system%20architecture.png" alt="Arquitectura del sistema EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/home/00-overview.png" alt="Espacio de trabajo de chat EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/chat-agent-loop.png" alt="Bucle de agente de chat EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/partners/00-partners%20overview.png" alt="Espacio de trabajo de partners EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/partners-architecture.png" alt="Arquitectura de partners EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/myagents/00-overview.png" alt="Espacio de trabajo de Mis Agentes EDUX" width="900">
</div>

Mis Agentes convierte a otros agentes en contexto para EDUX y hace dos cosas distintas. **Conectar un agente en vivo** — un Claude Code o Codex CLI en tu máquina, o uno de tus Partners — y consultarlo desde dentro de un turno de chat: EDUX realmente *ejecuta* el otro agente y transmite su trabajo al panel de Activity a través de la herramienta `consult_subagent`. Selecciónalo con el chip de Agente (o escribe `@`), y establece cuántas rondas puede tomar la consulta.
...
**Importar conversaciones pasadas** — trae tu historial existente de Claude Code y Codex como agentes nombrados, buscables y reanudables. Elige qué días importar; actualizar los vuelve a sincronizar. Referencia una conversación importada desde cualquier turno de chat a través de `+` → Mis Agentes, y EDUX la lee como un transcript de terceros — sigue siendo *su* conversación, no la voz propia de EDUX.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/co-writer/00-overview.png" alt="Espacio de trabajo Co-Writer EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/book/00-book_overview.png" alt="Biblioteca de libros EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/knowledge/00-overview.png" alt="Centro de Conocimiento EDUX" width="900">
</div>
...
Al crear una KB, puedes **crear nueva** (subir documentos y construir un índice nuevo) o **vincular existente** (reutilizar un índice construido en otro lugar, leer en el lugar sin re-indexar). La re-indexación escribe un nuevo directorio `version-N` plano y conserva los anteriores, de modo que un índice funcional nunca se destruye a mitad de la reconstrucción. Un solo documento puede eliminarse incluso de una base en estado de **error** — descartando un archivo que no se pudo analizar sin necesidad de borrar y reconstruir todo. El análisis de documentos — Text-only, MinerU, Docling, markitdown o PyMuPDF4LLM — se elige en **Settings → Knowledge Base**, con descargas de modelos locales desactivadas por defecto. La CLI refleja el ciclo de vida con `edux kb list`, `info`, `create`, `add`, `search`, `set-default` y `delete`.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/learning-space/00-overview.png" alt="Hub del Espacio de Aprendizaje EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/memory/00-overview.png" alt="Vista general de Memory EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/memory/01-3%20layer%20memory%20graph.png" alt="Gráfico de memoria EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/settings/00-setting%20overview.png" alt="Hub de configuración EDUX" width="900">
</div>
...
La autenticación está **desactivada por defecto** — EDUX corre en modo monousuario. Actívala y un árbol `data/` aloja un espacio de trabajo de administrador, espacios de trabajo aislados por usuario y espacios de trabajo de partner en paralelo:
...
**Activarlo:** activa la autenticación en `data/user/settings/auth.json`, reinicia `edux start`, registra al primer administrador en `/register`, luego agrega usuarios desde `/admin/users` y asigna modelos, KBs, skills, Partners, política de herramientas/MCP y acceso de ejecución de código a través de permisos.
...
## ⌨️ EDUX CLI — Interfaz Nativa de Agentes

Un binario `edux`, dos formas de entrar: un **REPL** interactivo para quienes viven en la terminal, y **JSON** estructurado para otros agentes que manejan EDUX como herramienta. Las mismas capacidades, herramientas y bases de conocimiento de cualquier manera.

<details>
<summary><b>Manejarlo tú mismo</b></summary>

`edux chat` abre un REPL interactivo; `edux run <capability> "<message>"` ejecuta un único turno y sale. Ambos comparten los mismos flags `--capability`, `--tool`, `--kb` y `--config`.
...
EDUX está construido para ser *operado por otro agente*. Agrega `--format json` a cualquier `run` y cada turno transmite **NDJSON — un evento por línea** (`content`, `tool_call`, `tool_result`, `done`, …), cada línea etiquetada con su `session_id`. Las ejecuciones son seguras sin TTY: una pausa `ask_user` sin TTY se resuelve automáticamente con una respuesta vacía en lugar de bloquearse.
...
El repo incluye un [`SKILL.md`](../../SKILL.md) raíz — un documento de traspaso de ~150 líneas que enseña a cualquier LLM que use herramientas toda la superficie en una lectura. Entrégaselo a Claude Code, Codex u OpenCode (los recogen automáticamente), o envuelve `edux run` como herramienta en un bucle LangChain / AutoGen. Recetas completas: [Agent Handoff](https://eduorbit.com/docs/cli/agent-handoff/).
...
| Comando | Descripción |
|:---|:---|
| `edux init` | Crear o actualizar `data/user/settings` para el espacio de trabajo actual |
| `edux start [--home PATH]` | Lanzar backend + frontend juntos |
| `edux serve [--port PORT]` | Iniciar solo el backend FastAPI |
| `edux run <capability> <message>` | Ejecutar un turno de capacidad único (`chat`, `deep_solve`, `deep_question`, `deep_research`, `visualize`, `math_animator`, `mastery_path`); agrega `--format json` para salida NDJSON |
| `edux chat` | REPL interactivo con controles de capacidad, herramienta, KB, notebook e historial |
| `edux partner list/create/start/stop` | Gestionar partners conectados por IM |
| `edux kb list/info/create/add/search/set-default/delete` | Gestionar bases de conocimiento LlamaIndex |
| `edux skill search/install/list/remove/login/logout/publish/update` | Gestionar skills, instalar desde hubs y publicar las propias (`eduhub:<slug>` por defecto, consulta Ecosistema) |
| `edux memory show/clear` | Inspeccionar documentos de memoria L2/L3 o borrar memoria L1/toda |
| `edux session list/show/open/rename/delete` | Gestionar sesiones compartidas |
| `edux notebook list/create/show/add-md/replace-md/remove-record` | Gestionar cuadernos desde archivos Markdown |
| `edux book list/health/refresh-fingerprints` | Inspeccionar libros y actualizar huellas dactilares de fuentes |
| `edux plugin list/info` | Inspeccionar herramientas y capacidades registradas |
| `edux config show` | Imprimir resumen de configuración |
| `edux provider login <provider>` | Autenticación del proveedor (`openai-codex` OAuth login; `github-copilot` valida una sesión de autenticación Copilot existente) |
...
El paquete de solo CLI vive en `packaging/edux-cli`. En este checkout, instálalo desde fuente:
...
Las skills de EDUX usan el formato abierto **Agent-Skills** — una carpeta con una guía `SKILL.md` (frontmatter YAML + Markdown) y archivos de referencia opcionales. No hay nada específico de EDUX en ello, así que cualquier registro que hable el formato se convierte en una fuente para tu biblioteca. EDUX incluye **[EduHub](https://eduhub.eduorbit.com/)** — nuestro propio registro de skills enfocado en educación — conectado como hub predeterminado.

<details>
<summary><b>EduHub — el ecosistema de skills de EDUX</b></summary>

[**EduHub**](https://eduhub.eduorbit.com/) es el hub comunitario que EDUX lanzó para compartir skills de agentes orientadas a la enseñanza — tutores socráticos, constructores de tarjetas flash, retroalimentación de ensayos, planos de examen, explicadores de conceptos y más. Está integrado en EDUX, así que no hay nada que configurar: un slug simple o un prefijo `eduhub:` se resuelve a él.
...
```bash
edux skill search "socratic tutor"               # buscar en EduHub (el hub predeterminado)
edux skill install socratic-tutor                # fetch → verificar → registrar
edux skill install eduhub:socratic-tutor@1.2.0   # fijar un hub y una versión
edux skill list                                  # skills locales con su proveniencia del hub
```

**Publicar la tuya propia** — empaqueta un `SKILL.md` y compártelo con la comunidad:

```bash
edux skill login                                 # inicio de sesión en EduHub desde el navegador
edux skill publish ./my-skill                    # interactivo: elige una pista + etiquetas, luego sube
edux skill update                                # revertir o lanzar una nueva versión
```

EduHub también es un registro independiente compatible con ClawHub, así que los agentes que no son EDUX (Claude Code, Codex, …) pueden usarlo directamente a través del CLI `eduhub` — `npx eduhub install socratic-tutor`.
...
- el frontmatter se normaliza al esquema de EDUX y `always:` se **elimina**, de modo que una skill descargada nunca puede forzarse en cada prompt del sistema;
...
Como EDUX habla el formato abierto Agent-Skills, **[ClawHub](https://clawhub.ai/)** también funciona como fuente de primera clase — está integrado junto con EduHub. Selecciónalo con el prefijo del hub:
...
EDUX es un proyecto de código abierto liderado por [Bingxi Zhao](https://github.com/pancacake) dentro del grupo [EduOrbit](https://github.com/EduOrbit), y se itera en **forma completamente de código abierto**, construido junto con la comunidad. Hasta ahora, **NO** tenemos productos en línea de pago de ningún tipo. No dudes en contactarnos en **bingxizhao39@gmail.com** para discusiones, ideas o colaboración.

### 🙏 Agradecimientos

Un agradecimiento de corazón a [**Chao Huang**](https://sites.google.com/view/chaoh), director del Data Intelligence Lab @ HKU, y a nuestros compañeros de EduOrbit por su cálido apoyo — especialmente [**Jiahao Zhang**](https://github.com/zzhtx258), [**Zirui Guo**](https://github.com/LarFii) y [**Xubin Ren**](https://github.com/Re-bin). También estamos profundamente agradecidos a la **comunidad de código abierto**: tus estrellas, issues, pull requests y discusiones dan forma a EDUX todos los días.

EDUX también se apoya en los hombros de destacados proyectos de código abierto que nos dieron herramientas e inspiración:
...
| [**nanobot**](https://github.com/EduOrbit/nanobot) | Motor de agente ultraligero que impulsó el TutorBot original *(EduOrbit)* |
| [**LightRAG**](https://github.com/EduOrbit/LightRAG) | RAG simple y rápido *(EduOrbit)* |
| [**AutoAgent**](https://github.com/EduOrbit/AutoAgent) | Marco de agentes sin código *(EduOrbit)* |
| [**AI-Researcher**](https://github.com/EduOrbit/AI-Researcher) | Pipeline de investigación automatizada *(EduOrbit)* |
...
| [**Claude Code**](https://github.com/anthropics/claude-code) | CLI de codificación agéntica que inspiró el bucle de agentes de EDUX |
...
Queremos que EDUX siga iterando y mejorando — y en última instancia se convierta en un regalo que devolvamos a la comunidad de código abierto. Nuestro [**roadmap**](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/issues/498) se actualiza continuamente; vota en los elementos allí o propone nuevos. Si deseas contribuir, consulta la [**Guía de Contribución**](../../CONTRIBUTING.md) para la estrategia de ramas, estándares de código y cómo comenzar.

<div align="center">

Esperamos que EDUX se convierta en un regalo para la comunidad. 🎁

<a href="https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=Rohit-girish-Belagali/EDUX-EDUORBIT&max=999" alt="Contribuidores" />
</a>
...
<a href="https://www.star-history.com/#Rohit-girish-Belagali/EDUX-EDUORBIT&type=timeline&legend=top-left">
  <picture>
    ...
    <img alt="Gráfico de historial de estrellas" src="https://api.star-history.com/svg?repos=Rohit-girish-Belagali/EDUX-EDUORBIT&type=timeline&legend=top-left" />
  </picture>
</a>
...
<p align="center">
 <a href="https://www.star-history.com/eduorbit/edux">
  <picture>
   ...
   <img alt="Clasificación del historial de estrellas" src="https://api.star-history.com/badge?repo=Rohit-girish-Belagali/EDUX-EDUORBIT" />
  </picture>
 </a>
</p>
...
<p>
  <img src="https://visitor-badge.laobi.icu/badge?page_id=EduOrbit.EDUX&style=for-the-badge&color=00d4ff" alt="Vistas">
</p>
