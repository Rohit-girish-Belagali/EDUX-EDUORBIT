...
# EDUX: Tutoria Personalizada Vitalícia
...
[Recursos](#-recursos-principais) · [Começar](#-começar) · [Explorar](#-explorar-o-edux) · [CLI](#️-edux-cli--interface-nativa-de-agentes) · [Ecossistema](#-ecossistema--eduhub-e-a-comunidade-de-skills) · [Comunidade](#-comunidade)

</div>

---

> 🤝 **Damos as boas-vindas a todo tipo de contribuições!** Vote em itens do roadmap ou proponha novos em [`Roadmap`](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/issues/498), e consulte o nosso [Guia de Contribuição](../../CONTRIBUTING.md) para a estratégia de branches, padrões de código e como começar.

### 📰 Notícias

- **2026-05-22** 🌐 Site oficial de documentação disponível em [**eduorbit.com**](https://eduorbit.com/) — guias, referências e tours de capacidades num só lugar.
...
- **2026-04-10** 📄 O nosso artigo está agora no arXiv! Leia o [preprint](https://arxiv.org/abs/2604.26962) para saber mais sobre o design e as ideias por trás do EDUX.
- **2026-02-06** 🚀 Atingimos 10k estrelas em apenas 39 dias! Um enorme obrigado à nossa incrível comunidade pelo apoio!
- **2026-01-01** 🎊 Feliz Ano Novo! Junte-se ao nosso [Discord](https://discord.gg/eRsjPgMU4t), [WeChat](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/issues/78) ou [Discussions](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/discussions) — juntos damos forma ao futuro do EDUX!
- **2025-12-29** 🎓 EDUX está oficialmente lançado!

## ✨ Recursos Principais

O EDUX é um espaço de trabalho de aprendizagem nativo de agentes que conecta tutoria, resolução de problemas, geração de quizzes, pesquisa, visualização e prática de domínio num sistema extensível.
...
O EDUX inclui quatro caminhos de instalação. Todos partilham um layout de espaço de trabalho: as configurações vivem em `data/user/settings/` sob o diretório a partir do qual é iniciado (ou sob `EDUX_HOME` / `edux start --home` se definido explicitamente). Para a aplicação completa, o fluxo recomendado é **escolher um diretório de espaço de trabalho → instalar → `edux init` → `edux start`**.

<details>
<summary><b>Opção 1 — Instalar a partir do PyPI</b> · aplicação web local completa + CLI, sem necessidade de clonar</summary>

Aplicação web local completa + CLI, sem necessidade de clonar. Requer **Python 3.11+** e um runtime **Node.js 20+** no PATH (o servidor standalone Next.js empacotado é iniciado por `edux start`).

```bash
mkdir -p my-edux && cd my-edux
pip install -U edux
edux init     # solicita portas + provedor LLM + embedding opcional
edux start    # inicia backend + frontend; manter o terminal aberto
```

`edux init` solicita a porta de backend (predefinição `8001`), a porta de frontend (predefinição `3782`), provedor LLM / URL base / chave API / modelo e um provedor de embeddings opcional para Base de Conhecimento / RAG.

Após `edux start`, abra a URL do frontend impressa no terminal — por predefinição [http://127.0.0.1:3782](http://127.0.0.1:3782). Prima `Ctrl+C` nesse terminal para parar tanto o backend como o frontend. Omitir `edux init` é adequado para um teste rápido; a aplicação arranca com portas predefinidas e configuração de modelo vazia, configure-as depois em **Settings → Models**.
...
**Servidor de desenvolvimento bloqueado:** se `edux start` reportar um frontend existente que não responde, pare o PID que imprime. Se não houver nenhum processo Next.js em execução, os ficheiros de bloqueio estão desatualizados — remova-os e tente novamente:
...
- `ghcr.io/eduorbit/edux:latest` — lançamento estável
- `ghcr.io/eduorbit/edux:pre` — pré-lançamento, quando disponível
...
- **Desconectado:** adicione `-d`, depois `docker logs -f edux` para seguir, `docker stop edux` para parar, `docker rm edux` antes de reutilizar o nome. O volume `edux-data` mantém as suas configurações e espaço de trabalho entre reinicializações.
...
`next_public_api_base_external` (e o seu alias `public_api_base`) são aceites como fallbacks de menor precedência. CORS usa **origens** de frontend, não URLs de API. Com a autenticação desativada, o EDUX permite origens normais de navegador HTTP/HTTPS por predefinição. Com a autenticação ativada, adicione as origens exatas do frontend:
...
`edux init --cli` partilha o mesmo layout `data/user/settings/` que a aplicação completa mas omite as solicitações de portas de backend/frontend e define embeddings como **desativado** (escolha `Yes` se planear usar `edux kb …` ou ferramentas RAG). Ainda escreve um layout de runtime completo (`system.json`, `auth.json`, `integrations.json`, `model_catalog.json`, `main.yaml`, `agents.yaml`) e ainda solicita o provedor LLM e modelo ativos.

<details>
<summary><b>Comandos comuns</b></summary>

```bash
edux chat                                          # REPL interativo
...
A instalação local de `edux-cli` não inclui ativos web nem dependências de servidor. Mantenha o checkout de fonte por perto — a instalação editável aponta para ele. Para adicionar a aplicação web mais tarde, instale o pacote PyPI (Opção 1) e execute `edux init` + `edux start` a partir do mesmo espaço de trabalho.
...
- **docker-compose:** encaminhado em vez disso para um **sidecar runner** endurecido e com privilégios
  mínimos (`Dockerfile.runner`) via `EDUX_SANDBOX_RUNNER_URL` — a postura mais sólida,
...
`EDUX_SANDBOX_ALLOW_SUBPROCESS=0`) para desativar a execução do lado do host, à custa das
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/OVERVIEW.png" alt="EDUX home — o espaço de trabalho Chat com todas as superfícies na barra lateral" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/system%20architecture.png" alt="Arquitetura do sistema EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/home/00-overview.png" alt="Espaço de trabalho de chat EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/chat-agent-loop.png" alt="Loop de agente de chat EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/partners/00-partners%20overview.png" alt="Espaço de trabalho de partners EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/partners-architecture.png" alt="Arquitetura de partners EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/myagents/00-overview.png" alt="Espaço de trabalho Meus Agentes EDUX" width="900">
</div>

Meus Agentes transforma outros agentes em contexto para o EDUX, e faz duas coisas distintas. **Conectar um agente ao vivo** — um Claude Code ou Codex CLI na sua máquina, ou um dos seus Partners — e consultá-lo a partir de dentro de um turno de chat: o EDUX *executa* mesmo o outro agente e transmite o seu trabalho para o painel de Atividade via a ferramenta `consult_subagent`. Selecione-o com o chip de Agente (ou escreva `@`), e defina quantas rondas a consulta pode ter.
...
**Importar conversas anteriores** — traga o seu histórico existente do Claude Code e Codex como agentes nomeados, pesquisáveis e retomáveis. Escolha quais os dias a importar; atualizar ressincroniza-os. Referencie uma conversa importada a partir de qualquer turno de chat via `+` → Meus Agentes, e o EDUX lê-a como uma transcrição de terceiros — permanece *a conversa deles*, não a voz própria do EDUX.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/co-writer/00-overview.png" alt="Espaço de trabalho Co-Writer EDUX" width="900">
</div>
...
A sua ideia central é a **edição cirúrgica**: selecione um trecho e peça ao EDUX para reescrever, expandir ou encurtar. O agente de edição pode fundamentar a alteração numa base de conhecimento ou evidência web, mantém um rasto das suas chamadas de ferramentas e mostra cada alteração como um diff aceitar/rejeitar — pelo que nada aterra até que aprove.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/book/00-book_overview.png" alt="Biblioteca de livros EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/knowledge/00-overview.png" alt="Centro de Conhecimento EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/learning-space/00-overview.png" alt="Hub do Espaço de Aprendizado EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/memory/00-overview.png" alt="Visão geral da memória EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/memory/01-3%20layer%20memory%20graph.png" alt="Gráfico de memória de três camadas EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/settings/00-setting%20overview.png" alt="Hub de configurações EDUX" width="900">
</div>
...
A autenticação está **desativada por predefinição** — o EDUX corre em modo de utilizador único. Ative-a e uma árvore `data/` aloja um espaço de trabalho de administrador, espaços de trabalho por utilizador isolados e espaços de trabalho de partners lado a lado:
...
**Ativar:** ligue a autenticação em `data/user/settings/auth.json`, reinicie `edux start`, registe o primeiro administrador em `/register`, depois adicione utilizadores em `/admin/users` e atribua modelos, KBs, skills, Partners, política de ferramentas/MCP e acesso de execução de código através de concessões.
...
## ⌨️ EDUX CLI — Interface Nativa de Agentes

Um binário `edux`, duas formas de entrada: um **REPL** interativo para pessoas que vivem no terminal, e **JSON** estruturado para outros agentes que conduzem o EDUX como uma ferramenta. As mesmas capacidades, ferramentas e bases de conhecimento de qualquer forma.

<details>
<summary><b>Conduzir você mesmo</b></summary>

`edux chat` abre um REPL interativo; `edux run <capability> "<message>"` dispara um único turno e sai. Ambos usam os mesmos flags `--capability`, `--tool`, `--kb` e `--config`.
...
O EDUX foi construído para ser *operado por outro agente*. Adicione `--format json` a qualquer `run` e cada turno transmite **NDJSON — um evento por linha** (`content`, `tool_call`, `tool_result`, `done`, …), cada linha marcada com o seu `session_id`. As execuções são seguras sem TTY: uma pausa `ask_user` sem TTY resolve-se automaticamente com uma resposta vazia em vez de bloquear.
...
O repositório inclui um [`SKILL.md`](../../SKILL.md) raiz — um documento de transferência de ~150 linhas que ensina qualquer LLM com uso de ferramentas toda a superfície numa leitura. Passe-o ao Claude Code, Codex ou OpenCode (eles pegam no `SKILL.md` automaticamente), ou envolva `edux run` como uma ferramenta num loop LangChain / AutoGen. Receitas completas: [Agent Handoff](https://eduorbit.com/docs/cli/agent-handoff/).
...
| Comando | Descrição |
|:---|:---|
| `edux init` | Criar ou atualizar `data/user/settings` para o espaço de trabalho atual |
| `edux start [--home PATH]` | Lançar backend + frontend juntos |
| `edux serve [--port PORT]` | Iniciar apenas o backend FastAPI |
| `edux run <capability> <message>` | Executar um turno de capacidade único (`chat`, `deep_solve`, `deep_question`, `deep_research`, `visualize`, `math_animator`, `mastery_path`); adicionar `--format json` para saída NDJSON |
| `edux chat` | REPL interativo com controlos de capacidade, ferramenta, KB, notebook e histórico |
| `edux partner list/create/start/stop` | Gerir partners ligados por IM |
| `edux kb list/info/create/add/search/set-default/delete` | Gerir bases de conhecimento LlamaIndex |
| `edux skill search/install/list/remove/login/logout/publish/update` | Gerir habilidades, instalar de hubs e publicar as suas (`eduhub:<slug>` por padrão, veja Ecossistema) |
| `edux memory show/clear` | Inspecionar documentos de memória L2/L3 ou limpar memória L1/toda |
| `edux session list/show/open/rename/delete` | Gerir sessões partilhadas |
| `edux notebook list/create/show/add-md/replace-md/remove-record` | Gerir notebooks a partir de ficheiros Markdown |
| `edux book list/health/refresh-fingerprints` | Inspecionar livros e atualizar impressões digitais de fontes |
| `edux plugin list/info` | Inspecionar ferramentas e capacidades registadas |
| `edux config show` | Imprimir resumo de configuração |
| `edux provider login <provider>` | Autenticação de provedor (`openai-codex` login OAuth; `github-copilot` valida uma sessão de autenticação Copilot existente) |
...
O pacote de apenas CLI encontra-se em `packaging/edux-cli`. Neste checkout, instale-o a partir de fonte:
...
As skills do EDUX usam o formato aberto **Agent-Skills** — uma pasta com um playbook `SKILL.md` (frontmatter YAML + Markdown) e ficheiros de referência opcionais. Nada nisto é específico do EDUX, por isso qualquer registo que fale o formato torna-se uma fonte para a sua biblioteca. O EDUX inclui o **[EduHub](https://eduhub.eduorbit.com/)** — o nosso próprio registo de skills focado em educação — como hub padrão.

<details>
<summary><b>EduHub — o ecossistema de habilidades do EDUX</b></summary>

[**EduHub**](https://eduhub.eduorbit.com/) é o hub da comunidade que o EDUX lançou para partilhar skills de agentes orientadas para o ensino — tutores socráticos, construtores de cartões flash, feedback de redações, planos de exames, explicadores de conceitos e muito mais. Está integrado no EDUX, por isso não há nada a configurar: um slug simples ou um prefixo `eduhub:` resolve para ele.
...
```bash
edux skill search "socratic tutor"               # pesquisar EduHub (o hub padrão)
edux skill install socratic-tutor                # buscar → verificar → registar
edux skill install eduhub:socratic-tutor@1.2.0   # fixar um hub e uma versão
edux skill list                                  # skills locais com a sua proveniência de hub
```

**Publicar a sua própria** — empacote um `SKILL.md` e partilhe-o de volta com a comunidade:

```bash
edux skill login                                 # login no navegador para o EduHub
edux skill publish ./my-skill                    # interativo: escolher uma faixa + etiquetas, depois carregar
edux skill update                                # reverter ou lançar uma nova versão
```

O EduHub também é um registo independente compatível com ClawHub, por isso agentes que não são o EDUX (Claude Code, Codex, …) podem usá-lo diretamente através da CLI `eduhub` — `npx eduhub install socratic-tutor`.
...
- o frontmatter é normalizado para o esquema do EDUX e `always:` é **removido**, por isso uma skill descarregada nunca pode forçar-se em cada prompt do sistema;
...
Como o EDUX fala o formato aberto Agent-Skills, o **[ClawHub](https://clawhub.ai/)** também funciona como fonte de primeira classe — está integrado juntamente com o EduHub. Escolha-o com o prefixo de hub:
...
O EDUX é um projeto de código aberto liderado por [Bingxi Zhao](https://github.com/pancacake) dentro do Grupo [EduOrbit](https://github.com/EduOrbit), e itera numa **forma totalmente de código aberto**, construído em conjunto com a comunidade. Até agora, **NÃO** temos produtos online pagos de qualquer forma. Sinta-se à vontade para contactar em **bingxizhao39@gmail.com** para discussões, ideias ou colaboração.

### 🙏 Agradecimentos

Um agradecimento sincero a [**Chao Huang**](https://sites.google.com/view/chaoh), diretor do Data Intelligence Lab @ HKU, e aos nossos colegas do EduOrbit pelo seu caloroso apoio — especialmente [**Jiahao Zhang**](https://github.com/zzhtx258), [**Zirui Guo**](https://github.com/LarFii) e [**Xubin Ren**](https://github.com/Re-bin). Também somos profundamente gratos à **comunidade de código aberto**: as suas estrelas, issues, pull requests e discussões moldam o EDUX todos os dias.

O EDUX também assenta nos ombros de projetos de código aberto excepcionais que nos deram tanto ferramentas como inspiração:
...
| [**nanobot**](https://github.com/EduOrbit/nanobot) | Motor de agente ultraligeiro que alimentou o TutorBot original *(EduOrbit)* |
| [**LightRAG**](https://github.com/EduOrbit/LightRAG) | RAG Simples e Rápido *(EduOrbit)* |
| [**AutoAgent**](https://github.com/EduOrbit/AutoAgent) | Framework de Agentes Sem Código *(EduOrbit)* |
| [**AI-Researcher**](https://github.com/EduOrbit/AI-Researcher) | Pipeline de investigação automatizada *(EduOrbit)* |
...
| [**Claude Code**](https://github.com/anthropics/claude-code) | CLI de programação agêntica que inspirou o loop de agente do EDUX |
...
Queremos que o EDUX continue a iterar e a melhorar — e, em última análise, a tornar-se um presente que devolvemos à comunidade de código aberto. O nosso [**roadmap**](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/issues/498) é atualizado continuamente; vote em itens lá ou proponha novos. Se quiser contribuir, veja o [**Guia de Contribuição**](../../CONTRIBUTING.md) para a estratégia de branches, padrões de código e como começar.

<div align="center">

Esperamos que o EDUX se torne um presente para a comunidade. 🎁
...
<a href="https://www.star-history.com/#Rohit-girish-Belagali/EDUX-EDUORBIT&type=timeline&legend=top-left">
  <picture>
    ...
    <img alt="Gráfico de histórico de estrelas" src="https://api.star-history.com/svg?repos=Rohit-girish-Belagali/EDUX-EDUORBIT&type=timeline&legend=top-left" />
  </picture>
</a>
...
<p align="center">
 <a href="https://www.star-history.com/eduorbit/edux">
  <picture>
   ...
   <img alt="Classificação do histórico de estrelas" src="https://api.star-history.com/badge?repo=Rohit-girish-Belagali/EDUX-EDUORBIT" />
  </picture>
 </a>
</p>
...
<p>
  <img src="https://visitor-badge.laobi.icu/badge?page_id=EduOrbit.EDUX&style=for-the-badge&color=00d4ff" alt="Visualizações">
</p>
