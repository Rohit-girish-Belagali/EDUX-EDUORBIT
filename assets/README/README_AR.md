...
<p align="center"><img src="../../assets/figs/logo/logo.png" alt="شعار EDUX" height="56" style="vertical-align: middle;">&nbsp;<img src="../../assets/figs/logo/banner.png" alt="EDUX" height="48" style="vertical-align: middle;"></p>

# EDUX: تدريس شخصي مدى الحياة

<p align="center">
  <a href="https://eduorbit.com" target="_blank"><img alt="الوثائق — eduorbit.com" src="https://img.shields.io/badge/Docs-eduorbit.com%20%E2%86%97-0A0A0A?style=for-the-badge&labelColor=F5F5F4" height="36"></a>
</p>
...
[الميزات](#-الميزات-الرئيسية) · [البدء](#-البدء) · [الاستكشاف](#-استكشاف-edux) · [واجهة CLI](#️-واجهة-سطر-أوامر-edux--الواجهة-الأصيلة-للوكلاء) · [النظام البيئي](#-النظام-البيئي--eduhub-ومجتمع-المهارات) · [المجتمع](#-المجتمع)

</div>

---

> 🤝 **نرحب بجميع أنواع المساهمات!** صوّت على عناصر خارطة الطريق أو اقترح عناصر جديدة في [`خارطة الطريق`](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/issues/498)، وراجع [دليل المساهمة](../../CONTRIBUTING.md) لمعرفة استراتيجية الفروع ومعايير البرمجة وكيفية البدء.

### 📰 الأخبار

- **2026-05-22** 🌐 موقع الوثائق الرسمي متاح على [**eduorbit.com**](https://eduorbit.com/) — الأدلة والمراجع وجولات القدرات كلها في مكان واحد.
- **2026-04-19** 🎉 20 ألف نجمة في 111 يومًا! شكراً على الدعم نحو تدريس شخصي وذكي حقيقي.
- **2026-04-10** 📄 ورقتنا البحثية متاحة على arXiv — اقرأ [النسخة الأولية](https://arxiv.org/abs/2604.26962) للتعرف على التصميم والأفكار وراء EDUX.
- **2026-02-06** 🚀 10 آلاف نجمة في 39 يومًا فقط! شكر جزيل لمجتمعنا الرائع.
- **2026-01-01** 🎊 كل عام وأنتم بخير! انضم إلى [Discord](https://discord.gg/eRsjPgMU4t) أو [WeChat](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/issues/78) أو [النقاشات](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/discussions) — لنشكّل معاً مستقبل EDUX.
- **2025-12-29** 🎓 تم إطلاق EDUX رسميًا!

## ✨ الميزات الرئيسية

EDUX هو بيئة تعلم أصيلة للوكلاء تربط التدريس وحل المشكلات وتوليد الاختبارات والبحث والتصور وممارسة الإتقان في نظام واحد قابل للتوسيع.
...
يأتي EDUX بأربعة مسارات تثبيت. وكلها تشترك في تخطيط مساحة عمل واحد: تعيش الإعدادات في `data/user/settings/` تحت الدليل الذي تُطلق منه التطبيق (أو تحت `EDUX_HOME` / `edux start --home` إذا حددت واحداً صراحةً). للتطبيق الكامل، التدفق الموصى به هو **اختر دليل مساحة عمل → تثبيت → `edux init` → `edux start`**.

<details>
<summary><b>الخيار 1 — التثبيت من PyPI</b> · تطبيق ويب محلي كامل + CLI، لا يلزم الاستنساخ</summary>

تطبيق ويب محلي كامل + CLI، لا يلزم الاستنساخ. يحتاج **Python 3.11+** وبيئة تشغيل **Node.js 20+** في PATH (يُشغَّل خادم Next.js المستقل المُحزَّم بواسطة `edux start`).

```bash
mkdir -p my-edux && cd my-edux
pip install -U edux
edux init     # يطلب المنافذ + مزود LLM + تضمين اختياري
edux start    # يشغّل الخلفية + الواجهة الأمامية؛ ابقِ الطرفية مفتوحة
```

يطلب `edux init` منفذ الخلفية (افتراضي `8001`)، ومنفذ الواجهة الأمامية (افتراضي `3782`)، ومزود LLM / عنوان URL الأساسي / مفتاح API / النموذج، ومزود تضمين اختياري لقاعدة المعرفة / RAG.

بعد `edux start`، افتح عنوان URL للواجهة الأمامية المطبوع في الطرفية — افتراضياً [http://127.0.0.1:3782](http://127.0.0.1:3782). اضغط `Ctrl+C` في تلك الطرفية لإيقاف الخلفية والواجهة الأمامية معاً. تخطي `edux init` لا بأس به للتجربة السريعة؛ يُقلع التطبيق بالمنافذ الافتراضية وإعدادات نموذج فارغة، قم بتهيئتها لاحقاً في **الإعدادات ← النماذج**.
...
**خادم تطوير متوقف:** إذا أبلغ `edux start` عن واجهة أمامية موجودة لا تستجيب، أوقف الـ PID الذي يطبعه. إذا لم يكن هناك أي عملية Next.js تعمل فعلياً، فملفات القفل قديمة — احذفها وأعد المحاولة:
...
- `ghcr.io/eduorbit/edux:latest` — إصدار مستقر
- `ghcr.io/eduorbit/edux:pre` — إصدار تجريبي، عند توفره
...
افتح [http://127.0.0.1:3782](http://127.0.0.1:3782). تُنشئ الحاوية `/app/data/user/settings/*.json` عند الإقلاع الأول؛ قم بتهيئة مزودي النماذج من صفحة إعدادات الويب. تبقى التهيئة ومفاتيح API والسجلات وملفات مساحة العمل والذاكرة وقواعد المعرفة في وحدة تخزين `edux-data`.
...
- **وضع المنفصل:** أضف `-d`، ثم `docker logs -f edux` للمتابعة، و`docker stop edux` للإيقاف، و`docker rm edux` قبل إعادة استخدام الاسم. تحتفظ وحدة تخزين `edux-data` بإعداداتك ومساحة عملك عبر إعادات التشغيل.
...
يُقبَل `next_public_api_base_external` (واسمه المستعار `public_api_base`) كبدائل ذات أولوية أقل. يستخدم CORS **منشآت** الواجهة الأمامية، وليس عناوين URL لواجهة برمجة التطبيقات. مع تعطيل المصادقة، يسمح EDUX بمنشآت متصفح HTTP/HTTPS العادية افتراضياً. مع تفعيل المصادقة، أضف منشآت الواجهة الأمامية الدقيقة:
...
لا يشحن تثبيت `edux-cli` المحلي بأصول الويب أو تبعيات الخادم. احتفظ بالنسخة المسحوبة من المصدر — يشير التثبيت القابل للتحرير إليها. لإضافة تطبيق الويب لاحقاً، ثبّت حزمة PyPI (الخيار 1) وشغّل `edux init` + `edux start` من نفس مساحة العمل.
...
- **docker-compose:** يُوجَّه بدلاً من ذلك إلى **مرافق runner** محصَّن وأقل امتيازاً (`Dockerfile.runner`) عبر `EDUX_SANDBOX_RUNNER_URL` — أقوى وضع، ويُفضَّل تلقائياً عند توفره.

يتحكم إعداد `sandbox_allow_subprocess` في `data/user/settings/system.json` (افتراضي `true`) في صندوق أمان العمليات الفرعية. تشغيل الرمز المُولَّد بواسطة النموذج على مضيفك هو قرار ثقة حقيقي — اضبطه على `false` (أو صدّر `EDUX_SANDBOX_ALLOW_SUBPROCESS=0`) لتعطيل التنفيذ على جانب المضيف، على حساب عدم قدرة مهارات المكتب على إنتاج الملفات بعد الآن.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/OVERVIEW.png" alt="الصفحة الرئيسية لـ EDUX — مساحة عمل Chat مع كل الأسطح في الشريط الجانبي" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/system%20architecture.png" alt="معمارية نظام EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/home/00-overview.png" alt="مساحة عمل محادثة EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/chat-agent-loop.png" alt="حلقة وكيل محادثة EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/partners/00-partners%20overview.png" alt="مساحة عمل شركاء EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/partners-architecture.png" alt="معمارية شركاء EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/myagents/00-overview.png" alt="مساحة عمل My Agents في EDUX" width="900">
</div>

يحوّل My Agents الوكلاء الآخرين إلى سياق لـ EDUX، ويقوم بشيئين متمايزين. **توصيل وكيل مباشر** — Claude Code أو Codex CLI على جهازك، أو أحد شركائك — واستشارته من داخل دور محادثة: EDUX *يشغّل* الوكيل الآخر فعلاً ويبث عمله إلى لوحة Activity عبر أداة `consult_subagent`. اختره بشريحة الوكيل (أو اكتب `@`)، وحدد عدد الجولات التي يمكن للاستشارة أن تأخذها.
...
**استيراد المحادثات السابقة** — أحضر تاريخ Claude Code وCodex الموجود لديك كوكلاء مسماة قابلة للبحث والاستئناف. اختر الأيام التي تريد استيرادها؛ التحديث يعيد مزامنتها. استشر محادثة مستوردة من أي دور محادثة عبر `+` ← My Agents، وسيقرأها EDUX كنصوص محادثة لطرف ثالث — تبقى *محادثتهم*، وليس صوت EDUX الخاص.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/co-writer/00-overview.png" alt="مساحة عمل Co-Writer في EDUX" width="900">
</div>
...
الفكرة المحورية هي **التحرير الجراحي**: حدد نطاقاً واطلب من EDUX إعادة كتابته أو توسيعه أو تقصيره. يمكن لوكيل التحرير ترسيخ التغيير في قاعدة معرفة أو دليل ويب، ويحتفظ بآثار استدعاءات أدواته، ويعرض كل تغيير كـ diff قبول/رفض — لذا لا شيء يُقبَل حتى توافق عليه.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/book/00-book_overview.png" alt="مكتبة كتب EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/knowledge/00-overview.png" alt="مركز المعرفة في EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/learning-space/00-overview.png" alt="مركز فضاء التعلم في EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/memory/00-overview.png" alt="نظرة عامة على ذاكرة EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/memory/01-3%20layer%20memory%20graph.png" alt="رسم بياني لذاكرة EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/settings/00-setting%20overview.png" alt="مركز إعدادات EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/settings/01-appearance%20settings.png" alt="إعدادات المظهر والثيمات في EDUX" width="900">
</div>

تستخدم معظم الأقسام تدفق صياغة-وتطبيق، لذا يمكنك اختبار مزود قبل الالتزام به. تشحن أربع ثيمات في الصندوق — Default وCream وDark وGlass. ملفات `.env` في جذر المشروع تُتجاهل عمداً؛ يعيش تهيئة وقت التشغيل تحت `data/user/settings/*.json` إلا إذا وجّه `EDUX_HOME` أو `edux start --home` التطبيق في مكان آخر.
...
المصادقة **معطلة افتراضياً** — يعمل EDUX لمستخدم واحد. فعّلها وشجرة `data/` واحدة تستضيف مساحة عمل المشرف ومساحات عمل معزولة لكل مستخدم ومساحات عمل الشريك جنباً إلى جنب:
...
**تفعيله:** فعّل المصادقة في `data/user/settings/auth.json`، وأعد تشغيل `edux start`، وسجّل أول مشرف على `/register`، ثم أضف المستخدمين من `/admin/users` وعيّن النماذج وقواعد المعرفة والمهارات والـ Partners وسياسة الأداة/MCP ووصول تنفيذ الرمز من خلال المنح.
...
## ⌨️ واجهة سطر أوامر EDUX — الواجهة الأصيلة للوكلاء

ثنائي `edux` واحد، طريقتان للدخول: **REPL** تفاعلي للأشخاص الذين يعيشون في الطرفية، و**JSON** منظَّم للوكلاء الأخرى التي تقود EDUX كأداة. نفس القدرات والأدوات وقواعد المعرفة في كلتا الحالتين.

<details>
<summary><b>قدّها بنفسك</b></summary>

`edux chat` يفتح REPL تفاعلياً؛ `edux run <capability> "<message>"` يُشغّل دوراً واحداً ويخرج. يتحدث كلاهما بنفس علامات `--capability` و`--tool` و`--kb` و`--config`.
...
EDUX مبنية لتكون *مُشغَّلة بواسطة وكيل آخر*. أضف `--format json` إلى أي `run` وكل دور يبث **NDJSON — حدث واحد في كل سطر** (`content` و`tool_call` و`tool_result` و`done` و...)، وكل سطر مُعنوَن بـ `session_id` الخاص به. التشغيلات آمنة بدون TTY: توقف `ask_user` بدون TTY يحل تلقائياً برد فارغ بدلاً من التعليق.
...
يشحن المستودع بملف [`SKILL.md`](../../SKILL.md) في الجذر — وثيقة تسليم بنحو 150 سطراً تعلّم أي LLM يستخدم الأدوات السطح بأكمله في قراءة واحدة. سلّمها إلى Claude Code أو Codex أو OpenCode (يلتقطون `SKILL.md` تلقائياً)، أو لفّ `edux run` كأداة في حلقة LangChain / AutoGen. الوصفات الكاملة: [Agent Handoff](https://eduorbit.com/docs/cli/agent-handoff/).
...
| الأمر | الوصف |
|:---|:---|
| `edux init` | إنشاء أو تحديث `data/user/settings` لمساحة العمل الحالية |
| `edux start [--home PATH]` | تشغيل الخلفية + الواجهة الأمامية معاً |
| `edux serve [--port PORT]` | تشغيل خلفية FastAPI فقط |
| `edux run <capability> <message>` | تشغيل دور قدرة واحدة (`chat` و`deep_solve` و`deep_question` و`deep_research` و`visualize` و`math_animator` و`mastery_path`)؛ أضف `--format json` لإخراج NDJSON |
| `edux chat` | REPL تفاعلي مع تحكمات القدرة والأداة وقاعدة المعرفة ودفتر الملاحظات والتاريخ |
| `edux partner list/create/start/stop` | إدارة الشركاء المتصلين بـ IM |
| `edux kb list/info/create/add/search/set-default/delete` | إدارة قواعد المعرفة LlamaIndex |
| `edux skill search/install/list/remove/login/logout/publish/update` | إدارة المهارات، التثبيت من المراكز، ونشر مهاراتك الخاصة (`eduhub:<slug>` افتراضياً، راجع النظام البيئي) |
| `edux memory show/clear` | فحص مستندات الذاكرة L2/L3 أو مسح ذاكرة L1/الكل |
| `edux session list/show/open/rename/delete` | إدارة الجلسات المشتركة |
| `edux notebook list/create/show/add-md/replace-md/remove-record` | إدارة دفاتر الملاحظات من ملفات Markdown |
| `edux book list/health/refresh-fingerprints` | فحص الكتب وتحديث بصمات المصادر |
| `edux plugin list/info` | فحص الأدوات والقدرات المسجلة |
| `edux config show` | طباعة ملخص التهيئة |
| `edux provider login <provider>` | مصادقة المزود (`openai-codex` OAuth login؛ `github-copilot` يتحقق من جلسة مصادقة Copilot موجودة) |
...
حزمة CLI فقط تعيش في `packaging/edux-cli`. في هذه النسخة، ثبّتها من المصدر:
...
تستخدم مهارات EDUX تنسيق **Agent-Skills** المفتوح — مجلد يحتوي دليل `SKILL.md` (YAML frontmatter + Markdown) وملفات مرجعية اختيارية. لا شيء في ذلك خاص بـ EDUX، لذا أي سجل يتحدث التنسيق يصبح مصدراً لمكتبتك. يشحن EDUX مع **[EduHub](https://eduhub.eduorbit.com/)** — سجل المهارات المركّز على التعليم الخاص بنا — مُوصَّلاً كمركز افتراضي.

<details>
<summary><b>EduHub — النظام البيئي للمهارات في EDUX</b></summary>

[**EduHub**](https://eduhub.eduorbit.com/) هو المركز المجتمعي الذي أطلقه EDUX لمشاركة مهارات الوكيل الموجهة نحو التعليم — موجهو سقراط وبناة بطاقات الفلاش وتغذية راجعة للمقالات وخطط الامتحانات وشارحو المفاهيم وغيرها. مُدمَج في EDUX، لذا لا شيء يجب تهيئته: slug مجرد أو بادئة `eduhub:` تحل إليه.
...
```bash
edux skill search "socratic tutor"               # البحث في EduHub (المركز الافتراضي)
edux skill install socratic-tutor                # جلب → التحقق → التسجيل
edux skill install eduhub:socratic-tutor@1.2.0   # تثبيت مركز وإصدار محددين
edux skill list                                  # المهارات المحلية مع مصدرها من المركز
```

**انشر مهارتك الخاصة** — حزّم `SKILL.md` وشاركها مع المجتمع:

```bash
edux skill login                                 # تسجيل دخول المتصفح إلى EduHub
edux skill publish ./my-skill                    # تفاعلي: اختر track + tags، ثم رفع
edux skill update                                # تراجع أو أصدر إصداراً جديداً
```

EduHub هو أيضاً سجل مستقل متوافق مع ClawHub، لذا الوكلاء الذين ليسوا EDUX (Claude Code وCodex و...) يمكنهم استخدامه مباشرةً من خلال واجهة `eduhub` — `npx eduhub install socratic-tutor`.
...
- تُعيَّر الـ frontmatter إلى مخطط EDUX و`always:` **تُزال**، لذا مهارة محملة لا يمكنها أبداً إجبار نفسها في كل مطالبة نظام؛
...
لأن EDUX يتحدث تنسيق Agent-Skills المفتوح، **[ClawHub](https://clawhub.ai/)** يعمل كمصدر من الدرجة الأولى أيضاً — مُدمَج جنباً إلى جنب مع EduHub. اختره ببادئة المركز:
...
EDUX هو مشروع مفتوح المصدر تقوده [Bingxi Zhao](https://github.com/pancacake) ضمن مجموعة [EduOrbit](https://github.com/EduOrbit)، ويتطور بشكل **مفتوح المصدر بالكامل**، مبني مع المجتمع. حتى الآن، **لا يوجد** لدينا أي منتجات مدفوعة عبر الإنترنت من أي شكل. تفضّل بالتواصل على **bingxizhao39@gmail.com** للنقاشات والأفكار والتعاون.

### 🙏 التقدير

خالص الشكر لـ [**Chao Huang**](https://sites.google.com/view/chaoh)، مدير مختبر ذكاء البيانات @ HKU، ولزملائنا في EduOrbit على دعمهم الحار — وخاصةً [**Jiahao Zhang**](https://github.com/zzhtx258)، و[**Zirui Guo**](https://github.com/LarFii)، و[**Xubin Ren**](https://github.com/Re-bin). ونحن ممتنون عميقاً أيضاً لـ **مجتمع المصادر المفتوحة**: نجومكم وإصداراتكم وطلبات السحب والنقاشات تشكّل EDUX كل يوم.

يقف EDUX أيضاً على أكتاف مشاريع مفتوحة المصدر متميزة أعطتنا أدوات وإلهاماً:

| المشروع | الدور / الإلهام |
|:---|:---|
| [**LlamaIndex**](https://github.com/run-llama/llama_index) | العمود الفقري لخط أنابيب RAG وفهرسة المستندات |
| [**nanobot**](https://github.com/EduOrbit/nanobot) | محرك وكيل خفيف الوزن للغاية مكّن TutorBot الأصلي *(EduOrbit)* |
| [**LightRAG**](https://github.com/EduOrbit/LightRAG) | RAG بسيط وسريع *(EduOrbit)* |
| [**AutoAgent**](https://github.com/EduOrbit/AutoAgent) | إطار وكيل بدون رمز *(EduOrbit)* |
| [**AI-Researcher**](https://github.com/EduOrbit/AI-Researcher) | خط أنابيب بحث آلي *(EduOrbit)* |
...
| [**Claude Code**](https://github.com/anthropics/claude-code) | واجهة برمجة للوكلاء ألهمت حلقة وكيل EDUX |
...
نريد لـ EDUX أن يستمر في التطور والتحسين — وفي نهاية المطاف أن يصبح هدية نقدمها للمجتمع مفتوح المصدر. يُحدَّث [**خارطة طريقنا**](https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/issues/498) باستمرار؛ صوّت على العناصر هناك أو اقترح عناصر جديدة. إذا كنت ترغب في المساهمة، راجع [**دليل المساهمة**](../../CONTRIBUTING.md) لمعرفة استراتيجية الفروع ومعايير البرمجة وكيفية البدء.

<div align="center">

نأمل أن يصبح EDUX هدية للمجتمع. 🎁

<a href="https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=Rohit-girish-Belagali/EDUX-EDUORBIT&max=999" alt="المساهمون" />
</a>
...
<a href="https://www.star-history.com/#Rohit-girish-Belagali/EDUX-EDUORBIT&type=timeline&legend=top-left">
  <picture>
    ...
    <img alt="مخطط تاريخ النجوم" src="https://api.star-history.com/svg?repos=Rohit-girish-Belagali/EDUX-EDUORBIT&type=timeline&legend=top-left" />
  </picture>
</a>
...
<p align="center">
 <a href="https://www.star-history.com/eduorbit/edux">
  <picture>
   ...
   <img alt="ترتيب تاريخ النجوم" src="https://api.star-history.com/badge?repo=Rohit-girish-Belagali/EDUX-EDUORBIT" />
  </picture>
 </a>
</p>
...
<p>
  <img src="https://visitor-badge.laobi.icu/badge?page_id=EduOrbit.EDUX&style=for-the-badge&color=00d4ff" alt="المشاهدات">
</p>
