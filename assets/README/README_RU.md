...
# EDUX: Персонализированное Обучение на Базе Агентов
...
- **2026-05-22** 🌐 Официальный сайт документации запущен на [**eduorbit.com**](https://eduorbit.com/) — руководства, справочники и туры по возможностям в одном месте.
...
EDUX — это агентная учебная рабочая среда, объединяющая репетиторство, решение задач, генерацию викторин, исследования, визуализацию и практику освоения в одной расширяемой системе.
...
EDUX поставляется с четырьмя путями установки. Все они используют одну структуру рабочего пространства: настройки хранятся в `data/user/settings/` в директории запуска (или в `EDUX_HOME` / `edux start --home`, если задано явно). Для полного приложения рекомендуемый процесс: **выбрать директорию рабочего пространства → установить → `edux init` → `edux start`**.

<details>
<summary><b>Вариант 1 — Установка из PyPI</b> · полное локальное веб-приложение + CLI, клонирование не требуется</summary>

Полное локальное веб-приложение + CLI без необходимости клонирования. Требуется **Python 3.11+** и среда выполнения **Node.js 20+** в PATH (упакованный автономный сервер Next.js запускается командой `edux start`).

```bash
mkdir -p my-edux && cd my-edux
pip install -U edux
edux init     # запрашивает порты + провайдер LLM + необязательное встраивание
edux start    # запускает бэкенд + фронтенд; держите терминал открытым
```

`edux init` запрашивает порт бэкенда (по умолчанию `8001`), порт фронтенда (по умолчанию `3782`), провайдер LLM / базовый URL / API-ключ / модель и необязательный провайдер встраивания для базы знаний / RAG.

После `edux start` откройте URL фронтенда, напечатанный в терминале — по умолчанию [http://127.0.0.1:3782](http://127.0.0.1:3782). Нажмите `Ctrl+C` в этом терминале, чтобы остановить бэкенд и фронтенд. Пропустить `edux init` можно для быстрого пробного запуска; приложение загружается с портами по умолчанию и пустыми настройками модели, которые можно настроить позже в **Настройки → Модели**.
...
**Зависший сервер разработки:** если `edux start` сообщает о существующем фронтенде, который не отвечает, остановите PID, который он печатает. Если процесс Next.js фактически не запущен, файлы блокировки устарели — удалите их и повторите попытку:
...
- `ghcr.io/eduorbit/edux:latest` — стабильный выпуск
- `ghcr.io/eduorbit/edux:pre` — предварительный выпуск, когда доступен
...
Откройте [http://127.0.0.1:3782](http://127.0.0.1:3782). Контейнер создаёт `/app/data/user/settings/*.json` при первом запуске; настройте провайдеров моделей на странице веб-настроек. Конфигурация, API-ключи, логи, файлы рабочего пространства, память и базы знаний сохраняются в томе `edux-data`.
...
- **Фоновый режим:** добавьте `-d`, затем `docker logs -f edux` для слежения, `docker stop edux` для остановки, `docker rm edux` перед повторным использованием имени. Том `edux-data` сохраняет ваши настройки и рабочее пространство между перезапусками.
...
`next_public_api_base_external` (и его псевдоним `public_api_base`) принимаются как запасные варианты с более низким приоритетом. CORS использует **источники** фронтенда, а не URL API. При отключённой аутентификации EDUX разрешает обычные HTTP/HTTPS источники браузера по умолчанию. При включённой аутентификации добавьте точные источники фронтенда:
...
Локальная установка `edux-cli` не включает веб-ресурсы или серверные зависимости. Сохраните чекаут исходного кода — редактируемая установка указывает на него. Чтобы позже добавить веб-приложение, установите пакет PyPI (Вариант 1) и запустите `edux init` + `edux start` из того же рабочего пространства.
...
- **docker-compose:** вместо этого направляется в защищённый сайдкар с минимальными привилегиями (**runner sidecar** `Dockerfile.runner`) через `EDUX_SANDBOX_RUNNER_URL` — наиболее строгий вариант, используемый автоматически при наличии.

Подпроцессная песочница управляется настройкой `sandbox_allow_subprocess` в `data/user/settings/system.json` (по умолчанию `true`). Запуск сгенерированного моделью кода на вашем хосте — это реальное доверительное решение; установите значение `false` (или экспортируйте `EDUX_SANDBOX_ALLOW_SUBPROCESS=0`) для отключения выполнения на стороне хоста, ценой того, что офисные навыки больше не смогут создавать файлы.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/OVERVIEW.png" alt="Главная страница EDUX — рабочее пространство чата со всеми поверхностями на боковой панели" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/system%20architecture.png" alt="Архитектура системы EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/home/00-overview.png" alt="Рабочее пространство чата EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/chat-agent-loop.png" alt="Цикл агента чата EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/partners/00-partners%20overview.png" alt="Рабочее пространство партнёров EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/system/partners-architecture.png" alt="Архитектура партнёров EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/myagents/00-overview.png" alt="Рабочее пространство «Мои Агенты» EDUX" width="900">
</div>

Мои Агенты превращают других агентов в контекст для EDUX и делают две разные вещи. **Подключите живого агента** — Claude Code или Codex CLI на вашей машине, или одного из ваших Партнёров — и обращайтесь к нему изнутри хода чата: EDUX фактически *запускает* другого агента и транслирует его работу в панель Activity через инструмент `consult_subagent`. Выберите его чипом агента (или введите `@`) и задайте, сколько раундов может занять обращение.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/co-writer/00-overview.png" alt="Рабочее пространство Co-Writer EDUX" width="900">
</div>
...
Его определяющая идея — **хирургическое редактирование**: выберите фрагмент и попросите EDUX переписать, расширить или сократить его. Агент редактирования может опираться на базу знаний или веб-данные, ведёт след вызовов инструментов и показывает каждое изменение как diff с принятием/отклонением — так ничто не применяется до вашего одобрения.
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/book/00-book_overview.png" alt="Библиотека книг EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/knowledge/00-overview.png" alt="Центр Знаний EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/learning-space/00-overview.png" alt="Хаб Пространства Обучения EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/memory/00-overview.png" alt="Обзор Памяти EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/memory/01-3%20layer%20memory%20graph.png" alt="Граф Памяти EDUX" width="900">
</div>
...
<div align="center">
<img src="../../assets/figs/web-1.4.6+/settings/00-setting%20overview.png" alt="Хаб настроек EDUX" width="900">
</div>
...
Аутентификация **отключена по умолчанию** — EDUX работает в однопользовательском режиме. Включите её, и одно дерево `data/` содержит рабочее пространство администратора, изолированные пользовательские рабочие пространства и партнёрские рабочие пространства рядом:
...
**Включение:** включите аутентификацию в `data/user/settings/auth.json`, перезапустите `edux start`, зарегистрируйте первого администратора по адресу `/register`, затем добавляйте пользователей из `/admin/users` и назначайте модели, KB, навыки, Partners, политику инструментов/MCP и доступ к выполнению кода через гранты.
...
## ⌨️ EDUX CLI — Интерфейс для Агентов

Один бинарный файл `edux`, два способа входа: интерактивный **REPL** для тех, кто живёт в терминале, и структурированный **JSON** для других агентов, которые управляют EDUX как инструментом. Одни и те же возможности, инструменты и базы знаний в любом случае.

<details>
<summary><b>Управляйте им самостоятельно</b></summary>

`edux chat` открывает интерактивный REPL; `edux run <capability> "<message>"` выполняет один ход и завершается. Оба используют одинаковые флаги `--capability`, `--tool`, `--kb` и `--config`.
...
В репозитории есть корневой [`SKILL.md`](SKILL.md) — документ передачи на ~150 строк, который знакомит любой использующий инструменты LLM со всей поверхностью за одно чтение. Передайте его Claude Code, Codex или OpenCode (они автоматически подхватывают `SKILL.md`), или оберните `edux run` как инструмент в цикл LangChain / AutoGen. Полные рецепты: [Agent Handoff](https://eduorbit.com/docs/cli/agent-handoff/).
...
| Команда | Описание |
|:---|:---|
| `edux init` | Создать или обновить `data/user/settings` для текущего рабочего пространства |
| `edux start [--home PATH]` | Запустить бэкенд + фронтенд вместе |
| `edux serve [--port PORT]` | Запустить только бэкенд FastAPI |
| `edux run <capability> <message>` | Запустить один ход возможности (`chat`, `deep_solve`, `deep_question`, `deep_research`, `visualize`, `math_animator`, `mastery_path`); добавьте `--format json` для вывода NDJSON |
| `edux chat` | Интерактивный REPL с управлением возможностями, инструментами, KB, блокнотами и историей |
| `edux partner list/create/start/stop` | Управление партнёрами, подключёнными к IM |
| `edux kb list/info/create/add/search/set-default/delete` | Управление базами знаний LlamaIndex |
| `edux skill search/install/list/remove/login/logout/publish/update` | Управление навыками, установка из хабов и публикация своих (`eduhub:<slug>` по умолчанию, см. Экосистема) |
| `edux memory show/clear` | Просмотр документов памяти L2/L3 или очистка памяти L1/всей памяти |
| `edux session list/show/open/rename/delete` | Управление общими сессиями |
| `edux notebook list/create/show/add-md/replace-md/remove-record` | Управление блокнотами из файлов Markdown |
| `edux book list/health/refresh-fingerprints` | Просмотр книг и обновление исходных отпечатков |
| `edux plugin list/info` | Просмотр зарегистрированных инструментов и возможностей |
| `edux config show` | Вывод сводки конфигурации |
| `edux provider login <provider>` | Аутентификация провайдера (`openai-codex` вход через OAuth; `github-copilot` проверяет существующую сессию аутентификации Copilot) |
...
Пакет только с CLI находится в `packaging/edux-cli`. В этом чекауте установите его из исходного кода:
...
Навыки EDUX используют открытый формат **Agent-Skills** — папку с плейбуком `SKILL.md` (YAML frontmatter + Markdown) и необязательными справочными файлами. В нём нет ничего специфичного для EDUX, поэтому любой реестр, говорящий на этом формате, становится источником для вашей библиотеки. EDUX поставляется с **[EduHub](https://eduhub.eduorbit.com/)** — нашим собственным образовательным реестром навыков — встроенным в качестве хаба по умолчанию.

<details>
<summary><b>EduHub — экосистема навыков EDUX</b></summary>

[**EduHub**](https://eduhub.eduorbit.com/) — это хаб сообщества, запущенный EDUX для обмена обучающими навыками агентов — сократовские репетиторы, создатели флэш-карточек, обратная связь по эссе, планы экзаменов, объяснители концепций и многое другое. Он встроен в EDUX, поэтому настраивать ничего не нужно: голый слаг или префикс `eduhub:` разрешается в него.
...
```bash
edux skill search "socratic tutor"               # поиск в EduHub (хаб по умолчанию)
edux skill install socratic-tutor                # получить → проверить → зарегистрировать
edux skill install eduhub:socratic-tutor@1.2.0   # указать хаб и версию
edux skill list                                  # локальные навыки с их хабовой принадлежностью
```

**Опубликуйте свой** — упакуйте `SKILL.md` и поделитесь им обратно с сообществом:

```bash
edux skill login                                 # вход через браузер в EduHub
edux skill publish ./my-skill                    # интерактивно: выберите трек + теги, затем загрузите
edux skill update                                # откатиться или выпустить новую версию
```

EduHub также является отдельным, совместимым с ClawHub реестром, поэтому агенты, отличные от EDUX (Claude Code, Codex, …), могут использовать его напрямую через CLI `eduhub` — `npx eduhub install socratic-tutor`.
...
- frontmatter нормализуется по схеме EDUX, и `always:` **удаляется**, поэтому загруженный навык никогда не может принудить себя в каждый системный промпт;
...
Поскольку EDUX использует открытый формат Agent-Skills, **[ClawHub](https://clawhub.ai/)** также работает как первоклассный источник — он встроен рядом с EduHub. Выберите его с префиксом хаба:
...
EDUX — это проект с открытым исходным кодом, который ведёт [Bingxi Zhao](https://github.com/pancacake) в составе группы [EduOrbit](https://github.com/EduOrbit), и он развивается в **полностью открытом формате**, создаваясь вместе с сообществом. До сих пор у нас **НЕТ** платных онлайн-продуктов в каком-либо виде. Не стесняйтесь обращаться по адресу **bingxizhao39@gmail.com** для обсуждений, идей или сотрудничества.

### 🙏 Благодарности

Сердечная благодарность [**Chao Huang**](https://sites.google.com/view/chaoh), директору Лаборатории интеллектуальных данных @ HKU, и нашим коллегам из EduOrbit за их тёплую поддержку — особенно [**Jiahao Zhang**](https://github.com/zzhtx258), [**Zirui Guo**](https://github.com/LarFii) и [**Xubin Ren**](https://github.com/Re-bin). Мы также глубоко благодарны **сообществу открытого исходного кода**: ваши звёзды, вопросы, пул-реквесты и обсуждения каждый день формируют EDUX.

EDUX также стоит на плечах выдающихся проектов с открытым исходным кодом, которые дали нам инструменты и вдохновение:

| Проект | Роль / Вдохновение |
|:---|:---|
| [**LlamaIndex**](https://github.com/run-llama/llama_index) | Конвейер RAG и основа индексирования документов |
| [**nanobot**](https://github.com/EduOrbit/nanobot) | Ультралёгкий движок агентов, обеспечивавший оригинальный TutorBot *(EduOrbit)* |
| [**LightRAG**](https://github.com/EduOrbit/LightRAG) | Простой и быстрый RAG *(EduOrbit)* |
| [**AutoAgent**](https://github.com/EduOrbit/AutoAgent) | Фреймворк агентов без кода *(EduOrbit)* |
| [**AI-Researcher**](https://github.com/EduOrbit/AI-Researcher) | Автоматизированный исследовательский конвейер *(EduOrbit)* |
...
| [**Claude Code**](https://github.com/anthropics/claude-code) | Агентный CLI кодирования, вдохновивший цикл агента EDUX |
...
<p>
  <img src="https://visitor-badge.laobi.icu/badge?page_id=EduOrbit.EDUX&style=for-the-badge&color=00d4ff" alt="Views">
</p>
