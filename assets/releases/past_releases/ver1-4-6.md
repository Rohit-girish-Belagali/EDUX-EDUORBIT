...
- **Login** accepts a username (not just an email), surfaces bad-login errors
  ...
  flag.
- **CLI** gains `edux skill` commands — browser login plus publish/update
  for your own skills.
...
- Drop-in from v1.4.5: `pip install -U edux`; Docker users pull
  `ghcr.io/eduorbit/edux:latest`. No migrations — existing data loads as-is.
...
- New engines and services are opt-in: GraphRAG needs the extra
  (`pip install 'edux[graphrag]'`), document parsing needs
  `edux[parse]`, and voice / image / video need their model catalogs
  configured.

**Full Changelog**: https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/compare/v1.4.5...v1.4.6
