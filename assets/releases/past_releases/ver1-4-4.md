...
EDUX skills share their format with the Agent-Skills ecosystem
(`SKILL.md` + support files), so external registries are now a native
source for the skill library:

```bash
edux skill search "git release notes"        # semantic search on the hub
edux skill install clawhub:gh-release-notes  # verify → fetch → register
edux skill install some-skill@1.2.0          # hub defaults to clawhub; @ pins a version
edux skill list                              # local skills with hub provenance
```
...
- Drop-in from v1.4.3: `pip install -U edux`; Docker users pull
  `ghcr.io/eduorbit/edux:latest`. No migrations.
- `edux skill install` talks to clawhub.ai over HTTPS; air-gapped
  deployments can point a mirror or a fetch CLI at it via
  `settings/skill_hubs.json`.

**Full Changelog**: https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/compare/v1.4.3...v1.4.4
