# 🚀 EDUX v1.0.0-beta1 Release Notes
...
- **Architecture** — Complete rewrite from `src/` to `edux/` + `edux_cli/` with agent-native runtime (Tools + Capabilities).
- **Entry Points** — Three unified entry points: standalone CLI (`edux`), WebSocket API (`/api/v1/ws`), and Python SDK facade.
...
|:---|:---|
| **CLI** (`edux`) | Typer-based CLI with sub-commands: `run`, `chat`, `bot`, `kb`, `memory`, `session`, `notebook`, `plugin`, `config`, `provider`, `serve` |
...
| **Python SDK** (`edux.app.facade`) | Programmatic facade for SDK-style integrations |
...
New autonomous bot system (`edux/tutorbot/`) that brings EDUX to messaging platforms:
...
- **Package layout:** `src/` → `edux/` + `edux_cli/`. Old `src/` directory fully removed (140 files).
- **Package renamed:** `ai-tutor` → `edux`, version `1.0.0`.
...
- Complete codebase rewrite with agent-native architecture (EDUX 2.0).
- Two-layer plugin model (Tools + Capabilities) with `ChatOrchestrator` coordinator.
- Standalone CLI package (`edux_cli/`) with 11 sub-commands via Typer.
