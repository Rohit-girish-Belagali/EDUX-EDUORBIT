# EDUX v1.4.0-beta Release Notes
...
maze and make `edux start` / `edux init` first-class.
...
- **`edux/runtime/banner.py`** — localized startup banner shared
  between `edux start` and `edux init`; reads the language
  preference from interface settings so the banner matches the UI locale.
- **`init_wizard.py`** — interactive `edux init` wizard with provider
...
- **`edux start` is the recommended launcher** — `start_web.py` /
  `start_tour.py` continue to work but are now thin wrappers around the
  new `runtime/launcher.py`. Run `edux init` once to seed providers
