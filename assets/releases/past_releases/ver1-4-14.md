...
LightRAG indexing hard-failed with "Parser 'mineru' is not properly installed" even when you'd picked a different parsing engine, because RAG-Anything ran a one-time install check on its default parser. EDUX already feeds it pre-parsed content, so that spurious check is now skipped and indexing proceeds regardless of which parse engine you chose (issue #594).
...
Drop-in from v1.4.13 on the default JSON/SQLite backend: `pip install -U edux`; Docker users pull `ghcr.io/eduorbit/edux:latest`. No migrations and no behavior changes for existing single-user setups.
...
**Full Changelog**: https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/compare/v1.4.13...v1.4.14
