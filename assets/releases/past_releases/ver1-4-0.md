...
isolation hardening, and the `edux start` / `edux init` launcher)
...
- **`build_provider_extra_kwargs`** in `edux/core/agentic/client.py`
...
The `edux chat` REPL surface was tightened so it matches the public
docs at [eduorbit.com](https://eduorbit.com).

- `edux chat --config key=value` and `--config-json '<json>'` seed
...
- `edux plugin info <name>` now also returns `cli_aliases` and the
...
- `.gitignore` covers `.playwright-cli/`; stale `.playwright-cli/`
  captures, `.env.example_CN`, and `EDUX.code-workspace` removed.
...
- New `tests/cli/test_docs_contract.py` keeps the public site docs in
  sync with the CLI surface (links + `edux …` examples).
- `tests/scripts/test_docker_compose.py` + `web/tests/api-resolve-base.test.ts`
  cover the new placeholder detection.

## Upgrade Notes
- **From v1.4.0-beta:** drop-in. PyPI installs with `pip install -U edux`
...
**Full Changelog**: https://github.com/Rohit-girish-Belagali/EDUX-EDUORBIT/compare/v1.4.0-beta...v1.4.0
